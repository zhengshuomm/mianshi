...
|---|---|---|---|
| 收集模型：Push vs Pull | A: Pull model<br>B: Push model<br>C: Hybrid | A ✅ Collector 能知道哪些 target 没响应，天然支持 health check。 采集频率由平台控制，避免 agent 乱打。 ❌ 需要 Service Discovery；collector 扩缩容时要重新分配 targets。<br>B ✅ agent 主动上报，collector 后面加 LB 即可扩展。 服务频繁扩缩容时不需要 collector 立刻发现 endpoint。 ❌ collector 只收到 90 台机器上报时，不容易知道另外 10 台是挂了还是没有数据。<br>C ✅ infra/service metrics 用 pull；logs/custom metrics 用 push。 ❌ 两套采集模型，配置和排障更复杂。 | 服务和机器核心指标优先 pull，能发现 missing target。<br>应用自定义指标和 logs 用 push。<br>Push 模型下要引入 heartbeat/expected target registry，否则无法判断缺失实例。 |
| 为什么需要 Message Queue | A: Collector 直接写 TSDB<br>B: Collector -> Kafka -> Consumers -> TSDB<br>C: Agent local buffer + queue | A ✅ 路径短，延迟低。 ❌ TSDB 抖动时 collector 只能丢数据或阻塞；扩容不灵活。<br>B ✅ 解耦采集和存储；存储不可用时可缓冲；consumer 可水平扩展。 ❌ 增加端到端延迟和运维复杂度。<br>C ✅ collector 短暂不可用时 agent 可暂存。 ❌ autoscaling 机器被销毁时本地 buffer 可能丢。 | 大规模一定要 MQ。<br>Kafka partition 可按 metric name/service/tenant，让 aggregation 更有 locality。<br>Critical alerts 可使用更短链路或双写 fast path，避免队列长时间积压。 |
| Aggregation 在哪里做 | A: Agent 端聚合<br>B: Ingestion pipeline 聚合<br>C: Query-time aggregation | A ✅ 减少网络和 ingestion 压力。 ❌ agent 聚合后 raw detail 丢失；agent 逻辑复杂。<br>B ✅ 集中控制聚合逻辑；可以 batch、dedup、处理 late events。 ❌ pipeline 计算压力大，端到端延迟增加。<br>C ✅ 保留 raw data，查询灵活。 ❌ 大量 dashboard/alert 会反复扫描，成本高。 | Agent 做轻量本地聚合。<br>Ingestion pipeline 做 1min/1h rollup。<br>Raw data 保留 7 天，长期数据只保留 downsampled。 |
| Tagging 和 Cardinality | A: 只允许低 cardinality labels<br>B: 允许任意 labels<br>C: 分层存储 | A ✅ TSDB index 可控，查询快。 ❌ 排障维度有限。<br>B ✅ 业务方灵活。 ❌ `user_id/device_id/request_id` 会造成 cardinality disaster。<br>C ✅ 低 cardinality metrics 进 TSDB；高 cardinality events/logs 进 log/OLAP store。 ❌ 用户需要知道查 metrics 还是查 logs/events。 | TSDB labels 控制在 service、region、status、method、endpoint template 等。<br>禁止 `user_id`、raw URL、request_id 作为 metric label。<br>高 cardinality 调试信息放 logs/traces/OLAP。 |
| Logs PII Masking 和字段查询 | A: 应用侧自行 masking<br>B: Ingestion pipeline masking<br>C: 双层存储 | A ✅ PII 在源头就不产生。 ❌ 依赖所有业务正确实现，容易漏。<br>B ✅ 统一规则，所有日志进入热存储前脱敏。 ❌ 正则/解析可能误伤或漏掉复杂 PII。<br>C ✅ masked logs 给大多数查询；encrypted raw logs 严格 RBAC 短期保留。 ❌ 系统复杂，审计要求高。 | 应用侧规范 + ingestion 强制 masking。<br>日志结构化，字段级 masking 比纯文本正则可靠。<br>Raw PII 只能短期加密保留，并且必须审计访问。 |
| Top K Exceptions | A: Query-time group by exception<br>B: Ingestion-time fingerprint + topK<br>C: Approximate topK | A ✅ 结果灵活。 ❌ 大规模日志扫描成本高，延迟高。<br>B ✅ 查询快，能实时看到 top exceptions。 ❌ fingerprint 规则要设计好，否则相同异常被拆散或不同异常被合并。<br>C ✅ 内存低，吞吐高。 ❌ 不是精确结果，需要给用户解释误差。 | 日志进入 pipeline 时生成 `exception_hash`。<br>按 `service + version + exception_hash + time_bucket` 聚合。<br>topK 结果写 TSDB/OLAP，原始样本日志保留用于 drill-down。 |
| Alert Rule 设计 | A: 简单 threshold<br>B: Window condition<br>C: Anomaly detection | A ✅ 容易理解。 ❌ 对增长型指标如 DAU 不适合；边界抖动会误报。<br>B ✅ 例如 “过去 15 分钟内多数点超过阈值” 更稳。 ❌ 规则表达比单点 threshold 复杂。<br>C ✅ 能发现 “突然掉一半” 这类相对异常。 ❌ 模型需要训练和调参，解释性较差。 | CPU 这种：不要写 “连续 15 分钟每个点都 > 90%”，否则第 15 分钟低一点就不报警。<br>更好写法：过去 15 分钟 60 个 datapoints 中至少 45 个超过 90%，或 avg/p95 超过阈值。<br>DAU 这种增长指标：用 week-over-week、same-hour-baseline、moving average deviation 检测突降。 |
| Query Cache 和滑动窗口优化 | A: 按完整 query string 缓存<br>B: 按 time bucket 缓存<br>C: Materialized dashboard panels | A ✅ 实现简单。 ❌ `1 <= time <= 10` 和 `2 <= time <= 11` 是两个 key，命中率低。<br>B ✅ 第二次查询只需要复用 `2..10` 的 bucket，再补 `11`。 ❌ 需要 query planner merge buckets。<br>C ✅ 延迟最低，保护 TSDB。 ❌ 灵活性低，预计算成本高。 | Cache key 不只用完整 time range，而是拆成：<br>Query Service 把时间范围切成 buckets，复用已缓存 bucket。<br>最新未完成 bucket TTL 短，历史 bucket TTL 长。 |
| 如果新增指标或新增维度怎么办 | A: 固定 schema<br>B: metric name + labels 通用模型<br>C: Schema registry / metric registry | A ✅ 查询和存储可控。 ❌ 新增 disk/memory 这类指标要改 schema 或 pipeline。<br>B ✅ 新增指标不需要改表；同一时间 CPU/disk/memory 都是不同 metric name。 ❌ 必须控制 label cardinality 和命名规范。<br>C ✅ 可以治理 metric owner、type、unit、allowed labels。 ❌ 增加治理流程。 | 用通用 time-series 模型：<br>新增 disk/memory 只是新增 metric_name 或 allowed label。<br>用 metric registry 防止乱打高 cardinality 标签。 |
| 可靠性和故障模式 | A: Best-effort metrics<br>B: Durable ingestion<br>C: Multi-region / replicated TSDB | A ✅ 成本低。 ❌ incident 时最需要数据，结果可能丢。<br>B ✅ Kafka/WAL 缓冲，TSDB 故障时可恢复写入。 ❌ 成本和延迟更高。<br>C ✅ 单 region 故障仍可报警。 ❌ 跨 region 成本、重复数据、查询一致性复杂。 | Collector、Kafka、TSDB 都要多副本。<br>Alert Manager active-passive 或 shard by rule，并持久化 alert state。<br>监控系统需要自监控：ingestion lag、drop rate、query latency、alert evaluation delay。 |
