...
| --- | --- | --- | --- | --- |
| 收集模型：Push vs Pull | Pull model<br>✅ Collector 能知道哪些 target 没响应，天然支持 health che...<br>❌ 需要 Service Discovery | Push model<br>✅ agent 主动上报，collector 后面加 LB 即可扩展<br>❌ collector 只收到 90 台机器上报时，不容易知道另外 10 台是挂了还是没... | Hybrid<br>✅ infra/service metrics 用 pull<br>❌ 两套采集模型，配置和排障更复杂 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 为什么需要 Message Queue | Collector 直接写 TSDB<br>✅ 路径短，延迟低<br>❌ TSDB 抖动时 collector 只能丢数据或阻塞 | Collector -> Kafka -> Consumers -> TSDB<br>✅ 解耦采集和存储<br>❌ 增加端到端延迟和运维复杂度 | Agent local buffer + queue<br>✅ collector 短暂不可用时 agent 可暂存<br>❌ autoscaling 机器被销毁时本地 buffer 可能丢 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
| Aggregation 在哪里做 | Agent 端聚合<br>✅ 减少网络和 ingestion 压力<br>❌ agent 聚合后 raw detail 丢失 | Ingestion pipeline 聚合<br>✅ 集中控制聚合逻辑<br>❌ pipeline 计算压力大，端到端延迟增加 | Query-time aggregation<br>✅ 保留 raw data，查询灵活<br>❌ 大量 dashboard/alert 会反复扫描，成本高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Tagging 和 Cardinality | 只允许低 cardinality labels<br>✅ TSDB index 可控，查询快<br>❌ 排障维度有限 | 允许任意 labels<br>✅ 业务方灵活<br>❌ `user_id/device_id/request_id` 会造成 cardina... | 分层存储<br>✅ 低 cardinality metrics 进 TSDB<br>❌ 用户需要知道查 metrics 还是查 logs/events | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Logs PII Masking 和字段查询 | 应用侧自行 masking<br>✅ PII 在源头就不产生<br>❌ 依赖所有业务正确实现，容易漏 | Ingestion pipeline masking<br>✅ 统一规则，所有日志进入热存储前脱敏<br>❌ 正则/解析可能误伤或漏掉复杂 PII | 双层存储<br>✅ masked logs 给大多数查询<br>❌ 系统复杂，审计要求高 | 我会把 privacy 和 authorization 当成 correctness requirement。前端过滤只能改善体验，真正防泄露要靠服务端强校验、audit log、删除语义和 least-privilege access。 |
| Top K Exceptions | Query-time group by exception<br>✅ 结果灵活<br>❌ 大规模日志扫描成本高，延迟高 | Ingestion-time fingerprint + topK<br>✅ 查询快，能实时看到 top exceptions<br>❌ fingerprint 规则要设计好，否则相同异常被拆散或不同异常被合并 | Approximate topK<br>✅ 内存低，吞吐高<br>❌ 不是精确结果，需要给用户解释误差 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Alert Rule 设计 | 简单 threshold<br>✅ 容易理解<br>❌ 对增长型指标如 DAU 不适合 | Window condition<br>✅ 例如 “过去 15 分钟内多数点超过阈值” 更稳<br>❌ 规则表达比单点 threshold 复杂 | Anomaly detection<br>✅ 能发现 “突然掉一半” 这类相对异常<br>❌ 模型需要训练和调参，解释性较差 | 我会把 observability 当成架构的一部分，而不是事后加 dashboard。关键链路需要 request_id、structured events、high-cardinality 控制、SLO alert，以及足够 raw data 支撑 debug/replay。 |
| Query Cache 和滑动窗口优化 | 按完整 query string 缓存<br>✅ 实现简单<br>❌ `1 <= time <= 10` 和 `2 <= time <= 11` 是两个... | 按 time bucket 缓存<br>✅ 第二次查询只需要复用 `2..10` 的 bucket，再补 `11`<br>❌ 需要 query planner merge buckets | Materialized dashboard panels<br>✅ 延迟最低，保护 TSDB<br>❌ 灵活性低，预计算成本高 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| 如果新增指标或新增维度怎么办 | 固定 schema<br>✅ 查询和存储可控<br>❌ 新增 disk/memory 这类指标要改 schema 或 pipeline | metric name + labels 通用模型<br>✅ 新增指标不需要改表<br>❌ 必须控制 label cardinality 和命名规范 | Schema registry / metric registry<br>✅ 可以治理 metric owner、type、unit、allowed labels<br>❌ 增加治理流程 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 可靠性和故障模式 | Best-effort metrics<br>✅ 成本低<br>❌ incident 时最需要数据，结果可能丢 | Durable ingestion<br>✅ Kafka/WAL 缓冲，TSDB 故障时可恢复写入<br>❌ 成本和延迟更高 | Multi-region / replicated TSDB<br>✅ 单 region 故障仍可报警<br>❌ 跨 region 成本、重复数据、查询一致性复杂 | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
