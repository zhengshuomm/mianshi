...
| --- | --- | --- | --- | --- |
| 收集模型：Push vs Pull | Pull model<br>✅ Collector 能知道哪些 target 没响应，天然支持 health che...<br>❌ 需要 Service Discovery | Push model<br>✅ agent 主动上报，collector 后面加 LB 即可扩展<br>❌ collector 只收到 90 台机器上报时，不容易知道另外 10 台是挂了还是没... | Hybrid<br>✅ infra/service metrics 用 pull<br>❌ 两套采集模型，配置和排障更复杂 | 服务和机器核心指标优先 pull，能发现 missing target。<br>应用自定义指标和 logs 用 push。 |
| 为什么需要 Message Queue | Collector 直接写 TSDB<br>✅ 路径短，延迟低<br>❌ TSDB 抖动时 collector 只能丢数据或阻塞 | Collector -> Kafka -> Consumers -> TSDB<br>✅ 解耦采集和存储<br>❌ 增加端到端延迟和运维复杂度 | Agent local buffer + queue<br>✅ collector 短暂不可用时 agent 可暂存<br>❌ autoscaling 机器被销毁时本地 buffer 可能丢 | 大规模一定要 MQ。<br>Kafka partition 可按 metric name/service/tenant，让 aggregation 更有 locality。 |
| Aggregation 在哪里做 | Agent 端聚合<br>✅ 减少网络和 ingestion 压力<br>❌ agent 聚合后 raw detail 丢失 | Ingestion pipeline 聚合<br>✅ 集中控制聚合逻辑<br>❌ pipeline 计算压力大，端到端延迟增加 | Query-time aggregation<br>✅ 保留 raw data，查询灵活<br>❌ 大量 dashboard/alert 会反复扫描，成本高 | Agent 做轻量本地聚合。<br>Ingestion pipeline 做 1min/1h rollup。 |
| Tagging 和 Cardinality | 只允许低 cardinality labels<br>✅ TSDB index 可控，查询快<br>❌ 排障维度有限 | 允许任意 labels<br>✅ 业务方灵活<br>❌ `user_id/device_id/request_id` 会造成 cardina... | 分层存储<br>✅ 低 cardinality metrics 进 TSDB<br>❌ 用户需要知道查 metrics 还是查 logs/events | TSDB labels 控制在 service、region、status、method、endpoint template 等。<br>禁止 `user_id`、raw URL、request_id 作为 metric label。 |
| Logs PII Masking 和字段查询 | 应用侧自行 masking<br>✅ PII 在源头就不产生<br>❌ 依赖所有业务正确实现，容易漏 | Ingestion pipeline masking<br>✅ 统一规则，所有日志进入热存储前脱敏<br>❌ 正则/解析可能误伤或漏掉复杂 PII | 双层存储<br>✅ masked logs 给大多数查询<br>❌ 系统复杂，审计要求高 | 应用侧规范 + ingestion 强制 masking。<br>日志结构化，字段级 masking 比纯文本正则可靠。 |
| Top K Exceptions | Query-time group by exception<br>✅ 结果灵活<br>❌ 大规模日志扫描成本高，延迟高 | Ingestion-time fingerprint + topK<br>✅ 查询快，能实时看到 top exceptions<br>❌ fingerprint 规则要设计好，否则相同异常被拆散或不同异常被合并 | Approximate topK<br>✅ 内存低，吞吐高<br>❌ 不是精确结果，需要给用户解释误差 | 日志进入 pipeline 时生成 `exception_hash`。<br>按 `service + version + exception_hash + time_bucket` 聚合。 |
| Alert Rule 设计 | 简单 threshold<br>✅ 容易理解<br>❌ 对增长型指标如 DAU 不适合 | Window condition<br>✅ 例如 “过去 15 分钟内多数点超过阈值” 更稳<br>❌ 规则表达比单点 threshold 复杂 | Anomaly detection<br>✅ 能发现 “突然掉一半” 这类相对异常<br>❌ 模型需要训练和调参，解释性较差 | CPU 这种：不要写 “连续 15 分钟每个点都 > 90%”，否则第 15 分钟低一点就不报警。<br>更好写法：过去 15 分钟 60 个 datapoints 中至少 45 个超过 90%，或 avg/p95 超过阈值。 |
| Query Cache 和滑动窗口优化 | 按完整 query string 缓存<br>✅ 实现简单<br>❌ `1 <= time <= 10` 和 `2 <= time <= 11` 是两个... | 按 time bucket 缓存<br>✅ 第二次查询只需要复用 `2..10` 的 bucket，再补 `11`<br>❌ 需要 query planner merge buckets | Materialized dashboard panels<br>✅ 延迟最低，保护 TSDB<br>❌ 灵活性低，预计算成本高 | Cache key 不只用完整 time range，而是拆成：<br>Query Service 把时间范围切成 buckets，复用已缓存 bucket。 |
| 如果新增指标或新增维度怎么办 | 固定 schema<br>✅ 查询和存储可控<br>❌ 新增 disk/memory 这类指标要改 schema 或 pipeline | metric name + labels 通用模型<br>✅ 新增指标不需要改表<br>❌ 必须控制 label cardinality 和命名规范 | Schema registry / metric registry<br>✅ 可以治理 metric owner、type、unit、allowed labels<br>❌ 增加治理流程 | 用通用 time-series 模型：<br>新增 disk/memory 只是新增 metric_name 或 allowed label。 |
| 可靠性和故障模式 | Best-effort metrics<br>✅ 成本低<br>❌ incident 时最需要数据，结果可能丢 | Durable ingestion<br>✅ Kafka/WAL 缓冲，TSDB 故障时可恢复写入<br>❌ 成本和延迟更高 | Multi-region / replicated TSDB<br>✅ 单 region 故障仍可报警<br>❌ 跨 region 成本、重复数据、查询一致性复杂 | Collector、Kafka、TSDB 都要多副本。<br>Alert Manager active-passive 或 shard by rule，并持久化 alert state。 |
