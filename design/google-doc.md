...
| --- | --- | --- | --- | --- |
| OT vs CRDT | Operational Transformation<br>✅ Google Docs 早期常用<br>❌ transformation 逻辑复杂 | CRDT<br>✅ 数学上保证最终收敛<br>❌ metadata 开销大 | Hybrid<br>✅ 在线时走中心 session 保低延迟和顺序<br>❌ 工程复杂度最高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Leader/Follower DB 和 Multi-DC | OT + leader/follower<br>✅ 每个 doc 的 op 顺序明确<br>❌ 跨 region 用户可能延迟高 | CRDT + multi-master<br>✅ 多个 DC 并发写可以 merge<br>❌ 存储和协议复杂，用户意图冲突可能仍需要产品层解释 | Doc home region<br>✅ 减少跨 DC 协调<br>❌ 远端协作者写入延迟较高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Version History：Snapshot vs Delta vs Hybrid | Snapshot-based versioning<br>✅ 读取历史版本快<br>❌ 存储成本高，每次版本都存全文 | Delta-based versioning<br>✅ 存储省<br>❌ 恢复历史版本要 replay 很多 ops，时间长 | Hybrid<br>✅ 定期 snapshot + 中间 delta，平衡存储和读取延迟<br>❌ compaction、snapshot consistency、restore 逻辑... | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Prevent Server Overload from Frequent Edits | 每个 keystroke 立即发 server<br>✅ 实现简单，延迟最低<br>❌ 高 QPS，server 和网络压力大 | Client-side batching<br>✅ 把多个小 edits 合并，显著降低 op 数<br>❌ 远端看到更新有几十到几百毫秒延迟 | Server-side coalescing + rate limiting<br>✅ 保护 server<br>❌ 过度合并可能影响实时体验 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Offline Edit Conflict Resolution | Pessimistic lock，离线不允许编辑<br>✅ 冲突少，实现简单<br>❌ 用户体验差，Google Docs 类产品不可接受 | Optimistic offline queue + sync<br>✅ 离线可继续编辑<br>❌ 冲突解决复杂，需要 OT transform 或 CRDT merge | Fork + manual merge<br>✅ 不会误合并破坏内容<br>❌ 用户体验差，适合少数冲突场景 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 打开的文档 vs 未打开的文档 | 只维护打开文档的内存 session<br>✅ 活跃文档低延迟<br>❌ 冷文档不能依赖 session 更新 | 所有修改都走持久化 OpLog<br>✅ 未打开文档也能被后台任务、安全扫描、导入 API 更新<br>❌ 所有 writer 必须遵循统一 op/version 协议 | 后台 materialization<br>✅ 不占用在线 session，仍能保持可打开性<br>❌ 索引和快照可能短暂落后 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Pub/Sub Fanout 和可靠性 | 直接用 Redis Pub/Sub 作为唯一通道<br>✅ 低延迟，简单<br>❌ 不持久，断线会丢消息 | OpLog + Pub/Sub<br>✅ OpLog 保证文档 ops 可恢复，Pub/Sub 只负责实时 fanout<br>❌ 两套路径，client 需要处理 missed ops | Kafka fanout<br>✅ 持久化，可 replay<br>❌ 端到端延迟和连接 fanout 复杂 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Cache Strategy | 缓存完整文档<br>✅ 打开快<br>❌ 实时编辑频繁 invalidation，容易 stale | 缓存 snapshot + recent ops<br>✅ 读打开快，近期编辑可 replay<br>❌ cache 和 OpLog version 要一致 | 只缓存 metadata/presence<br>✅ 一致性简单<br>❌ 冷启动打开大文档慢 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
