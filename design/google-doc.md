...
|---|---|---|---|
| OT vs CRDT | A: Operational Transformation<br>B: CRDT<br>C: Hybrid | A ✅ Google Docs 早期常用；可以保留用户意图；server 统一排序后结果可控。 ❌ transformation 逻辑复杂；通常需要 central server/leader 协调；多 master 跨 DC 难做。<br>B ✅ 数学上保证最终收敛；操作可交换；不需要中心 server transformation。 ❌ metadata 开销大；实现复杂；文本 CRDT 的性能和 compaction 要小心。<br>C ✅ 在线时走中心 session 保低延迟和顺序；离线/跨 DC 用 CRDT-like merge 或明确冲突策略。 ❌ 工程复杂度最高。 | 面试基础版选 OT，因为 Google Docs 经典答案清楚。<br>Staff+ 可以补充：CRDT 更适合离线和 multi-master，Redis CRDT 用来解决跨 DC 并发写和网络分区下自动合并。<br>如果系统要求强实时、中心化编辑，OT + per-doc leader 更直观；如果强调离线和多主，CRDT 更自然。 |
| Leader/Follower DB 和 Multi-DC | A: OT + leader/follower<br>B: CRDT + multi-master<br>C: Doc home region | A ✅ 每个 doc 的 op 顺序明确；冲突处理在 leader 完成。 ❌ 跨 region 用户可能延迟高；leader failover 要处理未确认 ops。<br>B ✅ 多个 DC 并发写可以 merge；网络分区后最终收敛。 ❌ 存储和协议复杂，用户意图冲突可能仍需要产品层解释。<br>C ✅ 减少跨 DC 协调；每个 doc 的 leader 清晰。 ❌ 远端协作者写入延迟较高。 | 默认 doc home region + leader/follower。<br>全球协作热点文档可迁移 home region 或使用更强 CRDT/multi-master 方案。<br>不要让每个 op 都跨全球共识。 |
| Version History：Snapshot vs Delta vs Hybrid | A: Snapshot-based versioning<br>B: Delta-based versioning<br>C: Hybrid | A ✅ 读取历史版本快；恢复简单。 ❌ 存储成本高，每次版本都存全文。<br>B ✅ 存储省；自然基于 op log。 ❌ 恢复历史版本要 replay 很多 ops，时间长。<br>C ✅ 定期 snapshot + 中间 delta，平衡存储和读取延迟。 ❌ compaction、snapshot consistency、restore 逻辑更复杂。 | 使用 hybrid。<br>每 N ops 或 N 分钟生成 snapshot。<br>用户 restore 历史版本时创建新版本，不破坏历史链。 |
| Prevent Server Overload from Frequent Edits | A: 每个 keystroke 立即发 server<br>B: Client-side batching<br>C: Server-side coalescing + rate limiting | A ✅ 实现简单，延迟最低。 ❌ 高 QPS，server 和网络压力大。<br>B ✅ 把多个小 edits 合并，显著降低 op 数。 ❌ 远端看到更新有几十到几百毫秒延迟。<br>C ✅ 保护 server；对 cursor/presence 可降频。 ❌ 过度合并可能影响实时体验。 | Client 端 50-200ms batching。<br>Cursor/presence 限频。<br>Server 对单用户、单文档、单连接做 rate limit/backpressure。 |
| Offline Edit Conflict Resolution | A: Pessimistic lock，离线不允许编辑<br>B: Optimistic offline queue + sync<br>C: Fork + manual merge | A ✅ 冲突少，实现简单。 ❌ 用户体验差，Google Docs 类产品不可接受。<br>B ✅ 离线可继续编辑；在线后同步。 ❌ 冲突解决复杂，需要 OT transform 或 CRDT merge。<br>C ✅ 不会误合并破坏内容。 ❌ 用户体验差，适合少数冲突场景。 | 默认 optimistic offline edits。<br>文本内容用 OT/CRDT 自动 merge。<br>无法合并的对象，如图片位置、表格结构冲突，可用 last-writer-wins 或提示用户解决。 |
| 打开的文档 vs 未打开的文档 | A: 只维护打开文档的内存 session<br>B: 所有修改都走持久化 OpLog<br>C: 后台 materialization | A ✅ 活跃文档低延迟。 ❌ 冷文档不能依赖 session 更新。<br>B ✅ 未打开文档也能被后台任务、安全扫描、导入 API 更新。 ❌ 所有 writer 必须遵循统一 op/version 协议。<br>C ✅ 不占用在线 session，仍能保持可打开性。 ❌ 索引和快照可能短暂落后。 | 打开文档：Document Session 提供实时协作。<br>未打开文档：OpLog + Snapshot Worker 负责持久化和更新。<br>任何写入都不能绕过 OpLog 直接改最终 snapshot。 |
| Pub/Sub Fanout 和可靠性 | A: 直接用 Redis Pub/Sub 作为唯一通道<br>B: OpLog + Pub/Sub<br>C: Kafka fanout | A ✅ 低延迟，简单。 ❌ 不持久，断线会丢消息。<br>B ✅ OpLog 保证文档 ops 可恢复，Pub/Sub 只负责实时 fanout。 ❌ 两套路径，client 需要处理 missed ops。<br>C ✅ 持久化，可 replay。 ❌ 端到端延迟和连接 fanout 复杂。 | 文档修改：先 commit OpLog，再 fanout。<br>Presence/cursor：Redis Pub/Sub + TTL 即可。<br>Client 重连后用 last_seen_version 拉 missed ops。 |
| Cache Strategy | A: 缓存完整文档<br>B: 缓存 snapshot + recent ops<br>C: 只缓存 metadata/presence | A ✅ 打开快。 ❌ 实时编辑频繁 invalidation，容易 stale。<br>B ✅ 读打开快，近期编辑可 replay。 ❌ cache 和 OpLog version 要一致。<br>C ✅ 一致性简单。 ❌ 冷启动打开大文档慢。 | 缓存 snapshot by `doc_id + snapshot_version`，不可变缓存。<br>recent ops 从 OpLog 拉。<br>不缓存可变 “current doc” 而不带 version，否则容易读到混合状态。 |
