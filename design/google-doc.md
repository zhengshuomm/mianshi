...
| --- | --- | --- | --- | --- |
| OT vs CRDT | Operational Transformation<br>✅ Google Docs 早期常用<br>❌ transformation 逻辑复杂 | CRDT<br>✅ 数学上保证最终收敛<br>❌ metadata 开销大 | Hybrid<br>✅ 在线时走中心 session 保低延迟和顺序<br>❌ 工程复杂度最高 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Leader/Follower DB 和 Multi-DC | OT + leader/follower<br>✅ 每个 doc 的 op 顺序明确<br>❌ 跨 region 用户可能延迟高 | CRDT + multi-master<br>✅ 多个 DC 并发写可以 merge<br>❌ 存储和协议复杂，用户意图冲突可能仍需要产品层解释 | Doc home region<br>✅ 减少跨 DC 协调<br>❌ 远端协作者写入延迟较高 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Version History：Snapshot vs Delta vs Hybrid | Snapshot-based versioning<br>✅ 读取历史版本快<br>❌ 存储成本高，每次版本都存全文 | Delta-based versioning<br>✅ 存储省<br>❌ 恢复历史版本要 replay 很多 ops，时间长 | Hybrid<br>✅ 定期 snapshot + 中间 delta，平衡存储和读取延迟<br>❌ compaction、snapshot consistency、restore 逻辑... | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| Prevent Server Overload from Frequent Edits | 每个 keystroke 立即发 server<br>✅ 实现简单，延迟最低<br>❌ 高 QPS，server 和网络压力大 | Client-side batching<br>✅ 把多个小 edits 合并，显著降低 op 数<br>❌ 远端看到更新有几十到几百毫秒延迟 | Server-side coalescing + rate limiting<br>✅ 保护 server<br>❌ 过度合并可能影响实时体验 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Offline Edit Conflict Resolution | Pessimistic lock，离线不允许编辑<br>✅ 冲突少，实现简单<br>❌ 用户体验差，Google Docs 类产品不可接受 | Optimistic offline queue + sync<br>✅ 离线可继续编辑<br>❌ 冲突解决复杂，需要 OT transform 或 CRDT merge | Fork + manual merge<br>✅ 不会误合并破坏内容<br>❌ 用户体验差，适合少数冲突场景 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 打开的文档 vs 未打开的文档 | 只维护打开文档的内存 session<br>✅ 活跃文档低延迟<br>❌ 冷文档不能依赖 session 更新 | 所有修改都走持久化 OpLog<br>✅ 未打开文档也能被后台任务、安全扫描、导入 API 更新<br>❌ 所有 writer 必须遵循统一 op/version 协议 | 后台 materialization<br>✅ 不占用在线 session，仍能保持可打开性<br>❌ 索引和快照可能短暂落后 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Pub/Sub Fanout 和可靠性 | 直接用 Redis Pub/Sub 作为唯一通道<br>✅ 低延迟，简单<br>❌ 不持久，断线会丢消息 | OpLog + Pub/Sub<br>✅ OpLog 保证文档 ops 可恢复，Pub/Sub 只负责实时 fanout<br>❌ 两套路径，client 需要处理 missed ops | Kafka fanout<br>✅ 持久化，可 replay<br>❌ 端到端延迟和连接 fanout 复杂 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Cache Strategy | 缓存完整文档<br>✅ 打开快<br>❌ 实时编辑频繁 invalidation，容易 stale | 缓存 snapshot + recent ops<br>✅ 读打开快，近期编辑可 replay<br>❌ cache 和 OpLog version 要一致 | 只缓存 metadata/presence<br>✅ 一致性简单<br>❌ 冷启动打开大文档慢 | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
