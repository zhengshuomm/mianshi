...
| --- | --- | --- | --- | --- |
| 数据分区：Range Partition vs Consistent Hash | Range partition<br>✅ range query 友好<br>❌ 顺序写容易打到同一个 partition | Consistent hash<br>✅ 扩容缩容时只迁移少量 key<br>❌ range query 很差 | Virtual nodes<br>✅ rebalance 粒度小<br>❌ metadata 更多 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 复制策略：Leader-Follower Async vs Quorum | leader + followers async replication<br>✅ 写延迟低<br>❌ leader ack 后宕机但还没复制，可能丢最新写 | quorum write/read<br>✅ `W + R > N` 时可读到较新版本<br>❌ 写延迟更高 | sync replication to all<br>✅ 一致性最强<br>❌ 可用性和延迟差，一个慢副本拖慢整体 | 我会按访问模式和恢复语义选择存储，而不是只说能 scale。Staff 级别要讲清 write amplification、read latency、replication consistency，以及系统如何从 WAL、snapshot 或 replay 恢复。 |
| Leader Election 和 WAL Recovery | 简单 follower election<br>✅ 读写路径清晰<br>❌ 必须保证新 leader 拥有足够新的 log，否则会丢写或回滚 | 基于 consensus 的 election<br>✅ leader 选举和 log commit 有严格一致性<br>❌ 吞吐和延迟成本高，不适合所有数据都走共识 | Dynamo-style 无 leader<br>✅ 任意 coordinator 可写，可用性高<br>❌ 冲突解决、read repair、vector clock 等复杂 | 我会按访问模式和恢复语义选择存储，而不是只说能 scale。Staff 级别要讲清 write amplification、read latency、replication consistency，以及系统如何从 WAL、snapshot 或 replay 恢复。 |
| 临时离线：Hinted Handoff vs Full Repair | hinted handoff<br>✅ 恢复快<br>❌ hint 存储有限 | read repair<br>✅ 按需修复，成本分散<br>❌ 冷数据长期不被读就长期不修复 | anti-entropy repair / snapshot rebuild<br>✅ 能系统性修复副本差异<br>❌ IO 和网络成本高，需要限速，避免影响线上读写 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 存储引擎：LSM vs B+Tree vs Columnar | LSM Tree<br>✅ 顺序写，适合 SSD<br>❌ compaction 带来 write amplification | B+Tree<br>✅ 查询路径稳定<br>❌ 随机写多 | Columnar storage<br>✅ 压缩率高，聚合快<br>❌ point lookup KV 不合适 | 我会按访问模式和恢复语义选择存储，而不是只说能 scale。Staff 级别要讲清 write amplification、read latency、replication consistency，以及系统如何从 WAL、snapshot 或 replay 恢复。 |
| 读路径优化：Bloom Filter / SSTable Index / Cache | 只扫 SSTables<br>✅ 实现简单<br>❌ SSTable 多了以后读放大严重 | Bloom filter + sparse index<br>✅ Bloom filter 快速排除不含 key 的 SSTable<br>❌ Bloom filter 有 false positive | Block cache / Row cache<br>✅ 显著降低 SSD IO<br>❌ cache invalidation 和内存管理复杂 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| 节点替换：ZK 健康检查 vs Controller 接管 | ZooKeeper 直接管理所有逻辑<br>✅ 看起来简单<br>❌ ZK 不适合做复杂 orchestration 和数据迁移 | ZK/etcd 做 membership，Controller 做决策<br>✅ 职责清晰<br>❌ 需要实现 controller 状态机和任务调度 | 手动替换<br>✅ 实现成本低<br>❌ 恢复慢，容易人为操作错误 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
