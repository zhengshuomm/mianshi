...
| --- | --- | --- | --- | --- |
| 数据分区：Range Partition vs Consistent Hash | Range partition<br>✅ range query 友好<br>❌ 顺序写容易打到同一个 partition | Consistent hash<br>✅ 扩容缩容时只迁移少量 key<br>❌ range query 很差 | Virtual nodes<br>✅ rebalance 粒度小<br>❌ metadata 更多 | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| 复制策略：Leader-Follower Async vs Quorum | leader + followers async replication<br>✅ 写延迟低<br>❌ leader ack 后宕机但还没复制，可能丢最新写 | quorum write/read<br>✅ `W + R > N` 时可读到较新版本<br>❌ 写延迟更高 | sync replication to all<br>✅ 一致性最强<br>❌ 可用性和延迟差，一个慢副本拖慢整体 | I would choose storage based on access patterns and recovery semantics, not just scale. The key Staff-level trade-off is write amplification, read latency, replication consistency, and how quickly the system can recover from WAL, snapshot, or replay. |
| Leader Election 和 WAL Recovery | 简单 follower election<br>✅ 读写路径清晰<br>❌ 必须保证新 leader 拥有足够新的 log，否则会丢写或回滚 | 基于 consensus 的 election<br>✅ leader 选举和 log commit 有严格一致性<br>❌ 吞吐和延迟成本高，不适合所有数据都走共识 | Dynamo-style 无 leader<br>✅ 任意 coordinator 可写，可用性高<br>❌ 冲突解决、read repair、vector clock 等复杂 | I would choose storage based on access patterns and recovery semantics, not just scale. The key Staff-level trade-off is write amplification, read latency, replication consistency, and how quickly the system can recover from WAL, snapshot, or replay. |
| 临时离线：Hinted Handoff vs Full Repair | hinted handoff<br>✅ 恢复快<br>❌ hint 存储有限 | read repair<br>✅ 按需修复，成本分散<br>❌ 冷数据长期不被读就长期不修复 | anti-entropy repair / snapshot rebuild<br>✅ 能系统性修复副本差异<br>❌ IO 和网络成本高，需要限速，避免影响线上读写 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 存储引擎：LSM vs B+Tree vs Columnar | LSM Tree<br>✅ 顺序写，适合 SSD<br>❌ compaction 带来 write amplification | B+Tree<br>✅ 查询路径稳定<br>❌ 随机写多 | Columnar storage<br>✅ 压缩率高，聚合快<br>❌ point lookup KV 不合适 | I would choose storage based on access patterns and recovery semantics, not just scale. The key Staff-level trade-off is write amplification, read latency, replication consistency, and how quickly the system can recover from WAL, snapshot, or replay. |
| 读路径优化：Bloom Filter / SSTable Index / Cache | 只扫 SSTables<br>✅ 实现简单<br>❌ SSTable 多了以后读放大严重 | Bloom filter + sparse index<br>✅ Bloom filter 快速排除不含 key 的 SSTable<br>❌ Bloom filter 有 false positive | Block cache / Row cache<br>✅ 显著降低 SSD IO<br>❌ cache invalidation 和内存管理复杂 | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
| 节点替换：ZK 健康检查 vs Controller 接管 | ZooKeeper 直接管理所有逻辑<br>✅ 看起来简单<br>❌ ZK 不适合做复杂 orchestration 和数据迁移 | ZK/etcd 做 membership，Controller 做决策<br>✅ 职责清晰<br>❌ 需要实现 controller 状态机和任务调度 | 手动替换<br>✅ 实现成本低<br>❌ 恢复慢，容易人为操作错误 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
