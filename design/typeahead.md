...
| --- | --- | --- | --- | --- |
| Prefix Index：Trie vs Prefix Hashtable | Trie<br>✅ Prefix 共享，空间效率好<br>❌ 实现复杂 | Prefix Hashtable<br>✅ 查询非常快，`O(1)` lookup<br>❌ 存储膨胀，同一个 suggestion 会重复出现在多个 prefix | Hybrid<br>✅ 短 prefix 查询极快<br>❌ 两套结构，构建和调试复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 更新链路：Message Queue 实时处理 vs Raw Log Offline Processing | Raw log offline processing<br>✅ 可以做复杂清洗、去 spam、质量过滤<br>❌ 新鲜度差，热点事件反应慢 | Message queue + stream processing<br>✅ 新鲜度高，可以分钟级更新<br>❌ 容易被 spam 或突发噪音污染 | Offline static index + realtime overlay<br>✅ 静态结果稳定，实时 overlay 补热点<br>❌ Query path 要 merge/dedup/rerank | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Real-time Trending：滑动窗口 vs Decay Score | 固定滑动窗口 count<br>✅ 语义清晰<br>❌ 窗口边界会有跳变 | Decayed score<br>✅ 排名更平滑<br>❌ 结果不是严格“过去 N 分钟” | 短窗口 + 长窗口混合<br>✅ 短窗口发现爆发，长窗口提供稳定性<br>❌ Scoring 和调参复杂 | I would first clarify whether the query requires fixed windows, arbitrary windows, exact counts, or approximate trends. Fixed hot windows can be precomputed; arbitrary historical windows belong in OLAP, and late or duplicate events require event-time processing plus reconciliation. |
| Ranking：popularity vs personalization vs business rules | 只按 global popularity<br>✅ 简单稳定<br>❌ 不够个性化 | Contextual ranking<br>✅ 结果更相关<br>❌ Cache key 变多，命中率下降 | Personalized rerank<br>✅ 用户体验最好<br>❌ 延迟更高 | I would separate the write path for scores from the read model for ranks. Redis sorted sets are excellent for hot Top-K serving, but durable score history, anti-cheat review, and shard-level aggregation should remain recoverable from the source of truth. |
| 缓存策略：全量 prefix cache vs 热门短 prefix cache | 缓存所有 prefix<br>✅ 查询快<br>❌ Prefix 组合太多，内存浪费 | 只缓存热门短 prefix<br>✅ 命中高，内存可控<br>❌ 长尾 prefix 仍要查 index | Edge cache + service local memory index<br>✅ 边缘挡住大部分重复短 prefix<br>❌ Index rollout 和 cache invalidation 更复杂 | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
| Index 更新：增量更新 vs 全量 rebuild | 全量 rebuild<br>✅ 构建结果一致，容易 rollback<br>❌ 新鲜度不够 | 增量更新<br>✅ 新鲜度更好<br>❌ Index mutation 复杂 | 全量 base index + realtime overlay<br>✅ Base index 可控，overlay 负责新鲜热点<br>❌ Query merge 复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Quality Metrics：Latency vs Picking Rate | 只看 latency<br>✅ 容易监控<br>❌ 快但结果差，用户仍然不会选 | Picking rate / CTR<br>✅ 直接反映用户是否选择建议词<br>❌ 受 position bias、UI、用户意图影响 | Latency + picking rate + downstream success<br>✅ 同时衡量体验、质量、业务结果<br>❌ 指标体系更复杂，需要实验平台 | I would design observability as part of the architecture, not a dashboard added later. Each critical path needs request IDs, structured events, high-cardinality controls, SLO-oriented alerts, and enough raw data to debug or replay incidents. |
