...
| Real-time Trending：滑动窗口 vs Decay Score | 固定滑动窗口 count<br>✅ 语义清晰<br>❌ 窗口边界会有跳变 | Decayed score<br>✅ 排名更平滑<br>❌ 结果不是严格“过去 N 分钟” | 短窗口 + 长窗口混合<br>✅ 短窗口发现爆发，长窗口提供稳定性<br>❌ Scoring 和调参复杂 | 我会先问清固定窗口还是任意窗口、精确统计还是近似趋势。固定热窗口可以预计算；任意历史窗口应该走 OLAP；late/duplicate event 需要 event-time processing 和 reconciliation。 |
| Ranking：popularity vs personalization vs business rules | 只按 global popularity<br>✅ 简单稳定<br>❌ 不够个性化 | Contextual ranking<br>✅ 结果更相关<br>❌ Cache key 变多，命中率下降 | Personalized rerank<br>✅ 用户体验最好<br>❌ 延迟更高 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| 缓存策略：全量 prefix cache vs 热门短 prefix cache | 缓存所有 prefix<br>✅ 查询快<br>❌ Prefix 组合太多，内存浪费 | 只缓存热门短 prefix<br>✅ 命中高，内存可控<br>❌ 长尾 prefix 仍要查 index | Edge cache + service local memory index<br>✅ 边缘挡住大部分重复短 prefix<br>❌ Index rollout 和 cache invalidation 更复杂 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| Index 更新：增量更新 vs 全量 rebuild | 全量 rebuild<br>✅ 构建结果一致，容易 rollback<br>❌ 新鲜度不够 | 增量更新<br>✅ 新鲜度更好<br>❌ Index mutation 复杂 | 全量 base index + realtime overlay<br>✅ Base index 可控，overlay 负责新鲜热点<br>❌ Query merge 复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Quality Metrics：Latency vs Picking Rate | 只看 latency<br>✅ 容易监控<br>❌ 快但结果差，用户仍然不会选 | Picking rate / CTR<br>✅ 直接反映用户是否选择建议词<br>❌ 受 position bias、UI、用户意图影响 | Latency + picking rate + downstream success<br>✅ 同时衡量体验、质量、业务结果<br>❌ 指标体系更复杂，需要实验平台 | 我会把 observability 当成架构的一部分，而不是事后加 dashboard。关键链路需要 request_id、structured events、high-cardinality 控制、SLO alert，以及足够 raw data 支撑 debug/replay。 |
