...
| --- | --- | --- | --- | --- |
| Prefix Index：Trie vs Prefix Hashtable | Trie<br>✅ Prefix 共享，空间效率好<br>❌ 实现复杂 | Prefix Hashtable<br>✅ 查询非常快，`O(1)` lookup<br>❌ 存储膨胀，同一个 suggestion 会重复出现在多个 prefix | Hybrid<br>✅ 短 prefix 查询极快<br>❌ 两套结构，构建和调试复杂 | 面试里可以先选 Prefix Hashtable，因为查询路径简单、好解释。<br>生产级可以用 compressed Trie/FST 或 hybrid。 |
| 更新链路：Message Queue 实时处理 vs Raw Log Offline Processing | Raw log offline processing<br>✅ 可以做复杂清洗、去 spam、质量过滤<br>❌ 新鲜度差，热点事件反应慢 | Message queue + stream processing<br>✅ 新鲜度高，可以分钟级更新<br>❌ 容易被 spam 或突发噪音污染 | Offline static index + realtime overlay<br>✅ 静态结果稳定，实时 overlay 补热点<br>❌ Query path 要 merge/dedup/rerank | 用 offline log processing 构建 base index。<br>用 message queue/streaming 更新 realtime trending overlay。 |
| Real-time Trending：滑动窗口 vs Decay Score | 固定滑动窗口 count<br>✅ 语义清晰<br>❌ 窗口边界会有跳变 | Decayed score<br>✅ 排名更平滑<br>❌ 结果不是严格“过去 N 分钟” | 短窗口 + 长窗口混合<br>✅ 短窗口发现爆发，长窗口提供稳定性<br>❌ Scoring 和调参复杂 | Trending 用 decay 或短长窗口混合。<br>不建议只靠单个短窗口，否则容易被噪声带偏。 |
| Ranking：popularity vs personalization vs business rules | 只按 global popularity<br>✅ 简单稳定<br>❌ 不够个性化 | Contextual ranking<br>✅ 结果更相关<br>❌ Cache key 变多，命中率下降 | Personalized rerank<br>✅ 用户体验最好<br>❌ 延迟更高 | 召回用 prefix index。<br>排序用 popularity + region/language + lightweight personalization。 |
| 缓存策略：全量 prefix cache vs 热门短 prefix cache | 缓存所有 prefix<br>✅ 查询快<br>❌ Prefix 组合太多，内存浪费 | 只缓存热门短 prefix<br>✅ 命中高，内存可控<br>❌ 长尾 prefix 仍要查 index | Edge cache + service local memory index<br>✅ 边缘挡住大部分重复短 prefix<br>❌ Index rollout 和 cache invalidation 更复杂 | 热门短 prefix 放 Redis/edge cache。<br>Suggestion Service 本地加载 read-only index。 |
| Index 更新：增量更新 vs 全量 rebuild | 全量 rebuild<br>✅ 构建结果一致，容易 rollback<br>❌ 新鲜度不够 | 增量更新<br>✅ 新鲜度更好<br>❌ Index mutation 复杂 | 全量 base index + realtime overlay<br>✅ Base index 可控，overlay 负责新鲜热点<br>❌ Query merge 复杂 | Base index 全量 rebuild + versioned publish。<br>Realtime trending 作为 overlay。 |
| Quality Metrics：Latency vs Picking Rate | 只看 latency<br>✅ 容易监控<br>❌ 快但结果差，用户仍然不会选 | Picking rate / CTR<br>✅ 直接反映用户是否选择建议词<br>❌ 受 position bias、UI、用户意图影响 | Latency + picking rate + downstream success<br>✅ 同时衡量体验、质量、业务结果<br>❌ 指标体系更复杂，需要实验平台 | 性能看 p50/p95/p99 latency。<br>质量看 picking rate、CTR、zero-result rate、reformulation rate。 |
