...
| --- | --- | --- | --- | --- |
| Chunking：固定长度 vs 结构化/语义切分 | 固定 token 长度切分<br>✅ 实现简单，chunk 大小可控<br>❌ 可能切断表格、标题、步骤，导致 retrieval 上下文不完整 | 按 heading/section 结构切分<br>✅ chunk 语义完整，citation 更清楚<br>❌ PDF/OCR/脏 HTML 解析难 | Semantic chunking + overlap<br>✅ 更符合语义边界<br>❌ 计算成本和实现复杂度更高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Retrieval：Vector vs BM25 vs Hybrid | 纯 vector search<br>✅ 能找到同义表达和语义相关内容<br>❌ 对精确 ID、错误码、函数名、专有名词不稳定 | BM25 / keyword search<br>✅ 可解释，精确匹配强<br>❌ 语义泛化弱 | Hybrid retrieval<br>✅ 兼顾语义和精确匹配<br>❌ score fusion、去重和 rerank 更复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Metadata / ACL Filtering | 检索后在应用层过滤 ACL<br>✅ 实现简单<br>❌ 如果过滤后结果不足，需要二次检索 | Vector DB metadata pre-filter<br>✅ 只在 allowed corpus 内 ANN search，安全边界更清楚<br>❌ allowed_ids 太多时 filter 成本高 | 按 tenant/permission group 分 collection/shard<br>✅ 隔离强，filter 更简单<br>❌ collection 多，管理和资源利用复杂 | 我会把 privacy 和 authorization 当成 correctness requirement。前端过滤只能改善体验，真正防泄露要靠服务端强校验、audit log、删除语义和 least-privilege access。 |
| Reranking 和 Context Assembly | 直接把 vector topK 塞进 prompt<br>✅ 低延迟，实现简单<br>❌ 容易混入不相关 chunk，浪费 context window | Cross-encoder rerank<br>✅ precision 明显提升<br>❌ 延迟和成本增加 | LLM rerank / answer-aware selection<br>✅ 能更智能地选上下文<br>❌ 成本高，且可能引入不稳定 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| Index Freshness：Streaming vs Batch Ingestion | 纯 batch reindex<br>✅ 实现简单，索引一致性好<br>❌ 新内容延迟高 | Streaming ingestion<br>✅ 新文档/更新可分钟级进入索引<br>❌ partial failure、重复消息、旧版本清理更复杂 | Streaming + batch reconciliation<br>✅ 实时性和最终正确性兼顾<br>❌ 需要维护 reindex job 和差异修复 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Prompt Injection 和安全 | 完全信任 retrieved context<br>✅ 简单<br>❌ 文档里可能包含 “忽略之前指令” 等 prompt injection | 把 context 当 untrusted data<br>✅ 系统指令明确模型不能执行文档中的指令<br>❌ 不能完全杜绝，需要模型和后处理配合 | Content sanitization + policy layer<br>✅ 可过滤明显恶意内容、敏感输出<br>❌ 误杀和漏检都存在 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Evaluation Metrics | 只看用户点赞<br>✅ 简单直接<br>❌ 稀疏、偏置大，无法定位 retrieval 还是 generation 问题 | Retrieval metrics<br>✅ Recall@K、MRR、nDCG 能定位是否找到了正确上下文<br>❌ 需要标注数据或 golden set | End-to-end answer metrics<br>✅ Faithfulness、answer correctness、citation a...<br>❌ 评估成本高，LLM-as-judge 有偏差 | 我会把 observability 当成架构的一部分，而不是事后加 dashboard。关键链路需要 request_id、structured events、high-cardinality 控制、SLO alert，以及足够 raw data 支撑 debug/replay。 |
| Latency 和 Cost Optimization | 每次都完整检索 + 强模型回答<br>✅ 质量高<br>❌ 延迟和成本高 | 多级缓存<br>✅ 低延迟低成本<br>❌ 文档更新后 cache invalidation 要绑定 index version | 模型路由<br>✅ 简单问题用小模型，复杂问题用大模型<br>❌ 需要 query classifier 和 fallback | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
