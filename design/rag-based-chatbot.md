...
| --- | --- | --- | --- | --- |
| Chunking：固定长度 vs 结构化/语义切分 | 固定 token 长度切分<br>✅ 实现简单，chunk 大小可控<br>❌ 可能切断表格、标题、步骤，导致 retrieval 上下文不完整 | 按 heading/section 结构切分<br>✅ chunk 语义完整，citation 更清楚<br>❌ PDF/OCR/脏 HTML 解析难 | Semantic chunking + overlap<br>✅ 更符合语义边界<br>❌ 计算成本和实现复杂度更高 | 先按结构切分，fallback 到 token chunking。<br>对长 section 做二级切分和少量 overlap。 |
| Retrieval：Vector vs BM25 vs Hybrid | 纯 vector search<br>✅ 能找到同义表达和语义相关内容<br>❌ 对精确 ID、错误码、函数名、专有名词不稳定 | BM25 / keyword search<br>✅ 可解释，精确匹配强<br>❌ 语义泛化弱 | Hybrid retrieval<br>✅ 兼顾语义和精确匹配<br>❌ score fusion、去重和 rerank 更复杂 | 使用 vector + BM25 hybrid。<br>先取较大候选集，再 rerank。 |
| Metadata / ACL Filtering | 检索后在应用层过滤 ACL<br>✅ 实现简单<br>❌ 如果过滤后结果不足，需要二次检索 | Vector DB metadata pre-filter<br>✅ 只在 allowed corpus 内 ANN search，安全边界更清楚<br>❌ allowed_ids 太多时 filter 成本高 | 按 tenant/permission group 分 collection/shard<br>✅ 隔离强，filter 更简单<br>❌ collection 多，管理和资源利用复杂 | tenant 级强隔离，tenant 内用 permission group/document ACL metadata filter。<br>allowed_ids 太多时不要传巨大列表，改用 permission group id / ACL version。 |
| Reranking 和 Context Assembly | 直接把 vector topK 塞进 prompt<br>✅ 低延迟，实现简单<br>❌ 容易混入不相关 chunk，浪费 context window | Cross-encoder rerank<br>✅ precision 明显提升<br>❌ 延迟和成本增加 | LLM rerank / answer-aware selection<br>✅ 能更智能地选上下文<br>❌ 成本高，且可能引入不稳定 | 常规 query 用 lightweight reranker。<br>高复杂度 query 用强 reranker。 |
| Index Freshness：Streaming vs Batch Ingestion | 纯 batch reindex<br>✅ 实现简单，索引一致性好<br>❌ 新内容延迟高 | Streaming ingestion<br>✅ 新文档/更新可分钟级进入索引<br>❌ partial failure、重复消息、旧版本清理更复杂 | Streaming + batch reconciliation<br>✅ 实时性和最终正确性兼顾<br>❌ 需要维护 reindex job 和差异修复 | 新增/更新文档走 streaming pipeline。<br>定期 batch reconciliation 检查 raw docs、metadata、vector index 是否一致。 |
| Prompt Injection 和安全 | 完全信任 retrieved context<br>✅ 简单<br>❌ 文档里可能包含 “忽略之前指令” 等 prompt injection | 把 context 当 untrusted data<br>✅ 系统指令明确模型不能执行文档中的指令<br>❌ 不能完全杜绝，需要模型和后处理配合 | Content sanitization + policy layer<br>✅ 可过滤明显恶意内容、敏感输出<br>❌ 误杀和漏检都存在 | System prompt 明确 retrieved docs are untrusted.<br>Citation validation 和 output policy filter。 |
| Evaluation Metrics | 只看用户点赞<br>✅ 简单直接<br>❌ 稀疏、偏置大，无法定位 retrieval 还是 generation 问题 | Retrieval metrics<br>✅ Recall@K、MRR、nDCG 能定位是否找到了正确上下文<br>❌ 需要标注数据或 golden set | End-to-end answer metrics<br>✅ Faithfulness、answer correctness、citation a...<br>❌ 评估成本高，LLM-as-judge 有偏差 | 同时看 retrieval 和 generation。<br>线上看 answer rate、citation click、thumbs up/down、fallback rate。 |
| Latency 和 Cost Optimization | 每次都完整检索 + 强模型回答<br>✅ 质量高<br>❌ 延迟和成本高 | 多级缓存<br>✅ 低延迟低成本<br>❌ 文档更新后 cache invalidation 要绑定 index version | 模型路由<br>✅ 简单问题用小模型，复杂问题用大模型<br>❌ 需要 query classifier 和 fallback | Cache query rewrite/retrieval result by `query + tenant + ACL version + index version`。<br>Prompt/answer cache 只用于稳定知识和低风险问题。 |
