...
| --- | --- | --- | --- | --- |
| GeoHash vs Quadtree | GeoHash<br>✅ 实现简单<br>❌ cell size 固定，不能按人口密度动态调整 | Quadtree<br>✅ 可以按 density 动态拆分，热点区域 cell 更小，稀疏区域 cell 更大<br>❌ 实现复杂 | H3 / S2<br>✅ 成熟库支持多 resolution 和邻居查询<br>❌ 需要理解 cell resolution、边界覆盖和索引建模 | 面试基础版可以讲 GeoHash。<br>Staff+ 版本建议指出 H3/S2 更适合生产系统。 |
| 边界问题：只查当前 cell vs current + neighbours | 只查当前 GeoHash prefix<br>✅ 查询简单、低延迟<br>❌ 用户在 cell 边缘时会漏掉隔壁 cell 的近邻 | 查 current cell + neighboring cells<br>✅ 解决大部分边界漏召回问题<br>❌ 查询 cell 数增加，候选集变大 | 按圆形范围覆盖 cells<br>✅ 召回更稳定，适合 H3/S2 k-ring 或 covering API<br>❌ 实现比固定 8 邻居复杂 | GeoHash 至少查 current + 8 neighbours。<br>H3/S2 用 k-ring 或 region covering。 |
| 不够结果时怎么办：扩大半径 vs 降低 precision | 固定半径返回不足结果<br>✅ 语义清晰<br>❌ 稀疏区域体验差，可能返回很少甚至空 | 逐步扩大 search radius<br>✅ 用户体验好<br>❌ 查询成本可能逐步升高，远距离结果相关性下降 | GeoHash remove last digit / lower precision<br>✅ 实现简单，prefix 更短覆盖更大区域<br>❌ 候选集可能突然变很大，尤其城市中心 | 查询先按目标 radius 找 cells。<br>结果不足时逐步扩大 radius 或降低 resolution。 |
| Redis GEO 是否适合 | Redis GEO<br>✅ API 简单，`GEOADD` / `GEOSEARCH` 上手快<br>❌ 本质是 sorted set，频繁更新写会有压力 | Redis GEO per region shard<br>✅ 可以按 city/geohash prefix 分散写入<br>❌ 跨边界查询要 scatter-gather 多个 shard | H3/S2 + NoSQL/KV<br>✅ cell id 天然作为 shard key<br>❌ 需要自己实现距离过滤、候选聚合和 TTL | Redis GEO 适合原型或中小规模。<br>高频移动位置更新更推荐 H3/S2 + KV/NoSQL + TTL。 |
| 高频位置更新：同步写索引 vs 异步索引 | 每次位置更新同步更新所有索引<br>✅ 查询最新<br>❌ 司机/设备高频更新会导致写放大和尾延迟 | 异步 location update stream<br>✅ 写入解耦<br>❌ nearby query 可能短暂读到旧位置 | 客户端 adaptive update interval<br>✅ 静止或慢速时降低更新频率，高速/转向时提高频率<br>❌ 客户端逻辑更复杂，可能被恶意或异常客户端绕过 | Source DB 保存 latest location。<br>Geo index 异步更新，并用 timestamp/version 防旧位置覆盖新位置。 |
| 静态 POI vs 动态实体 | 统一索引<br>✅ 实现简单<br>❌ 静态店铺和动态司机的更新频率完全不同，混在一起影响性能 | 静态/动态分离<br>✅ 静态 POI 可离线构建和缓存<br>❌ 查询时需要合并两个来源 | 多层索引<br>✅ 不同实体使用不同存储和 freshness 策略<br>❌ Query Service 需要做结果融合和排序 | 静态 POI 用离线/低频索引，可缓存。<br>动态实体用实时 geo index + TTL。 |
| Sharding：按 entity_id vs 按 geo cell | 按 `entity_id` shard<br>✅ 更新某个实体容易<br>❌ nearby query 需要跨很多 shard 扫描，不适合范围查找 | 按 geo cell shard<br>✅ 查询附近只访问相关 cells<br>❌ 城市中心 cell 热点明显 | 双写 latest store + geo index<br>✅ entity lookup 和 nearby query 各自优化<br>❌ 两份数据要处理一致性和重建 | Source DB 按 `entity_id`。<br>Geo Index 按 cell id / region。 |
| H3/S2 高阶方案 | H3<br>✅ 多 resolution<br>❌ hexagon 边界仍需距离过滤 | S2<br>✅ Google 常用<br>❌ 实现和调参比 GeoHash 更复杂 | 时空数据库 / OLAP<br>✅ GeoMesa、ClickHouse+H3 适合大规模 geo/time query<br>❌ 不一定适合毫秒级在线 dispatch 主路径 | 在线 nearby 主路径使用 H3/S2 + KV/NoSQL。<br>历史分析、热区统计、运营报表使用 ClickHouse + H3 或 GeoMesa。 |
