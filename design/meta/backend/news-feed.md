...
| --- | --- | --- | --- | --- |
| SQL vs NoSQL | SQL<br>✅ 事务强，查询灵活，数据一致性好<br>❌ 大规模 timeline、follow graph、fanout 写入很难水平扩展 | NoSQL<br>✅ 按 `user_id / tweet_id` 分区，吞吐高，扩展简单<br>❌ 不能依赖复杂 JOIN，需要为查询模式设计表 | Hybrid<br>✅ User/account 配置可用 SQL，tweet/timeline 用 NoS...<br>❌ 多套存储带来一致性和运维复杂度 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Push vs Pull | Pull / fanout-on-read<br>✅ 写 tweet 很便宜<br>❌ 读 feed 时要查很多 followees 并 merge，延迟高 | Push / fanout-on-write<br>✅ 读 feed 很快，直接读 home timeline cache<br>❌ 明星用户发 tweet 写放大巨大 | Hybrid<br>✅ 普通用户 push，大 V pull，平衡读写成本<br>❌ 读路径要合并 pushed timeline 和 celebrity pull re... | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Active User Cache 策略 | 缓存所有用户 timeline<br>✅ 读路径简单<br>❌ 大量不活跃用户浪费内存 | 只缓存活跃用户<br>✅ 内存成本可控<br>❌ 不活跃用户回来时首次读取较慢 | 分层缓存<br>✅ hot users timeline 放内存，warm users 放 SSD/No...<br>❌ 系统复杂，cache consistency 更难 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| Partition / Replication | 按 `user_id` 分区 timeline<br>✅ 读某个用户 feed 很快<br>❌ 超级活跃用户或热门用户可能产生热点 | 按 `tweet_id` / `author_id` 分区 tweets<br>✅ 作者 timeline 查询自然<br>❌ 读 home feed 需要批量跨分区取 tweet body | 多副本 + read replica<br>✅ 提升读吞吐和可用性<br>❌ 复制延迟导致读到旧数据 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Cache Invalidation 和 Privacy | 写时清理所有 cache<br>✅ cache 更干净<br>❌ 删除 tweet、block、private change 可能需要清理海量 tim... | 读时过滤<br>✅ 避免大规模 cache invalidation<br>❌ 读路径多一次 policy check | 异步清理 + 读时兜底<br>✅ 最终清理缓存，同时保证读时不会泄露不可见内容<br>❌ 实现复杂，需要 policy version 或 visibility check | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| Follow/Unfollow 后 Timeline 如何处理 | follow 时立即 backfill<br>✅ 体验好<br>❌ follow 大量用户时写放大明显 | unfollow 时立即删除 timeline 里的历史 tweets<br>✅ 用户不会再看到 unfollow 对象内容<br>❌ 需要扫描 timeline cache，成本高 | lazy rebuild / read-time filter<br>✅ 写路径轻<br>❌ timeline 里可能有 stale candidate，需要读时过滤 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
