...
|---|---|---|---|
| Scheduler：扫大表 vs due task 小表 vs 直接入队 | A: 定期扫 Product Price DB 大表<br>B: 维护 due task 小表<br>C: 生成任务后直接放 queue | A ✅ 实现简单。 ❌ 商品量大时非常贵；按 GSI shard 扫描也容易产生热点。<br>B ✅ 只扫描即将执行的任务；可按 `time_bucket + shard_id` 分片。 ❌ Product Price DB 和 task table 有一致性问题，需要重建/修复任务。<br>C ✅ worker 直接消费，延迟低。 ❌ 如果不落表，queue 消息丢失或过期后任务难以恢复。 | Product Price DB 存 `next_scrape_at`。<br>另有 due task table：`PK=time_bucket#shard_id, SK=product_id`。<br>Scheduler 扫 due task 小表后放 queue；reconciliation job 从 Product Price DB 修复漏任务。 |
| Stale Price Scanner 的职责 | A: Scanner 直接抓网页<br>B: Scanner 只产生 scrape task<br>C: 优先级调度 | A ✅ 少一个组件。 ❌ 调度和抓取耦合，慢网站会拖慢扫描。<br>B ✅ 调度和执行分离；scraper worker 可独立扩展。 ❌ 需要任务状态和重试。<br>C ✅ 资源用在更有价值的商品上。 ❌ 调度策略复杂。 | Stale Price Scanner 只决定哪些 product 需要重新抓取。<br>Scraper 执行抓取。<br>next_scrape_at 可根据 watchers 数量、价格波动、失败率动态调整。 |
| 价格变化触发：DB Trigger vs CDC vs Application Event | A: DB trigger<br>B: Debezium / CDC<br>C: Application writes event | A ✅ 价格更新和触发逻辑靠近 DB。 ❌ 复杂逻辑放 DB 里难维护；NoSQL 通常不支持传统 trigger。<br>B ✅ 无需侵入业务代码；可过滤关心的 price update 事件并发到 Kafka。 ❌ CDC 有延迟；schema 变化和 connector 运维要处理。<br>C ✅ 逻辑清晰，事件 payload 可控。 ❌ DB 更新成功但事件发送失败会漏，需要 outbox。 | 如果 SQL DB，用 outbox 或 Debezium。<br>如果 DynamoDB，用 DynamoDB Streams。<br>不要依赖“消费者自己比较新旧值”作为唯一触发，最好价格更新事件带 old/new price 和 version。 |
| DynamoDB LSI / GSI 怎么理解 | A: Local Secondary Index<br>B: Global Secondary Index<br>C: 反向表 | A ✅ 和主表共享 partition key；适合 `PK=product_id`，在同 product 内按 threshold/timestamp 排序。 ❌ LSI 必须建表时创建；不能换 partition key；单 partition 仍可能热点。<br>B ✅ 例如主表按 `product_id`，GSI 按 `user_id` 查询用户 watch list。 ❌ GSI 异步更新，可能短暂 eventual consistency；额外写放大和成本。<br>C ✅ 可以分别优化 product->watchers 和 user->watches。 ❌ 双写一致性和修复更复杂。 | Price Watch 主表：`PK=product_id, SK=price_threshold#user_id`。<br>用户查询用 GSI 或单独 `user_watch` 表：`PK=user_id, SK=watch_id`。<br>热门 product partition 要考虑 threshold bucket / sub-shard。 |
| Price History Table 怎么存 | A: 只存当前价格<br>B: 每次 scrape 都存一条 history<br>C: 只存价格变化 + 定期 snapshot | A ✅ 简单，存储小。 ❌ 无法展示趋势，也无法 debug 某次通知为什么触发。<br>B ✅ 可复盘所有抓取结果。 ❌ 数据量大，重复价格多。<br>C ✅ 存储成本低，仍能画趋势和 debug。 ❌ 无法知道每次抓取是否都成功看到相同价格。 | `price_history(PK=product_id, SK=timestamp)`。<br>存 price change event + 每日/每小时 snapshot。<br>长期历史可 downsample 为 daily min/avg/max。 |
| 查询 watchers：product_id + threshold 的热点问题 | A: 按 product_id 查询所有 watchers，在应用层过滤 threshold<br>B: Sort key 用 price_threshold 做 range query<br>C: Threshold bucket + sub-shard | A ✅ 实现简单。 ❌ 热门商品 watcher 可能非常多，扫描浪费。<br>B ✅ 只读可能触发通知的 watchers。 ❌ 热门商品仍可能单 partition 热点。<br>C ✅ 分散热门 product 的读写压力。 ❌ 查询要 fanout 多个 bucket/shard，合并结果。 | 默认 `product_id + price_threshold` range query。<br>热门商品按 `product_id#bucket#shard` 拆分。<br>Notifier 分页查询，避免一次拉出百万用户。 |
| 通知幂等和重复提醒 | A: 每次价格低于 threshold 都发通知<br>B: 记录 last_notified_price/version<br>C: Notification dedup table | A ✅ 逻辑简单。 ❌ 同一价格多次 scrape 会重复骚扰用户。<br>B ✅ 同一 price/version 不重复通知。 ❌ 价格降到同一价格但隔很久，是否再提醒要产品定义。<br>C ✅ 强幂等，避免 replay 重复发送。 ❌ 多一张表和 TTL 清理。 | dedup key：`watch_id + product_price_version`。<br>`last_notified_price/at` 做 cooldown。<br>Notification log 设置 TTL，保留近期去重记录。 |
| TTL 和过期事件 | A: DynamoDB TTL + Streams<br>B: Cassandra TTL<br>C: 显式 expiration table | A ✅ TTL 自动过期；Streams 可以捕获过期删除事件做后续处理。 ❌ TTL 删除不是准实时，延迟可能到小时级，不能做精确 scheduler。<br>B ✅ 写入时设置 TTL 简单。 ❌ 通常不能可靠捕获“过期事件”；tombstone 多了会影响性能。<br>C ✅ 可控，可扫描 due tasks。 ❌ 需要自己管理状态和清理。 | TTL 用于清理 notification dedup、过期临时记录。<br>Scheduler 不依赖 TTL 事件触发。<br>需要准时抓取时用 due task table / queue。 |
