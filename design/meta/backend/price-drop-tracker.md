...
| --- | --- | --- | --- | --- |
| Scheduler：扫大表 vs due task 小表 vs 直接入队 | 定期扫 Product Price DB 大表<br>✅ 实现简单<br>❌ 商品量大时非常贵 | 维护 due task 小表<br>✅ 只扫描即将执行的任务<br>❌ Product Price DB 和 task table 有一致性问题，需要重建/... | 生成任务后直接放 queue<br>✅ worker 直接消费，延迟低<br>❌ 如果不落表，queue 消息丢失或过期后任务难以恢复 | I would not use TTL as the correctness mechanism. TTL is cleanup; correctness should come from an explicit expires_at check plus a recoverable scheduler or delayed queue that can tolerate duplicate execution and worker failure. |
| Stale Price Scanner 的职责 | Scanner 直接抓网页<br>✅ 少一个组件<br>❌ 调度和抓取耦合，慢网站会拖慢扫描 | Scanner 只产生 scrape task<br>✅ 调度和执行分离<br>❌ 需要任务状态和重试 | 优先级调度<br>✅ 资源用在更有价值的商品上<br>❌ 调度策略复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 价格变化触发：DB Trigger vs CDC vs Application Event | DB trigger<br>✅ 价格更新和触发逻辑靠近 DB<br>❌ 复杂逻辑放 DB 里难维护 | Debezium / CDC<br>✅ 无需侵入业务代码<br>❌ CDC 有延迟 | Application writes event<br>✅ 逻辑清晰，事件 payload 可控<br>❌ DB 更新成功但事件发送失败会漏，需要 outbox | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| DynamoDB LSI / GSI 怎么理解 | Local Secondary Index<br>✅ 和主表共享 partition key<br>❌ LSI 必须建表时创建 | Global Secondary Index<br>✅ 例如主表按 `product_id`，GSI 按 `user_id` 查询用户 wa...<br>❌ GSI 异步更新，可能短暂 eventual consistency | 反向表<br>✅ 可以分别优化 product->watchers 和 user->watches<br>❌ 双写一致性和修复更复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Price History Table 怎么存 | 只存当前价格<br>✅ 简单，存储小<br>❌ 无法展示趋势，也无法 debug 某次通知为什么触发 | 每次 scrape 都存一条 history<br>✅ 可复盘所有抓取结果<br>❌ 数据量大，重复价格多 | 只存价格变化 + 定期 snapshot<br>✅ 存储成本低，仍能画趋势和 debug<br>❌ 无法知道每次抓取是否都成功看到相同价格 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 查询 watchers：product_id + threshold 的热点问题 | 按 product_id 查询所有 watchers，在应用层过滤 threshold<br>✅ 实现简单<br>❌ 热门商品 watcher 可能非常多，扫描浪费 | Sort key 用 price_threshold 做 range query<br>✅ 只读可能触发通知的 watchers<br>❌ 热门商品仍可能单 partition 热点 | Threshold bucket + sub-shard<br>✅ 分散热门 product 的读写压力<br>❌ 查询要 fanout 多个 bucket/shard，合并结果 | I would first define the business invariant, such as no oversell, one winner, or one successful payment capture. The critical transition should be protected with a conditional write or state machine, and all external side effects should be idempotent and reconciled asynchronously. |
| 通知幂等和重复提醒 | 每次价格低于 threshold 都发通知<br>✅ 逻辑简单<br>❌ 同一价格多次 scrape 会重复骚扰用户 | 记录 last_notified_price/version<br>✅ 同一 price/version 不重复通知<br>❌ 价格降到同一价格但隔很久，是否再提醒要产品定义 | Notification dedup table<br>✅ 强幂等，避免 replay 重复发送<br>❌ 多一张表和 TTL 清理 | I would avoid promising end-to-end exactly-once. The stronger Staff-level framing is at-least-once delivery with deterministic idempotency keys, idempotent sinks, and replayable raw events; that gives us failure recovery without pretending the broker solves external side effects. |
| TTL 和过期事件 | DynamoDB TTL + Streams<br>✅ TTL 自动过期<br>❌ TTL 删除不是准实时，延迟可能到小时级，不能做精确 scheduler | Cassandra TTL<br>✅ 写入时设置 TTL 简单<br>❌ 通常不能可靠捕获“过期事件” | 显式 expiration table<br>✅ 可控，可扫描 due tasks<br>❌ 需要自己管理状态和清理 | I would not use TTL as the correctness mechanism. TTL is cleanup; correctness should come from an explicit expires_at check plus a recoverable scheduler or delayed queue that can tolerate duplicate execution and worker failure. |
