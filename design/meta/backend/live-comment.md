...
|---|---|---|---|
| 实时协议：Polling vs SSE vs WebSocket | A: Polling<br>B: SSE<br>C: WebSocket | A ✅ 实现简单，纯 HTTP，负载均衡和重试都容易。 ❌ 延迟取决于轮询间隔；大量空请求浪费 QPS；热门视频成本高。<br>B ✅ 基于 HTTP，浏览器原生支持自动重连；比 WebSocket 运维简单。 ❌ 单向通道；长连接仍然需要专门的 gateway、heartbeat 和 backpressure。<br>C ✅ 双向、低延迟、协议灵活。 ❌ 连接管理复杂；水平扩展、连接迁移、服务发现和重试都更难。 推荐 如果只是 live comment，优先 SSE。 如果还有实时互动、presence、游戏化玩法，可以选 WebSocket。 不管选哪种，可靠性都不要靠连接本身，而靠 Comment Store + cursor 补齐。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| Pub/Sub 分发：pull-based partition vs push-based routing | A: partitioned pub/sub pull<br>B: 直接 push 到 SSE / WebSocket server<br>C: Hybrid | A ✅ 天然有 backpressure；单视频顺序容易保证；worker 扩缩容清晰。 ❌ 订阅关系变化频繁时，很多 worker 可能拉到没有本地观众的消息。<br>B ✅ 只发给有观众的节点，延迟低，内网无效流量少。 ❌ 路由表更新频繁；节点失败、扩缩容、连接迁移会让元数据管理复杂。<br>C ✅ 热门视频走 push routing，冷门视频走共享消费池，成本和延迟平衡。 ❌ 系统路径更多，调试和容量规划更复杂。 推荐 面试里可以说：fanout worker 从 pub/sub 消费，然后按 `video_id -> node_id` push 到相关 stream nodes。 对冷门视频可以让多个 video 共享普通 stream pool；对热门视频单独建立 server group。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| 让看相同视频的人到哪里：单节点 vs server group vs 完全随机 | A: 同一视频 sticky 到单个 server<br>B: 同一视频 sticky 到 server group<br>C: 完全随机分配连接 | A ✅ 本地 broadcast 最简单；不需要跨节点 fanout。 ❌ 热门视频会把单节点打爆。<br>B ✅ fanout 只打到这个 video 的 server group；可动态扩容。 ❌ 需要管理 group membership、连接迁移、负载不均。<br>C ✅ LB 简单，连接分布均匀。 ❌ 一个热门视频的评论可能要 fanout 到几乎所有 stream servers。 推荐 使用 `video_id -> server group`。 冷门视频共享 group，热门视频独立 group 并可动态扩容。 这个点能体现 Staff+ 判断：热点不是平均 QPS，而是单个 video 的 fanout 半径。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| Routing metadata 如何维护 | A: 只维护 `user_id -> node_id`<br>B: 维护 `video_id -> list(node_id)`<br>C: 三张 routing 表组合 | A ✅ 简单，按用户推送直接。 ❌ live comment 是按 video fanout，仅有 user routing 会导致要遍历用户。<br>B ✅ fanout 范围明确，不需要遍历所有用户。 ❌ 用户进出视频频繁，列表更新和过期清理压力大。<br>C ✅ 支持不同查询方向；节点失败后可以更快清理相关订阅。 ❌ 元数据一致性复杂，需要 TTL、heartbeat 和幂等更新。 推荐 Redis / in-memory store 存三类 routing，但每条 entry 都有 TTL。 Stream Gateway 定期 heartbeat，上报 subscription count。 routing table 是 best-effort derived state，真实恢复靠 reconnect + history cursor。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| 历史评论分页：DB cursor vs Elasticsearch search_after vs cache | A: DB cursor<br>B: Elasticsearch `search_after`<br>C: recent comments cache | A ✅ 稳定、正确、容易和 source of truth 对齐。 ❌ 复杂搜索、过滤、审核条件不灵活。<br>B ✅ 复杂查询能力强，深分页比 offset 更稳定。 ❌ 索引最终一致，不适合作为评论正确性的唯一来源。<br>C ✅ 延迟低，能明显减轻 DB 压力。 ❌ cache 可能过期或缺失，需要 DB fallback。 推荐 用户正常看历史走 DB cursor。 热门最近评论加 Redis cache。 搜索和 moderation 用 Elasticsearch，但明确它是 derived index。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| 多 DC 同步：同步写 vs CDC 异步 vs video home region | A: 同步跨 DC 写<br>B: CDC / Kafka 异步复制<br>C: video home region | A ✅ 一致性强，读任何 DC 都更接近最新。 ❌ 写延迟高；跨区故障会影响核心发评论链路。<br>B ✅ 本地写低延迟；跨 DC 解耦；可以只同步热门视频。 ❌ 会有延迟、重复、乱序，需要幂等和排序策略。<br>C ✅ per-video ordering 更清晰。 ❌ 远端用户发评论可能跨区写，延迟上升。 推荐 默认本地写 + CDC 异步同步。 对全球热门直播可指定 home region，或者多 region 本地接入后统一事件排序。 用 `comment_id` 去重，用 `comment_seq` 或 server timestamp 控制展示顺序。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| 热点与不均衡：固定容量 vs 动态扩容 vs 降级 | A: 固定 server group<br>B: 动态扩容 server group<br>C: 服务降级 | A ✅ 实现简单，容量规划直观。 ❌ 热点突发时容易过载；冷门时浪费。<br>B ✅ 可以把资源集中给热门视频。 ❌ 扩容时需要更新 routing、迁移新连接、处理旧连接。<br>C ✅ 保护核心写入和观看体验。 ❌ 用户看到的评论可能不是全量实时评论。 推荐 热门视频优先动态扩容 server group。 极端热点时做 batching、sampling、rate limiting。 关键是保证评论写入不丢，实时展示可以有策略地降级。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
| 推送可靠性：实时推送 vs cursor 补齐 vs per-user inbox | A: 只靠实时推送<br>B: 实时推送 + cursor 补齐<br>C: per-user inbox | A ✅ 实现简单，延迟最低。 ❌ 连接断开或 server failure 会丢事件。<br>B ✅ 在线低延迟，断线后可恢复；写放大较低。 ❌ 客户端需要 merge、dedup 和维护 cursor。<br>C ✅ delivery tracking 精确。 ❌ 公开 live comment 会产生巨大写放大，不适合大规模观看场景。 推荐 live comment 用实时推送 + cursor 补齐。 不为每个 viewer 写 inbox。 这能展示一个很重要的判断：不是所有实时系统都需要 per-user delivery state。 | 先给简单可运营方案，再说明规模、可靠性或一致性要求变化时如何演进。 |
