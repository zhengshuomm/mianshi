...
| 深挖点 | 方案 A | 方案 B | 方案 C | 方案 D | 方案 E | 推荐表达 |
| --- | --- | --- | --- | --- | --- | --- |
| 实时更新：Polling vs SSE vs WebSocket | Polling<br>✅ 实现简单，HTTP 兼容好<br>❌ 延迟取决于轮询间隔 | SSE<br>✅ 比 WebSocket 简单<br>❌ 单向 | WebSocket<br>✅ 双向低延迟，适合高互动 auction 页面<br>❌ 连接管理、扩容和故障恢复更复杂 | - | - | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Redis Pub/Sub vs Dispatcher | Redis Pub/Sub 直接广播<br>✅ 轻量、低延迟、实现快<br>❌ 不持久 | Kafka + Dispatcher<br>✅ 事件持久化、可 replay、消费者可扩展<br>❌ 链路更长，延迟略高，运维复杂 | Hybrid<br>✅ Kafka 做 durable log，Redis/pubsub 或内存 fanou...<br>❌ 两套通道可能短暂不一致 | - | - | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| 出价正确性：DB CAS vs Redis ZSET vs 单 Auction Writer | DB conditional update / CAS<br>✅ source of truth 内保证最高价更新正确<br>❌ 热门 auction 会对同一 row/item 产生写竞争 | Redis sorted set<br>✅ `ZREVRANGE` 很快，适合读和候选计算<br>❌ Redis 不是最终账本 | 单 auction writer / actor<br>✅ 同一 auction 的出价串行处理，避免 DB 冲突<br>❌ 单 auction 吞吐受单 worker 限制 | - | - | I would define the auction invariant as one authoritative highest valid bid and one final winner. Redis can accelerate reads or candidate lookup, but winner calculation and rollback must be reconciled against the bid ledger or a per-auction single-writer state machine. |
| 最高价撤回怎么处理 | 每次撤回后扫描 DB 找第二高价<br>✅ 结果可信，直接来自 source of truth<br>❌ 热门 auction bid 很多时查询成本高 | Redis sorted set 找第二高价<br>✅ 撤回后 `ZREM`，再 `ZREVRANGE` 取 top<br>❌ Redis 可能和 DB 不一致，取到的 bid 需要回 DB 校验 active | 异步 Recalc Worker<br>✅ 不阻塞撤回请求<br>❌ 用户短时间看到状态滞后 | - | - | I would define the auction invariant as one authoritative highest valid bid and one final winner. Redis can accelerate reads or candidate lookup, but winner calculation and rollback must be reconciled against the bid ledger or a per-auction single-writer state machine. |
| DB 和 Cache Consistency | Write-through cache<br>✅ 写 DB 时同步更新 cache，读路径简单<br>❌ 不能真正保证强一致 | Write-behind cache<br>✅ 写延迟低，吞吐高<br>❌ cache 崩溃可能丢写 | Cache-aside<br>✅ 简单，cache miss 回源 DB<br>❌ 高并发下 cache invalidation 和 stale read 需要处理 | 先写 Kafka，再由 worker 更新 DB 和 cache<br>✅ Kafka 是统一顺序日志，DB/cache 都从日志派生<br>❌ 用户出价后看到结果有延迟 | 本地写 log，然后 update cache 和 MQ<br>✅ 本地日志可用于恢复 cache/MQ 更新失败<br>❌ 实现复杂，接近自建 WAL/outbox | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 触发 Payment：同步触发 vs Close Worker | auction end 时同步触发 payment<br>✅ 流程简单<br>❌ payment 慢或失败会阻塞 auction close | Close Worker 异步触发 payment<br>✅ auction close 和 payment 解耦<br>❌ 需要状态机表达 payment_pending | 用户主动 checkout<br>✅ 不会自动扣款，用户体验清晰<br>❌ winner 不付款风险高，需要超时和 second chance | - | - | I would first define the business invariant, such as no oversell, one winner, or one successful payment capture. The critical transition should be protected with a conditional write or state machine, and all external side effects should be idempotent and reconciled asynchronously. |
| Auction Close Scheduler 和 TTL | 依赖 DB TTL 触发 close<br>✅ 配置简单<br>❌ TTL 删除不是准时调度，可能延迟 | 定期扫大表<br>✅ 实现简单<br>❌ 规模大时扫描成本高 | time bucket 小表 + close queue<br>✅ 只扫即将结束的 auctions<br>❌ 要维护 task table 和修复漏任务 | - | - | I would define the auction invariant as one authoritative highest valid bid and one final winner. Redis can accelerate reads or candidate lookup, but winner calculation and rollback must be reconciled against the bid ledger or a per-auction single-writer state machine. |
| Fault Tolerance | Pub/Sub 作为唯一事件源<br>✅ 低延迟<br>❌ pub/sub 挂了或订阅断开会丢事件 | Durable event log<br>✅ Kafka 可 replay，consumer failover 后从 offset...<br>❌ 运维成本更高 | DB outbox<br>✅ DB 写和 outbox 写在同一事务，避免漏事件<br>❌ 需要 outbox relay 和幂等 consumer | - | - | I would assume workers can crash after doing partial work. The system should use leases or visibility timeouts, bounded retries, DLQs, and idempotent handlers so recovery is a normal execution path rather than an exceptional manual repair. |
