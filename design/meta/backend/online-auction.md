...
|---|---|---|---|
| 实时更新：Polling vs SSE vs WebSocket | A: Polling<br>B: SSE<br>C: WebSocket | A ✅ 实现简单，HTTP 兼容好。 ❌ 延迟取决于轮询间隔；热门 auction 会产生大量无效读。<br>B ✅ 比 WebSocket 简单；适合价格变化、auction closed 等状态流。 ❌ 单向；长连接占资源；浏览器连接数限制要考虑。<br>C ✅ 双向低延迟，适合高互动 auction 页面。 ❌ 连接管理、扩容和故障恢复更复杂。 | 实时价格更新优先 SSE。<br>如果还要聊天室、实时出价确认、复杂互动，可用 WebSocket。<br>无论哪种，最终状态都能通过 GET 查询补齐。 |
| Redis Pub/Sub vs Dispatcher | A: Redis Pub/Sub 直接广播<br>B: Kafka + Dispatcher<br>C: Hybrid | A ✅ 轻量、低延迟、实现快。 ❌ 不持久；订阅者断开就丢消息；Redis pub/sub 本身容易成为单点或热点。<br>B ✅ 事件持久化、可 replay、消费者可扩展；Dispatcher 故障后可从 offset 恢复。 ❌ 链路更长，延迟略高，运维复杂。<br>C ✅ Kafka 做 durable log，Redis/pubsub 或内存 fanout 做低延迟 delivery。 ❌ 两套通道可能短暂不一致。 | Bid events 先写 Kafka。<br>Dispatcher 消费 Kafka 后推送 SSE/WebSocket。<br>Redis Pub/Sub 可作为 Gateway 内部 fanout 优化，但不是可靠事件源。 |
| 出价正确性：DB CAS vs Redis ZSET vs 单 Auction Writer | A: DB conditional update / CAS<br>B: Redis sorted set<br>C: 单 auction writer / actor | A ✅ source of truth 内保证最高价更新正确。 ❌ 热门 auction 会对同一 row/item 产生写竞争。<br>B ✅ `ZREVRANGE` 很快，适合读和候选计算。 ❌ Redis 不是最终账本；和 DB 同步复杂。<br>C ✅ 同一 auction 的出价串行处理，避免 DB 冲突。 ❌ 单 auction 吞吐受单 worker 限制；worker failover 要处理。 | 普通 auction 用 DB CAS。<br>Redis ZSET 作为 read/cache 和撤回辅助。<br>超热门 auction 用 `auction_id` 分区队列 + single writer。 |
| 最高价撤回怎么处理 | A: 每次撤回后扫描 DB 找第二高价<br>B: Redis sorted set 找第二高价<br>C: 异步 Recalc Worker | A ✅ 结果可信，直接来自 source of truth。 ❌ 热门 auction bid 很多时查询成本高。<br>B ✅ 撤回后 `ZREM`，再 `ZREVRANGE` 取 top。 ❌ Redis 可能和 DB 不一致，取到的 bid 需要回 DB 校验 active。<br>C ✅ 不阻塞撤回请求；后台可分页扫描/校验。 ❌ 用户短时间看到状态滞后。 | 撤回 current highest 后进入 `highest_recalculating` 或直接异步更新。<br>Redis ZSET 快速给候选第二高。<br>Recalc Worker 以 DB active bids 为准更新 final highest。 |
| DB 和 Cache Consistency | A: Write-through cache<br>B: Write-behind cache<br>C: Cache-aside<br>D: 先写 Kafka，再由 worker 更新 DB 和 cache<br>E: 本地写 log，然后 update cache 和 MQ | A ✅ 写 DB 时同步更新 cache，读路径简单。 ❌ 不能真正保证强一致；DB 写成功 cache 写失败、并发乱序都会导致 cache stale；写压力大。<br>B ✅ 写延迟低，吞吐高。 ❌ cache 崩溃可能丢写；auction 这种正确性核心不适合只靠 write-behind。<br>C ✅ 简单，cache miss 回源 DB。 ❌ 高并发下 cache invalidation 和 stale read 需要处理。<br>D ✅ Kafka 是统一顺序日志，DB/cache 都从日志派生。 ❌ 用户出价后看到结果有延迟；同步拒绝低价 bid 更难。<br>E ✅ 本地日志可用于恢复 cache/MQ 更新失败。 ❌ 实现复杂，接近自建 WAL/outbox。 | Correctness：先写 DB source of truth，成功后写 Kafka/outbox。<br>Cache：由事件更新，带 version，旧事件不能覆盖新值。<br>Read：cache miss/stale 时回源 DB。<br>热门 auction 若 cache 准确性极重要，可用 single writer 同时顺序更新 DB、cache、event log。 |
| 触发 Payment：同步触发 vs Close Worker | A: auction end 时同步触发 payment<br>B: Close Worker 异步触发 payment<br>C: 用户主动 checkout | A ✅ 流程简单。 ❌ payment 慢或失败会阻塞 auction close。<br>B ✅ auction close 和 payment 解耦；可重试、可恢复。 ❌ 需要状态机表达 payment_pending。<br>C ✅ 不会自动扣款，用户体验清晰。 ❌ winner 不付款风险高，需要超时和 second chance。 | Auction close 后写 winner 和 payment task。<br>Payment Service 创建 payment intent 或 checkout session。<br>失败/超时后按业务处理第二高价或取消交易。 |
| Auction Close Scheduler 和 TTL | A: 依赖 DB TTL 触发 close<br>B: 定期扫大表<br>C: time bucket 小表 + close queue | A ✅ 配置简单。 ❌ TTL 删除不是准时调度，可能延迟；不应作为唯一 close 机制。<br>B ✅ 实现简单。 ❌ 规模大时扫描成本高。<br>C ✅ 只扫即将结束的 auctions；可按 shard 并发。 ❌ 要维护 task table 和修复漏任务。 | `close_tasks(PK=end_time_bucket#shard_id, SK=auction_id)`。<br>Close Scheduler 每分钟扫描 due buckets 放入 queue。<br>DynamoDB TTL Streams 可用于清理/补偿，但不是精确 scheduler。 |
| Fault Tolerance | A: Pub/Sub 作为唯一事件源<br>B: Durable event log<br>C: DB outbox | A ✅ 低延迟。 ❌ pub/sub 挂了或订阅断开会丢事件。<br>B ✅ Kafka 可 replay，consumer failover 后从 offset 继续。 ❌ 运维成本更高。<br>C ✅ DB 写和 outbox 写在同一事务，避免漏事件。 ❌ 需要 outbox relay 和幂等 consumer。 | Bid DB 更新和 outbox 同事务。<br>Outbox relay 写 Kafka。<br>Dispatcher、read model、cache 都可以从 Kafka/outbox replay 恢复。 |
