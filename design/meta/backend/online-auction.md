...
| --- | --- | --- | --- | --- | --- | --- |
| 实时更新：Polling vs SSE vs WebSocket | Polling<br>✅ 实现简单，HTTP 兼容好<br>❌ 延迟取决于轮询间隔 | SSE<br>✅ 比 WebSocket 简单<br>❌ 单向 | WebSocket<br>✅ 双向低延迟，适合高互动 auction 页面<br>❌ 连接管理、扩容和故障恢复更复杂 | - | - | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Redis Pub/Sub vs Dispatcher | Redis Pub/Sub 直接广播<br>✅ 轻量、低延迟、实现快<br>❌ 不持久 | Kafka + Dispatcher<br>✅ 事件持久化、可 replay、消费者可扩展<br>❌ 链路更长，延迟略高，运维复杂 | Hybrid<br>✅ Kafka 做 durable log，Redis/pubsub 或内存 fanou...<br>❌ 两套通道可能短暂不一致 | - | - | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 出价正确性：DB CAS vs Redis ZSET vs 单 Auction Writer | DB conditional update / CAS<br>✅ source of truth 内保证最高价更新正确<br>❌ 热门 auction 会对同一 row/item 产生写竞争 | Redis sorted set<br>✅ `ZREVRANGE` 很快，适合读和候选计算<br>❌ Redis 不是最终账本 | 单 auction writer / actor<br>✅ 同一 auction 的出价串行处理，避免 DB 冲突<br>❌ 单 auction 吞吐受单 worker 限制 | - | - | 我会先定义 auction invariant：只有一个 authoritative highest valid bid，最终只有一个 winner。Redis 可以加速读和候选查找，但 winner 计算、撤回和回滚必须回到 bid ledger 或 per-auction single writer 状态机。 |
| 最高价撤回怎么处理 | 每次撤回后扫描 DB 找第二高价<br>✅ 结果可信，直接来自 source of truth<br>❌ 热门 auction bid 很多时查询成本高 | Redis sorted set 找第二高价<br>✅ 撤回后 `ZREM`，再 `ZREVRANGE` 取 top<br>❌ Redis 可能和 DB 不一致，取到的 bid 需要回 DB 校验 active | 异步 Recalc Worker<br>✅ 不阻塞撤回请求<br>❌ 用户短时间看到状态滞后 | - | - | 我会先定义 auction invariant：只有一个 authoritative highest valid bid，最终只有一个 winner。Redis 可以加速读和候选查找，但 winner 计算、撤回和回滚必须回到 bid ledger 或 per-auction single writer 状态机。 |
| DB 和 Cache Consistency | Write-through cache<br>✅ 写 DB 时同步更新 cache，读路径简单<br>❌ 不能真正保证强一致 | Write-behind cache<br>✅ 写延迟低，吞吐高<br>❌ cache 崩溃可能丢写 | Cache-aside<br>✅ 简单，cache miss 回源 DB<br>❌ 高并发下 cache invalidation 和 stale read 需要处理 | 先写 Kafka，再由 worker 更新 DB 和 cache<br>✅ Kafka 是统一顺序日志，DB/cache 都从日志派生<br>❌ 用户出价后看到结果有延迟 | 本地写 log，然后 update cache 和 MQ<br>✅ 本地日志可用于恢复 cache/MQ 更新失败<br>❌ 实现复杂，接近自建 WAL/outbox | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 触发 Payment：同步触发 vs Close Worker | auction end 时同步触发 payment<br>✅ 流程简单<br>❌ payment 慢或失败会阻塞 auction close | Close Worker 异步触发 payment<br>✅ auction close 和 payment 解耦<br>❌ 需要状态机表达 payment_pending | 用户主动 checkout<br>✅ 不会自动扣款，用户体验清晰<br>❌ winner 不付款风险高，需要超时和 second chance | - | - | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| Auction Close Scheduler 和 TTL | 依赖 DB TTL 触发 close<br>✅ 配置简单<br>❌ TTL 删除不是准时调度，可能延迟 | 定期扫大表<br>✅ 实现简单<br>❌ 规模大时扫描成本高 | time bucket 小表 + close queue<br>✅ 只扫即将结束的 auctions<br>❌ 要维护 task table 和修复漏任务 | - | - | 我会先定义 auction invariant：只有一个 authoritative highest valid bid，最终只有一个 winner。Redis 可以加速读和候选查找，但 winner 计算、撤回和回滚必须回到 bid ledger 或 per-auction single writer 状态机。 |
| Fault Tolerance | Pub/Sub 作为唯一事件源<br>✅ 低延迟<br>❌ pub/sub 挂了或订阅断开会丢事件 | Durable event log<br>✅ Kafka 可 replay，consumer failover 后从 offset...<br>❌ 运维成本更高 | DB outbox<br>✅ DB 写和 outbox 写在同一事务，避免漏事件<br>❌ 需要 outbox relay 和幂等 consumer | - | - | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
