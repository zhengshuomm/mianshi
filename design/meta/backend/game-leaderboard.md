...
| --- | --- | --- | --- | --- |
| Global Top10：Redis ZSET vs DB GSI | Redis ZSET 全量存所有玩家<br>✅ top10、rank 查询非常快<br>❌ 玩家量巨大时内存贵 | DynamoDB GSI 按 score 排序<br>✅ 持久化，少一套 serving cache<br>❌ 高频 score update 写放大 | 只维护 top candidates<br>✅ 内存省，只存前 N 万或达到阈值的玩家<br>❌ 普通玩家 rank 只能近似 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| Friends Top10：预计算 vs 查询时计算 | 为每个用户预计算 friends leaderboard<br>✅ 读非常快<br>❌ 写放大巨大 | 查询时拉好友分数并排序<br>✅ 写路径轻，简单可靠<br>❌ 好友很多时读会慢 | 活跃用户预计算 + 普通用户实时计算<br>✅ 成本和延迟平衡<br>❌ 系统复杂 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| 按 Score Shard vs 按 User Shard | 按 score range shard<br>✅ top score 查询直观<br>❌ 玩家分数频繁变化，需要跨 shard 移动 | 按 user_id shard<br>✅ 写入均匀，用户查分简单<br>❌ global top10 要跨 shard 聚合 | user shard + 每 shard topN 上报<br>✅ 写均匀，同时支持全局 topN<br>❌ 需要定期/实时 merge shard topN | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 是否存 Rank | 每次写分数都更新所有 rank<br>✅ 读 rank 快<br>❌ 不可行 | 读时实时算 rank<br>✅ 写路径简单<br>❌ 全量 rank 查询成本高，除非 Redis ZSET 存全量 | 离线 batch 存 rank/percentile<br>✅ 适合展示 percentile、赛季结算<br>❌ 不是实时 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| Score Update 语义 | 最新分数覆盖<br>✅ 简单<br>❌ 不适合大多数游戏排行榜 | 历史最高分<br>✅ 符合 leaderboard 直觉<br>❌ 需要 conditional update：新分数必须大于旧分数 | 赛季/时间窗口分数<br>✅ 支持每日榜、赛季榜<br>❌ 需要按 season/window 维护多套榜 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| Redis Scale | 单 Redis ZSET 存全局榜<br>✅ 简单<br>❌ 内存和写 QPS 有上限 | Redis Cluster 分片<br>✅ 容量更大<br>❌ 全局 top10 需要跨 shard merge，不能直接一个 ZSET 解决 | Local topN + global merge<br>✅ 适合超大规模<br>❌ global top10 有轻微延迟 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 反作弊和可信分数 | 客户端直接提交分数<br>✅ 简单<br>❌ 极易作弊 | 服务端校验<br>✅ 更可信<br>❌ 游戏类型不同，校验复杂 | 异步风控<br>✅ 不阻塞写路径<br>❌ 可疑高分可能短暂上榜 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Fault Tolerance | 只写 Redis<br>✅ 快<br>❌ Redis 丢失后排行榜不可恢复 | 先写 DB，再异步更新 Redis<br>✅ DB 是 source of truth，可恢复<br>❌ Redis leaderboard 有短暂延迟 | 写 event log，再 worker 更新 DB/Redis<br>✅ 可 replay<br>❌ 用户提交后看到结果有延迟 | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
