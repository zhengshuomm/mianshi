...
| --- | --- | --- | --- | --- |
| 查询能力：固定窗口 vs 任意 lookup window | 固定窗口预聚合<br>✅ 查询极快，可直接读 Redis<br>❌ 不支持任意时间范围 | Druid/OLAP 任意窗口查询<br>✅ 灵活，支持任意 window 和历史查询<br>❌ 查询延迟比 Redis 高 | 固定窗口 + Druid hybrid<br>✅ 固定热点查询快<br>❌ 两套 serving path，一致性和新鲜度要解释清楚 | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| Sliding window：two-pointer vs bucket + heap refresh | two-pointer / 两个 consumer<br>✅ 精确维护窗口内 count<br>❌ 只适合固定窗口 | time bucket + heap refresh<br>✅ 逻辑清晰：每分钟一个 bucket，查询时合并最近 N 个 bucket<br>❌ 合并多个 bucket 有开销 | Flink event-time window<br>✅ 内置 window aggregation、watermark、state clea...<br>❌ 需要理解 Flink state 和 checkpoint | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| 旧窗口淘汰：lazy update vs 热点 key + lazy vs sketch | Lazy update<br>✅ 写路径简单<br>❌ 查询时可能多次 pop stale entries | 只维护热点 key + lazy update<br>✅ 大幅降低 state 和 heap 大小<br>❌ 可能漏掉突然变热的新 key | Count-Min Sketch + 定期合并 bucket sketch<br>✅ 内存更低<br>❌ 只能估计频率，存在 over-estimation | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| K 的大小和是否可变 | K 小且固定<br>✅ Heap 内存小<br>❌ 无法回答更大的 K | K 较大但窗口固定<br>✅ 可以通过离线 MapReduce/Spark 批处理<br>❌ 延迟高，不适合实时 | K 任意<br>✅ 查询灵活<br>❌ 在线系统很难低延迟支持任意 K | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| Redis 存储：sorted set 所有 key vs 只存 Top K vs key count | 每个窗口一个 sorted set，存所有 key<br>✅ 查询 Top K 简单<br>❌ 内存大 | Redis 只存 Top K list/sorted set<br>✅ 内存小，查询快<br>❌ 不能回答任意 K | Redis 存每分钟 key count，cron 聚合 Top K<br>✅ 实现简单<br>❌ Cron 聚合会有延迟 | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| 精确统计 vs 近似统计 | Exact count + heap<br>✅ 结果精确，可解释<br>❌ 高基数和长窗口下 state 很大 | Count-Min Sketch<br>✅ 内存低<br>❌ 有误差，通常 over-estimate | Decayed count<br>✅ 不需要精确删除旧事件<br>❌ 不回答“过去 1 小时精确 Top K” | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| Flink 容错：replica vs checkpoint | 每个 worker 做 active replica<br>✅ 理论上恢复快<br>❌ 资源翻倍 | Flink checkpoint + replay<br>✅ Checkpoint state + Kafka offset，恢复语义清晰<br>❌ 恢复需要时间 | 输出端 snapshot + offline rebuild<br>✅ Redis 丢了可从 Flink/Druid/warehouse 恢复<br>❌ Rebuild 期间结果可能 stale 或不可用 | Flink 容错我会表述成 checkpointed state + 从 log replay，而不是每个 worker 做 active replica。即使 Flink 内部是 exactly-once，外部 sink 仍然需要幂等或事务写，否则不会得到端到端 exactly-once effect。 |
| Late event 和 duplicate event | processing time window<br>✅ 简单，延迟低<br>❌ 乱序事件会落到错误窗口 | event time + watermark + allowed lateness<br>✅ 能处理乱序和迟到事件<br>❌ Watermark 选择影响延迟和准确性 | 离线 reconciliation<br>✅ 可用完整数据修复实时误差<br>❌ 不实时 | 我会用 event time、watermark、allowed lateness 和 deterministic event_id 处理乱序与重复。实时窗口只承诺在 lateness bound 内正确；重要统计还需要离线 reconciliation 修正极晚事件。 |
