...
| --- | --- | --- | --- | --- |
| Status feed：fanout-on-write vs fanout-on-read vs hybrid | fanout-on-write<br>✅ 读 feed 很快，直接读 user inbox<br>❌ 写放大，发布瞬间要写很多 viewer inbox | fanout-on-read<br>✅ 发布成本低，不会因为一个作者写爆 inbox<br>❌ 读 feed 需要查联系人列表和多作者 status，延迟高，容易 scatter-... | hybrid fanout<br>✅ 普通用户读快，大联系人用户避免写放大<br>❌ 实现复杂，需要区分作者类型，并在读路径 merge 两类候选 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Search status：服务端索引 vs per-user 可见索引 vs client-side search | 全局服务端搜索索引<br>✅ 搜索能力强，支持全文、prefix、ranking、拼写纠错<br>❌ 权限过滤复杂 | 按 viewer 构建可见索引<br>✅ 搜索天然只返回用户可见内容，读路径简单<br>❌ 写放大巨大，一个 status 要写入多个 viewer 的搜索索引 | client-side local search<br>✅ 隐私最好，服务端只提供同步和权限过滤后的密文内容<br>❌ 跨设备同步复杂 | I would make search freshness eventually consistent, but permissions, deletion, and visibility must be enforced at read time or by a strongly maintained ACL filter. That gives us a scalable index without turning stale search results into a security bug. |
| 权限一致性：发布时快照 vs 读取时动态计算 | 发布时快照 viewer list<br>✅ fanout 和搜索索引稳定，读路径简单<br>❌ 用户 block 某人后，历史 status 是否还能看会变得不符合直觉 | 读取时动态计算权限<br>✅ 隐私更安全，权限变更实时生效<br>❌ 读路径依赖 Privacy Service，延迟和可用性压力更大 | 快照 + deny list 动态覆盖<br>✅ 大多数权限走快照，block/delete 这种强隐私事件动态覆盖<br>❌ 语义更复杂，需要清晰定义优先级 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 24 小时过期：DB TTL vs 延迟队列 vs 时间桶扫描 | DB TTL<br>✅ 实现简单，自动清理，成本低<br>❌ TTL 删除不精确 | 延迟队列<br>✅ 到期处理及时，可以触发 index/cache/CDN 清理<br>❌ 队列规模巨大，重试和 worker failure 要处理 | 时间桶扫描<br>✅ 稳定、可恢复、容易 backfill<br>❌ 分钟级延迟 | I would not use TTL as the correctness mechanism. TTL is cleanup; correctness should come from an explicit expires_at check plus a recoverable scheduler or delayed queue that can tolerate duplicate execution and worker failure. |
| View receipt：同步写 vs 异步写 | 同步写 DB<br>✅ 数据立即可见，逻辑简单<br>❌ 观看路径延迟高，热点 status 可能把 DB 打爆 | 写 Kafka 异步聚合<br>✅ 观看路径快，削峰填谷，可批量写入 receipt store<br>❌ 作者看到 view list 有延迟 | 客户端批量上报<br>✅ 减少网络请求，提高电池效率<br>❌ 客户端 crash 可能丢事件 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Search index freshness 和一致性 | 同步写搜索索引<br>✅ 发布后立刻可搜<br>❌ 搜索索引故障会影响发布主链路 | Kafka 异步索引<br>✅ 解耦主链路<br>❌ 短暂不可搜索 | 异步索引 + read-time fallback<br>✅ 新 status 未进索引时，可从 inbox/metadata 补查最近内容<br>❌ Search Service 更复杂 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 多 Region：home region vs global active-active | 按用户 home region 存储<br>✅ 数据边界清晰，写入简单<br>❌ 跨 region 联系人查看可能有额外延迟 | 跨 region 复制 metadata<br>✅ 跨区域查看更快<br>❌ 复制延迟、删除/过期传播、隐私变更一致性更难 | active-active 多主<br>✅ 区域故障影响小<br>❌ 冲突处理、权限撤销、搜索索引一致性复杂度最高 | I would avoid global strong consistency unless it protects a concrete invariant. A home-region or regional-shard model keeps the write path understandable; cross-region replication can be asynchronous with conflict rules and reconciliation for derived views. |
