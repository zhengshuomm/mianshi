...
|---|---|---|---|
| Queue 可靠性：Kafka vs SQS | A: Kafka + manual retry topic<br>B: SQS<br>C: Kafka for raw log + SQS for fetch task | A ✅ 吞吐高。 可 replay，适合大规模日志式处理。 Consumer group 可自动 rebalance。 ❌ Kafka 本身没有 message visibility timeout。 Exponential backoff、retry count、DLQ 需要自己设计 retry topic。<br>B ✅ Visibility timeout：worker 拉到消息后，消息暂时对其他 worker 不可见。 处理成功后显式 delete。 原生支持 DLQ，retry 超过次数自动进入 DLQ。 ❌ 吞吐通常低于 Kafka。 消息模型更像任务队列，不适合大规模 replay 分析。<br>C ✅ Kafka 保存 crawl log/replay。 SQS 管具体 fetch task 的 visibility/retry。 ❌ 两套系统，运维和一致性更复杂。 | 如果强调高吞吐和 replay，用 Kafka + retry topics + DLQ。<br>如果强调任务不丢和简单 failure handling，用 SQS 更直接。<br>面试里要主动解释 visibility timeout 和 Kafka offset/retry 语义差异。 |
| Frontier 设计：单全局队列 vs front/back queues | A: 单全局 priority queue<br>B: Front queues + back queues<br>C: 每个 domain 独立 token bucket | A ✅ 实现简单。 ❌ 很难限制同一 domain 抓取频率。 热门 domain 可能被集中请求打爆。<br>B ✅ Front queues 管优先级。 Back queues 按 domain 管 politeness。 可以融合 robots.txt crawl-delay。 ❌ 调度器复杂。 需要维护 domain metadata。<br>C ✅ 速率控制清晰。 可动态调整慢站点。 ❌ 海量 domain 的 metadata 和 timer 管理复杂。 | 用 front/back queue。<br>Back queue 以 domain/host 为粒度，用 domain metadata 控制 crawl delay。 |
| URL 去重：直接比 URL vs hash index vs Bloom filter | A: 直接比较 URL string<br>B: URL normalize 后 hash，metadata DB 上建索引<br>C: Bloom filter on URL hash | A ✅ 简单，可解释。 ❌ URL 很长，占存储。 Normalization 不做好会漏掉重复。<br>B ✅ 查询快，存储小。 支持精确状态管理。 ❌ DB 写压力大。 Hash collision 虽小但要考虑。<br>C ✅ 内存低，查询快。 可减少 DB lookup。 ❌ 有 false positive，可能错过少量新 URL。 删除困难，适合 seen-set 而不是精确状态。 | Normalize + hash index 是主方案。<br>Bloom filter 做前置加速。<br>关键 URL 仍回查 metadata DB。 |
| URL normalization 和 redirect | A: 不 normalize<br>B: 规则化 normalization<br>C: Canonical URL + redirect alias graph | A ✅ 简单。 ❌ 重复抓取严重。 Crawl trap 更容易爆炸。<br>B ✅ 减少重复。 Query 参数排序、去 fragment、lowercase host 都可控。 ❌ 某些网站 query 参数顺序或大小写可能有语义。 规则过强可能误合并不同 URL。<br>C ✅ 可以处理 301/302、canonical link、alias。 A redirect 到 B 时 A 标记为 fetched/redirected，避免重复抓 A。 ❌ 需要维护 alias/canonical mapping。 | 使用 conservative normalization。<br>记录 redirect relation。<br>URL A 302 到 B 时，A 标记为 downloaded/redirected，B 进入 frontier。 |
| 内容去重：URL dedup vs content hash | A: 只做 URL dedup<br>B: content hash<br>C: near-duplicate fingerprint / SimHash | A ✅ 简单。 ❌ Alias、mirror、tracking 参数会导致重复内容。<br>B ✅ 相同内容可以识别。 Metadata table 里存 `url_hash + content_hash`，便于重爬比较。 ❌ 小改动会导致 hash 完全不同。 需要先下载内容才能判断。<br>C ✅ 能识别近重复页面。 ❌ 计算和调参复杂。 | URL dedup + content hash 是基本款。<br>大规模搜索索引用 SimHash/MinHash 做近重复。<br>Redis Bloom filter 可做 content hash 前置过滤，但最终状态仍进 metadata DB。 |
| Failure handling：立即重试 vs exponential backoff vs DLQ | A: 立即重试<br>B: Exponential backoff retry<br>C: DLQ after max retries | A ✅ 实现简单。 ❌ 容易放大故障。 对慢站点不 polite。<br>B ✅ 减少对故障站点压力。 给临时故障恢复时间。 ❌ 任务状态和 retry schedule 更复杂。<br>C ✅ 避免坏任务无限循环。 便于人工分析和规则修复。 ❌ DLQ 需要运营和监控，否则会堆积。 | 用 exponential backoff + max retry + DLQ。<br>Kafka 需要手动 retry topic；SQS 可以利用 visibility timeout 和 DLQ。 |
| Crawl trap：depth limit vs URL pattern detection vs tarpit timeout | A: depth limit<br>B: URL pattern / parameter explosion detection<br>C: Tarpit detection | A ✅ 简单，例如 `depth <= 20`。 ❌ 不能识别同一层级的无限 URL。<br>B ✅ 能过滤大量 spider traps。 ❌ 规则维护复杂。 可能误杀合法页面。<br>C ✅ 保护 worker 资源。 可对慢站点降级或拉黑。 ❌ 需要统计和异常检测。 | Depth limit + URL pattern detection + short timeout。<br>对异常 domain 降低 crawl rate 或 quarantine。 |
| Dynamic content：普通 HTML fetch vs headless browser | A: 只抓原始 HTML<br>B: Headless browser，如 Puppeteer<br>C: 选择性渲染 | A ✅ 快、便宜、吞吐高。 ❌ 抓不到 JS 渲染内容和动态链接。<br>B ✅ 能看到渲染后的 DOM。 能提取 JS 动态 URL。 ❌ 很慢，CPU/内存成本高。 容易触发反爬。<br>C ✅ 只对高价值或检测到 JS-heavy 页面使用 headless。 成本可控。 ❌ 需要分类器或规则判断是否渲染。 | 默认普通 fetch。<br>对高价值 JS-heavy 页面进入低 QPS render pool。 |
| Politeness：固定 delay vs domain adaptive rate limiter | A: 固定 crawl delay<br>B: robots.txt crawl-delay + domain metadata<br>C: adaptive domain rate limiter | A ✅ 实现简单。 ❌ 不同 domain 能力差异大。<br>B ✅ 遵守站点规则。 可记录 `last_url_crawl_ts`。 ❌ 不是所有站点都设置 crawl-delay。<br>C ✅ 根据 error rate、latency、429/503 动态降速。 对健康站点提高吞吐。 ❌ 调参复杂。 | Robots + domain metadata 是基础。<br>生产系统加 adaptive rate limiter。<br>Politeness 应该由 back queues 控制，而不是 worker 自己随便抓。 |
