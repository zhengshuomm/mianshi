...
| --- | --- | --- | --- | --- |
| Queue 可靠性：Kafka vs SQS | Kafka + manual retry topic<br>✅ 吞吐高<br>❌ Kafka 本身没有 message visibility timeout | SQS<br>✅ Visibility timeout：worker 拉到消息后，消息暂时对其他 wo...<br>❌ 吞吐通常低于 Kafka | Kafka for raw log + SQS for fetch task<br>✅ Kafka 保存 crawl log/replay<br>❌ 两套系统，运维和一致性更复杂 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
| Frontier 设计：单全局队列 vs front/back queues | 单全局 priority queue<br>✅ 实现简单<br>❌ 很难限制同一 domain 抓取频率 | Front queues + back queues<br>✅ Front queues 管优先级<br>❌ 调度器复杂 | 每个 domain 独立 token bucket<br>✅ 速率控制清晰<br>❌ 海量 domain 的 metadata 和 timer 管理复杂 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
| URL 去重：直接比 URL vs hash index vs Bloom filter | 直接比较 URL string<br>✅ 简单，可解释<br>❌ URL 很长，占存储 | URL normalize 后 hash，metadata DB 上建索引<br>✅ 查询快，存储小<br>❌ DB 写压力大 | Bloom filter on URL hash<br>✅ 内存低，查询快<br>❌ 有 false positive，可能错过少量新 URL | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| URL normalization 和 redirect | 不 normalize<br>✅ 简单<br>❌ 重复抓取严重 | 规则化 normalization<br>✅ 减少重复<br>❌ 某些网站 query 参数顺序或大小写可能有语义 | Canonical URL + redirect alias graph<br>✅ 可以处理 301/302、canonical link、alias<br>❌ 需要维护 alias/canonical mapping | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 内容去重：URL dedup vs content hash | 只做 URL dedup<br>✅ 简单<br>❌ Alias、mirror、tracking 参数会导致重复内容 | content hash<br>✅ 相同内容可以识别<br>❌ 小改动会导致 hash 完全不同 | near-duplicate fingerprint / SimHash<br>✅ 能识别近重复页面<br>❌ 计算和调参复杂 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| Failure handling：立即重试 vs exponential backoff vs DLQ | 立即重试<br>✅ 实现简单<br>❌ 容易放大故障 | Exponential backoff retry<br>✅ 减少对故障站点压力<br>❌ 任务状态和 retry schedule 更复杂 | DLQ after max retries<br>✅ 避免坏任务无限循环<br>❌ DLQ 需要运营和监控，否则会堆积 | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
| Crawl trap：depth limit vs URL pattern detection vs tarpit timeout | depth limit<br>✅ 简单，例如 `depth <= 20`<br>❌ 不能识别同一层级的无限 URL | URL pattern / parameter explosion detection<br>✅ 能过滤大量 spider traps<br>❌ 规则维护复杂 | Tarpit detection<br>✅ 保护 worker 资源<br>❌ 需要统计和异常检测 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Dynamic content：普通 HTML fetch vs headless browser | 只抓原始 HTML<br>✅ 快、便宜、吞吐高<br>❌ 抓不到 JS 渲染内容和动态链接 | Headless browser，如 Puppeteer<br>✅ 能看到渲染后的 DOM<br>❌ 很慢，CPU/内存成本高 | 选择性渲染<br>✅ 只对高价值或检测到 JS-heavy 页面使用 headless<br>❌ 需要分类器或规则判断是否渲染 | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| Politeness：固定 delay vs domain adaptive rate limiter | 固定 crawl delay<br>✅ 实现简单<br>❌ 不同 domain 能力差异大 | robots.txt crawl-delay + domain metadata<br>✅ 遵守站点规则<br>❌ 不是所有站点都设置 crawl-delay | adaptive domain rate limiter<br>✅ 根据 error rate、latency、429/503 动态降速<br>❌ 调参复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
