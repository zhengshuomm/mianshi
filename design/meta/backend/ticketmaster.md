...
|---|---|---|---|
| 库存模型：assigned seat vs general admission | A: Assigned seat，每个 seat 一条库存记录<br>B: General admission，按 ticket type 维护计数<br>C: Section-level inventory | A ✅ 精确控制每个座位。 用户体验好，可以展示 seat map。 ❌ 热门 event 下 seat-level contention 高。 seat map 更新和缓存更复杂。<br>B ✅ 写入简单，只需要扣减 counter。 容易分片和扩展。 ❌ 无法支持精确选座。 需要防止 counter oversell。<br>C ✅ 比 seat-level 更容易扩展。 比 GA 更有价格/区域控制。 ❌ 最终座位分配可能带来用户体验问题。 | 选座 event 用 assigned seat。<br>GA event 用 counter。<br>系统抽象成 inventory item，不同 event 选择不同 inventory strategy。 |
| 防 oversell：DB lock vs conditional write vs single-writer | A: DB transaction + row lock<br>B: Conditional write / compare-and-set<br>C: 按 event/section 单 writer actor | A ✅ 语义清晰，正确性强。 容易处理 `AVAILABLE -> HELD -> SOLD`。 ❌ 热点 seat/section 下锁竞争高。 DB 压力大。<br>B ✅ 只有状态仍是 `AVAILABLE` 才能更新成功。 易于水平扩展。 ❌ 复杂事务和跨多座位 hold 更难。 失败重试逻辑更复杂。<br>C ✅ 避免并发写冲突。 容易保证同一 partition 内顺序。 ❌ 单 partition 吞吐有限。 需要处理 actor failover 和 backlog。 | 基础设计用 DB row lock 或 conditional write。<br>热门 event 演进到按 `event_id/section_id` partition 的 single-writer 或 queue-based processor。 |
| Hold 票策略：短 TTL hold vs 直接支付扣库存 | A: 短 TTL hold<br>B: 不 hold，支付成功后再抢库存<br>C: 排队后限时 hold + 用户风控 | A ✅ 用户有时间支付。 避免支付过程中座位被别人买走。 ❌ 恶意用户可能大量 hold 不支付。 需要超时释放和清理机制。<br>B ✅ 库存不被无效占用。 系统状态更少。 ❌ 用户支付后可能发现没票，体验很差。 退款/补偿复杂。<br>C ✅ 控制并发和恶意占票。 可以按用户/设备/payment method 限制 hold。 ❌ 产品和风控复杂度更高。 | 用短 TTL hold。<br>热门 event 加 waiting room、per-user hold limit、风控校验。 |
| 开售高峰：直接放流量 vs waiting room | A: 直接让所有用户进购买页<br>B: Waiting room / virtual queue<br>C: Lottery / invite-based sale | A ✅ 实现简单，用户路径短。 ❌ 热门 event 会造成库存服务雪崩。 用户体验不稳定。<br>B ✅ 平滑流量，保护库存服务。 可以做 bot filtering 和公平排序。 ❌ 用户等待体验复杂。 排队公平性和作弊防护难。<br>C ✅ 极大降低瞬时峰值。 更容易做公平性和风控。 ❌ 不是所有业务都能接受。 产品形态变化较大。 | 普通 event 直接购买。<br>热门 event 使用 waiting room。<br>极端热门活动可以用 lottery 或 verified fan 模式。 |
| Seat map availability：强实时 vs 缓存近似 | A: 强实时读 Inventory DB<br>B: 缓存 availability summary<br>C: 实时推送 seat changes | A ✅ 展示准确。 ❌ 热门 event 下读压力巨大。 用户看到时准确，点选时仍可能被别人 hold。<br>B ✅ 读路径快，保护 Inventory DB。 section-level availability 足够用于浏览。 ❌ 显示可能短暂过期。 用户点 seat 时还要重新校验。<br>C ✅ 用户看到的 seat map 更接近实时。 ❌ 对热门 event 推送量大。 仍不能替代最终 hold 校验。 | 浏览层用缓存/近似 availability。<br>`POST /holds` 时做强校验。<br>面试里要主动说：展示可近似，购买必须强一致。 |
| 支付和出票：同步链路 vs 异步补偿 | A: 支付成功后同步出票和发通知<br>B: 支付成功后发布事件，异步出票和通知<br>C: Outbox + worker | A ✅ 用户路径直观。 ❌ 出票/通知失败会拖慢甚至破坏支付链路。<br>B ✅ 支付主链路短。 出票和通知可重试、可补偿。 ❌ 用户可能短暂看到 `payment succeeded, ticket pending`。<br>C ✅ 避免 DB 成功但 event 丢失。 适合支付/出票这种关键链路。 ❌ 实现更复杂。 | 用 payment callback 幂等更新 order。<br>用 outbox/event 异步出票。<br>用户允许短暂 pending，但系统必须最终补偿到一致状态。 |
| 反黄牛和公平性：限流 vs 身份校验 vs 队列策略 | A: 基础 rate limit<br>B: Verified account / payment method / phone<br>C: Waiting room + bot scoring + per-user quota | A ✅ 实现简单，成本低。 ❌ 对分布式 bot 效果有限。<br>B ✅ 提高作弊成本。 可以限制每人购买数量。 ❌ 增加用户摩擦。 误伤真实用户。<br>C ✅ 同时保护系统和公平性。 可以把可疑用户降权或拦截。 ❌ 风控系统复杂。 排队策略需要解释公平性。 | 普通 event 用 rate limit。<br>热门 event 用 waiting room + verified user + quota。<br>防黄牛是产品/风控/系统共同问题，不只是限流。 |
