...
| --- | --- | --- | --- | --- |
| Rollback 语义：物理 undo vs 版本指针切换 | 物理 undo<br>✅ 实现直观<br>❌ 需要扫描大量 key | MVCC + visible version 指针<br>✅ rollback 只更新 metadata，速度快<br>❌ 内存占用更高 | Copy-on-write snapshot tree<br>✅ snapshot 成本低，结构共享好<br>❌ 实现复杂 | 我会对 model、adapter、prompt template 和 safety policy 都做版本化，通过 shadow/canary 后再全量。Staff 级别要强调：质量、延迟、成本和安全回归都必须可回滚、可审计，而不是只关注部署成功。 |
| 版本粒度：per-key version vs global version | per-key version<br>✅ 简单，写入不依赖全局版本服务<br>❌ 无法表达“整个 namespace 回到同一个时间点” | global commit version<br>✅ 可以定义一致 snapshot<br>❌ 全局版本生成可能成为瓶颈 | hybrid logical clock / per-shard version + vector watermark<br>✅ 扩展性更好，跨 shard 写入少协调<br>❌ rollback 到全局一致点需要维护 vector clock/watermark... | 除非保护明确的业务 invariant，否则我会避免 global strong consistency。home region 或 regional shard 能让写路径更清楚；跨区复制可以异步，并为冲突规则和 reconciliation 留出设计空间。 |
| 持久化：纯内存 vs WAL + snapshot vs replicated log | 纯内存<br>✅ 最低延迟，最低复杂度<br>❌ 节点 crash 后数据和历史版本全部丢失 | WAL + periodic snapshot<br>✅ 写入 append-only，恢复路径清晰<br>❌ WAL fsync 影响写延迟 | quorum replicated log<br>✅ leader crash 后 follower 可接管<br>❌ 写延迟更高 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 内存控制：保留所有历史 vs retention + compaction | 保留所有历史版本<br>✅ 任何时间点都可恢复<br>❌ 内存不可控，不适合 in-memory KV | 按时间/版本数 retention<br>✅ 内存可预测<br>❌ 超过 retention 的版本不能 rollback | 冷热分层<br>✅ 热数据在内存，冷历史在 SSD/Object Store，成本低<br>❌ 历史读和远距离 rollback 变慢 | 我会按数据用途设计 retention：raw log 用于 replay，compacted state 用于 serving，cold storage 用于 audit。compaction/GC 可以省成本，但不能删掉恢复、合规或用户可见正确性所需的数据。 |
| 读写一致性：linearizable read vs snapshot read | eventual consistency<br>✅ 延迟最低，扩展简单<br>❌ rollback 后不同客户端可能读到不同当前版本，语义混乱 | 单 key linearizable<br>✅ 同一个 key 的写入顺序明确<br>❌ 跨 key 事务和全局快照仍不保证 | namespace snapshot isolation<br>✅ 读请求绑定一个 `visible_version/epoch`，不会读到混合 rol...<br>❌ Router、Shard、Metadata 都要传递 epoch | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| Rollback 后继续写：线性历史 vs 分支历史 | 线性历史继续前进<br>✅ 简单<br>❌ V11-V20 变成不可见但仍存在，语义需要解释清楚 | 显式分支<br>✅ 历史语义严谨，可以并行保留多条分支<br>❌ API、GC、读路径都复杂很多 | rollback 生成 compensating writes<br>✅ 审计友好<br>❌ 需要为每个 key 写补偿事件，rollback 很慢 | 我会对 model、adapter、prompt template 和 safety policy 都做版本化，通过 shadow/canary 后再全量。Staff 级别要强调：质量、延迟、成本和安全回归都必须可回滚、可审计，而不是只关注部署成功。 |
| 分片和全局 rollback | 暂停全局写入后 rollback<br>✅ 实现简单，correctness 清晰<br>❌ 影响可用性 | epoch 切换<br>✅ Metadata 原子更新 epoch 和 visible version<br>❌ 所有请求都要带 epoch | 两阶段 rollback<br>✅ 可以确保所有 shard 准备好再切换<br>❌ 协调成本高，失败恢复复杂 | 我会对 model、adapter、prompt template 和 safety policy 都做版本化，通过 shadow/canary 后再全量。Staff 级别要强调：质量、延迟、成本和安全回归都必须可回滚、可审计，而不是只关注部署成功。 |
