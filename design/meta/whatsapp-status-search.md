...
| --- | --- | --- | --- | --- |
| Status feed：fanout-on-write vs fanout-on-read vs hybrid | fanout-on-write<br>✅ 读 feed 很快，直接读 user inbox<br>❌ 写放大，发布瞬间要写很多 viewer inbox | fanout-on-read<br>✅ 发布成本低，不会因为一个作者写爆 inbox<br>❌ 读 feed 需要查联系人列表和多作者 status，延迟高，容易 scatter-... | hybrid fanout<br>✅ 普通用户读快，大联系人用户避免写放大<br>❌ 实现复杂，需要区分作者类型，并在读路径 merge 两类候选 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Search status：服务端索引 vs per-user 可见索引 vs client-side search | 全局服务端搜索索引<br>✅ 搜索能力强，支持全文、prefix、ranking、拼写纠错<br>❌ 权限过滤复杂 | 按 viewer 构建可见索引<br>✅ 搜索天然只返回用户可见内容，读路径简单<br>❌ 写放大巨大，一个 status 要写入多个 viewer 的搜索索引 | client-side local search<br>✅ 隐私最好，服务端只提供同步和权限过滤后的密文内容<br>❌ 跨设备同步复杂 | Search freshness 可以最终一致，但 permission、delete、visibility 不能最终一致。索引可以异步更新和重建，但查询时必须做 ACL/delete 校验，避免 stale index 变成安全问题。 |
| 权限一致性：发布时快照 vs 读取时动态计算 | 发布时快照 viewer list<br>✅ fanout 和搜索索引稳定，读路径简单<br>❌ 用户 block 某人后，历史 status 是否还能看会变得不符合直觉 | 读取时动态计算权限<br>✅ 隐私更安全，权限变更实时生效<br>❌ 读路径依赖 Privacy Service，延迟和可用性压力更大 | 快照 + deny list 动态覆盖<br>✅ 大多数权限走快照，block/delete 这种强隐私事件动态覆盖<br>❌ 语义更复杂，需要清晰定义优先级 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 24 小时过期：DB TTL vs 延迟队列 vs 时间桶扫描 | DB TTL<br>✅ 实现简单，自动清理，成本低<br>❌ TTL 删除不精确 | 延迟队列<br>✅ 到期处理及时，可以触发 index/cache/CDN 清理<br>❌ 队列规模巨大，重试和 worker failure 要处理 | 时间桶扫描<br>✅ 稳定、可恢复、容易 backfill<br>❌ 分钟级延迟 | 我不会把 TTL 当成 correctness 机制。TTL 适合清理；正确性要靠 read path 检查 expires_at，再配合可恢复的 scheduler 或 delayed queue，且 worker 执行必须幂等。 |
| View receipt：同步写 vs 异步写 | 同步写 DB<br>✅ 数据立即可见，逻辑简单<br>❌ 观看路径延迟高，热点 status 可能把 DB 打爆 | 写 Kafka 异步聚合<br>✅ 观看路径快，削峰填谷，可批量写入 receipt store<br>❌ 作者看到 view list 有延迟 | 客户端批量上报<br>✅ 减少网络请求，提高电池效率<br>❌ 客户端 crash 可能丢事件 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Search index freshness 和一致性 | 同步写搜索索引<br>✅ 发布后立刻可搜<br>❌ 搜索索引故障会影响发布主链路 | Kafka 异步索引<br>✅ 解耦主链路<br>❌ 短暂不可搜索 | 异步索引 + read-time fallback<br>✅ 新 status 未进索引时，可从 inbox/metadata 补查最近内容<br>❌ Search Service 更复杂 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 多 Region：home region vs global active-active | 按用户 home region 存储<br>✅ 数据边界清晰，写入简单<br>❌ 跨 region 联系人查看可能有额外延迟 | 跨 region 复制 metadata<br>✅ 跨区域查看更快<br>❌ 复制延迟、删除/过期传播、隐私变更一致性更难 | active-active 多主<br>✅ 区域故障影响小<br>❌ 冲突处理、权限撤销、搜索索引一致性复杂度最高 | 除非保护明确的业务 invariant，否则我会避免 global strong consistency。home region 或 regional shard 能让写路径更清楚；跨区复制可以异步，并为冲突规则和 reconciliation 留出设计空间。 |
