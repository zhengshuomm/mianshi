...
| Rider Location Store：按 rider_id vs geo cell | 按 rider_id 存当前位置<br>✅ 查询单 rider 很快，更新简单<br>❌ 找附近 rider 需要额外索引 | 按 geo cell 维护 rider set<br>✅ 查附近可用 rider 快，可按 city/zone 分片<br>❌ rider 移动时需要从旧 cell 删除、加入新 cell，处理旧位置和 TTL... | 双写 rider_id current + geo index<br>✅ tracking 和 dispatch 都高效<br>❌ 双写一致性要处理，可能出现 geo index 残留旧 rider | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| ETA：规则估算 vs ML 估算 | 规则估算<br>✅ 简单可解释，冷启动快<br>❌ 无法捕捉餐馆备餐差异、天气、交通、高峰拥堵 | 地图路线时间 + 历史备餐时间<br>✅ 准确度显著提升，成本可控<br>❌ 地图 API 成本高 | ML ETA<br>✅ 能利用餐馆、rider、天气、时间、区域、历史误差做校准<br>❌ 训练复杂，线上要监控 drift | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| Cache 和派生数据一致性 | 强同步更新所有派生数据<br>✅ 读到旧数据概率低<br>❌ 写链路依赖太多，任何索引/cache 故障会影响主业务 | CDC/Event async update<br>✅ 解耦主写入<br>❌ 短暂不一致，用户可能看到已售罄/关店餐馆 | 异步更新 + read-time validation<br>✅ 搜索快，同时下单前保证正确<br>❌ 需要额外校验和降级逻辑 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 故障处理：支付、商家、rider、位置链路 | 同步等待所有步骤完成<br>✅ 用户立刻知道最终结果<br>❌ 外部系统慢会拖垮下单体验 | 订单状态机 + 异步事件<br>✅ 每步可重试、可超时、可补偿<br>❌ 状态和边界条件多，需要良好可观测性 | Workflow engine<br>✅ 超时、重试、补偿、人工介入更清晰<br>❌ 引入 Temporal/Step Functions 等复杂度 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
