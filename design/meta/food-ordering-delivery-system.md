...
| --- | --- | --- | --- | --- |
| 搜索：Search Index vs DB Query vs Cache | 直接查 DB<br>✅ source of truth，数据一致性强<br>❌ 全文搜索、模糊匹配、相关性排序弱 | Elasticsearch/OpenSearch<br>✅ 搜索能力强，支持 inverted index、filter、ranking<br>❌ 索引是异步派生数据，可能和 DB 状态短暂不一致 | 热门 query/cache + 搜索索引<br>✅ 降低搜索成本，热门词延迟低<br>❌ 个性化、位置和实时营业状态会降低缓存命中率 | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
| 下单一致性：同步事务 vs Saga | 单体事务<br>✅ 实现简单，一致性强<br>❌ 跨支付外部系统和派单系统不可行，扩展差 | 两阶段提交<br>✅ 理论一致性强<br>❌ 外部支付/商家/rider 不支持 | Saga + 状态机<br>✅ 每步本地提交，通过事件推进<br>❌ 状态机和补偿逻辑复杂，最终一致 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 重复下单和重复扣款 | 前端禁用按钮<br>✅ 减少重复请求<br>❌ 不能作为正确性保障，网络重试仍会发生 | 订单 idempotency key<br>✅ 同一个用户同一个 key 只创建一次订单<br>❌ 需要保存 key 到结果的映射，并设置合理 TTL | 支付幂等 + webhook 去重<br>✅ 避免重复扣款<br>❌ 要管理 payment_id、external_payment_id、回调状态 | I would first define the business invariant, such as no oversell, one winner, or one successful payment capture. The critical transition should be protected with a conditional write or state machine, and all external side effects should be idempotent and reconciled asynchronously. |
| Rider 派单：最近 rider vs 优化匹配 | 选择最近空闲 rider<br>✅ 实现简单，响应快<br>❌ 不考虑备餐时间、rider 当前任务、未来需求，整体效率不一定最优 | 规则打分<br>✅ 可解释，能考虑距离、ETA、接单率、当前负载<br>❌ 规则调参复杂，区域差异大 | 批量匹配/优化算法<br>✅ 可以全局优化等待时间、空驶距离和准时率<br>❌ 计算复杂，等待 batching window 会增加单个订单延迟 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Rider 实时位置：Push vs Pull，WebSocket vs SSE | Client polling<br>✅ 实现简单，连接成本低<br>❌ 实时性差，高频 polling 浪费资源 | SSE<br>✅ 比 WebSocket 简单，天然适合 user 只接收更新<br>❌ 只支持服务端到客户端 | WebSocket<br>✅ 实时性好，双向能力强<br>❌ 连接管理和扩容更复杂，gateway failure 要支持重连恢复 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Rider Location Store：按 rider_id vs geo cell | 按 rider_id 存当前位置<br>✅ 查询单 rider 很快，更新简单<br>❌ 找附近 rider 需要额外索引 | 按 geo cell 维护 rider set<br>✅ 查附近可用 rider 快，可按 city/zone 分片<br>❌ rider 移动时需要从旧 cell 删除、加入新 cell，处理旧位置和 TTL... | 双写 rider_id current + geo index<br>✅ tracking 和 dispatch 都高效<br>❌ 双写一致性要处理，可能出现 geo index 残留旧 rider | I would choose the geo index based on update frequency, boundary behavior, and query radius rather than implementation convenience. For high-scale dynamic location, hierarchical cells such as H3/S2 plus geo-sharding are usually easier to operate than a single relational geo index. |
| ETA：规则估算 vs ML 估算 | 规则估算<br>✅ 简单可解释，冷启动快<br>❌ 无法捕捉餐馆备餐差异、天气、交通、高峰拥堵 | 地图路线时间 + 历史备餐时间<br>✅ 准确度显著提升，成本可控<br>❌ 地图 API 成本高 | ML ETA<br>✅ 能利用餐馆、rider、天气、时间、区域、历史误差做校准<br>❌ 训练复杂，线上要监控 drift | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| Cache 和派生数据一致性 | 强同步更新所有派生数据<br>✅ 读到旧数据概率低<br>❌ 写链路依赖太多，任何索引/cache 故障会影响主业务 | CDC/Event async update<br>✅ 解耦主写入<br>❌ 短暂不一致，用户可能看到已售罄/关店餐馆 | 异步更新 + read-time validation<br>✅ 搜索快，同时下单前保证正确<br>❌ 需要额外校验和降级逻辑 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 故障处理：支付、商家、rider、位置链路 | 同步等待所有步骤完成<br>✅ 用户立刻知道最终结果<br>❌ 外部系统慢会拖垮下单体验 | 订单状态机 + 异步事件<br>✅ 每步可重试、可超时、可补偿<br>❌ 状态和边界条件多，需要良好可观测性 | Workflow engine<br>✅ 超时、重试、补偿、人工介入更清晰<br>❌ 引入 Temporal/Step Functions 等复杂度 | I would first define the business invariant, such as no oversell, one winner, or one successful payment capture. The critical transition should be protected with a conditional write or state machine, and all external side effects should be idempotent and reconciled asynchronously. |
