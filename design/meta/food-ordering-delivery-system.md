...
| --- | --- | --- | --- | --- |
| 搜索：Search Index vs DB Query vs Cache | 直接查 DB<br>✅ source of truth，数据一致性强<br>❌ 全文搜索、模糊匹配、相关性排序弱 | Elasticsearch/OpenSearch<br>✅ 搜索能力强，支持 inverted index、filter、ranking<br>❌ 索引是异步派生数据，可能和 DB 状态短暂不一致 | 热门 query/cache + 搜索索引<br>✅ 降低搜索成本，热门词延迟低<br>❌ 个性化、位置和实时营业状态会降低缓存命中率 | 用 Search Index 做召回和文本相关性，返回前通过 Availability/Restaurant Service 做最终校验。热门 query 缓存候选，不缓存最终排序。 |
| 下单一致性：同步事务 vs Saga | 单体事务<br>✅ 实现简单，一致性强<br>❌ 跨支付外部系统和派单系统不可行，扩展差 | 两阶段提交<br>✅ 理论一致性强<br>❌ 外部支付/商家/rider 不支持 | Saga + 状态机<br>✅ 每步本地提交，通过事件推进<br>❌ 状态机和补偿逻辑复杂，最终一致 | 用 Saga + order state machine。订单状态是 source of truth；支付授权失败取消订单，商家拒单触发退款，rider 超时触发重新派单。 |
| 重复下单和重复扣款 | 前端禁用按钮<br>✅ 减少重复请求<br>❌ 不能作为正确性保障，网络重试仍会发生 | 订单 idempotency key<br>✅ 同一个用户同一个 key 只创建一次订单<br>❌ 需要保存 key 到结果的映射，并设置合理 TTL | 支付幂等 + webhook 去重<br>✅ 避免重复扣款<br>❌ 要管理 payment_id、external_payment_id、回调状态 | 下单 API 使用 idempotency key，Payment Service 也使用独立 payment idempotency key。订单和支付都要幂等，不能只靠一个层面。 |
| Rider 派单：最近 rider vs 优化匹配 | 选择最近空闲 rider<br>✅ 实现简单，响应快<br>❌ 不考虑备餐时间、rider 当前任务、未来需求，整体效率不一定最优 | 规则打分<br>✅ 可解释，能考虑距离、ETA、接单率、当前负载<br>❌ 规则调参复杂，区域差异大 | 批量匹配/优化算法<br>✅ 可以全局优化等待时间、空驶距离和准时率<br>❌ 计算复杂，等待 batching window 会增加单个订单延迟 | 常规用规则打分；高峰和高密度区域用短窗口 batch matching。接单 lease 使用 TTL，避免同一 rider 被多个订单占用。 |
...
| Rider Location Store：按 rider_id vs geo cell | 按 rider_id 存当前位置<br>✅ 查询单 rider 很快，更新简单<br>❌ 找附近 rider 需要额外索引 | 按 geo cell 维护 rider set<br>✅ 查附近可用 rider 快，可按 city/zone 分片<br>❌ rider 移动时需要从旧 cell 删除、加入新 cell，处理旧位置和 TTL... | 双写 rider_id current + geo index<br>✅ tracking 和 dispatch 都高效<br>❌ 双写一致性要处理，可能出现 geo index 残留旧 rider | 双写 current location 和 geo cell index。位置带 timestamp/sequence_id，geo index 用 TTL 和 periodic cleanup 处理残留。 |
| ETA：规则估算 vs ML 估算 | 规则估算<br>✅ 简单可解释，冷启动快<br>❌ 无法捕捉餐馆备餐差异、天气、交通、高峰拥堵 | 地图路线时间 + 历史备餐时间<br>✅ 准确度显著提升，成本可控<br>❌ 地图 API 成本高 | ML ETA<br>✅ 能利用餐馆、rider、天气、时间、区域、历史误差做校准<br>❌ 训练复杂，线上要监控 drift | 先用 route estimate + 历史备餐分位数，后续升级 ML ETA。用户端展示做平滑，避免每次位置更新 ETA 大幅跳动。 |
| Cache 和派生数据一致性 | 强同步更新所有派生数据<br>✅ 读到旧数据概率低<br>❌ 写链路依赖太多，任何索引/cache 故障会影响主业务 | CDC/Event async update<br>✅ 解耦主写入<br>❌ 短暂不一致，用户可能看到已售罄/关店餐馆 | 异步更新 + read-time validation<br>✅ 搜索快，同时下单前保证正确<br>❌ 需要额外校验和降级逻辑 | Search/Menu/Availability 用异步派生数据，但下单前必须 read-time validate source of truth：餐馆营业、菜单价格、库存、配送范围。 |
| 故障处理：支付、商家、rider、位置链路 | 同步等待所有步骤完成<br>✅ 用户立刻知道最终结果<br>❌ 外部系统慢会拖垮下单体验 | 订单状态机 + 异步事件<br>✅ 每步可重试、可超时、可补偿<br>❌ 状态和边界条件多，需要良好可观测性 | Workflow engine<br>✅ 超时、重试、补偿、人工介入更清晰<br>❌ 引入 Temporal/Step Functions 等复杂度 | 订单核心用状态机；超时和补偿可用 workflow engine。rider 不响应触发重新派单，商家超时触发取消/退款，位置中断时 fallback 到最后位置 + ETA 降级提示。 |
