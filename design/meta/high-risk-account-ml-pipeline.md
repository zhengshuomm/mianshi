...
| --- | --- | --- | --- | --- |
| 在线同步拦截 vs 异步检测 | 同步评分并立即决策<br>✅ 可以实时阻止损失<br>❌ 增加主链路延迟 | 异步检测和事后处置<br>✅ 不拖慢主链路<br>❌ 可能放过短时间攻击 | 同步轻模型 + 异步重模型<br>✅ 同步路径低延迟，异步路径提高 recall<br>❌ 策略复杂，需要合并多个模型/规则输出 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Label 设计：人工审核、业务结果、用户申诉 | 只用人工审核 label<br>✅ label 可解释，质量相对高<br>❌ 覆盖不足 | 只用业务结果 label<br>✅ 客观，规模大<br>❌ label delay 长 | 多来源 label + confidence<br>✅ 覆盖广，可给不同 label source 加权<br>❌ label 冲突和噪声处理复杂 | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| Feature freshness：离线特征 vs 实时特征 | 只用离线 batch features<br>✅ 稳定，成本低，特征复杂度高<br>❌ 对突发攻击不敏感 | 只用实时 streaming features<br>✅ 反应快<br>❌ 窗口状态成本高 | 离线长期画像 + 实时短窗口<br>✅ 既有稳定画像，又能捕捉突发异常<br>❌ 训练-serving consistency 更难 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 特征一致性：训练-serving skew | 训练和在线各自写特征逻辑<br>✅ 实现快<br>❌ 容易 skew | 统一 Feature Store 和 feature definition<br>✅ 特征定义一致，复用高<br>❌ 平台建设成本高 | point-in-time join + 在线特征回放<br>✅ 避免 data leakage，可以复现线上决策<br>❌ 数据工程复杂，存储成本高 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 模型选择：规则、GBDT、深度模型、图模型 | 规则引擎<br>✅ 可解释，发布快，适合紧急止血<br>❌ 泛化差，容易被攻击者绕过 | GBDT/Logistic Regression<br>✅ 效果强、延迟低、可解释性较好<br>❌ 对序列行为和复杂图关系表达有限 | Deep Learning / GNN<br>✅ 能捕捉复杂模式和团伙风险<br>❌ 训练/ serving 成本高，可解释性差，上线风险更高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 阈值和动作策略：单阈值 vs 分层决策 | 单一 block threshold<br>✅ 实现简单<br>❌ 不能区分不同风险等级 | 多档策略<br>✅ 低风险 allow，中风险 challenge/review，高风险 block<br>❌ 阈值调参和监控更复杂 | 按 action_type 动态阈值<br>✅ 提现比浏览更严格，策略贴合业务<br>❌ 需要按场景维护模型校准和策略 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 高风险团伙检测：单账户特征 vs 图关联 | 只看单账户特征<br>✅ 简单，在线低延迟<br>❌ 对团伙攻击 recall 差 | 离线图计算风险分<br>✅ 能发现共享设备、IP、支付方式、邀请链路的团伙<br>❌ 实时性弱 | 在线邻居特征<br>✅ 可以实时判断新账户是否连接到高风险子图<br>❌ 在线图查询成本高，热点实体容易放大延迟 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Failover 和降级策略 | fail open<br>✅ 业务可用性高<br>❌ 攻击窗口变大 | fail closed<br>✅ 安全优先<br>❌ 可能导致大面积用户不可用 | cached decision / fallback model<br>✅ Feature Store 或模型故障时仍可给出保守决策<br>❌ 缓存过期和策略一致性要管理 | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
