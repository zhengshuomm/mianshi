...
| --- | --- | --- | --- | --- |
| 在线同步拦截 vs 异步检测 | 同步评分并立即决策<br>✅ 可以实时阻止损失<br>❌ 增加主链路延迟 | 异步检测和事后处置<br>✅ 不拖慢主链路<br>❌ 可能放过短时间攻击 | 同步轻模型 + 异步重模型<br>✅ 同步路径低延迟，异步路径提高 recall<br>❌ 策略复杂，需要合并多个模型/规则输出 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Label 设计：人工审核、业务结果、用户申诉 | 只用人工审核 label<br>✅ label 可解释，质量相对高<br>❌ 覆盖不足 | 只用业务结果 label<br>✅ 客观，规模大<br>❌ label delay 长 | 多来源 label + confidence<br>✅ 覆盖广，可给不同 label source 加权<br>❌ label 冲突和噪声处理复杂 | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| Feature freshness：离线特征 vs 实时特征 | 只用离线 batch features<br>✅ 稳定，成本低，特征复杂度高<br>❌ 对突发攻击不敏感 | 只用实时 streaming features<br>✅ 反应快<br>❌ 窗口状态成本高 | 离线长期画像 + 实时短窗口<br>✅ 既有稳定画像，又能捕捉突发异常<br>❌ 训练-serving consistency 更难 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| 特征一致性：训练-serving skew | 训练和在线各自写特征逻辑<br>✅ 实现快<br>❌ 容易 skew | 统一 Feature Store 和 feature definition<br>✅ 特征定义一致，复用高<br>❌ 平台建设成本高 | point-in-time join + 在线特征回放<br>✅ 避免 data leakage，可以复现线上决策<br>❌ 数据工程复杂，存储成本高 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 模型选择：规则、GBDT、深度模型、图模型 | 规则引擎<br>✅ 可解释，发布快，适合紧急止血<br>❌ 泛化差，容易被攻击者绕过 | GBDT/Logistic Regression<br>✅ 效果强、延迟低、可解释性较好<br>❌ 对序列行为和复杂图关系表达有限 | Deep Learning / GNN<br>✅ 能捕捉复杂模式和团伙风险<br>❌ 训练/ serving 成本高，可解释性差，上线风险更高 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 阈值和动作策略：单阈值 vs 分层决策 | 单一 block threshold<br>✅ 实现简单<br>❌ 不能区分不同风险等级 | 多档策略<br>✅ 低风险 allow，中风险 challenge/review，高风险 block<br>❌ 阈值调参和监控更复杂 | 按 action_type 动态阈值<br>✅ 提现比浏览更严格，策略贴合业务<br>❌ 需要按场景维护模型校准和策略 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 高风险团伙检测：单账户特征 vs 图关联 | 只看单账户特征<br>✅ 简单，在线低延迟<br>❌ 对团伙攻击 recall 差 | 离线图计算风险分<br>✅ 能发现共享设备、IP、支付方式、邀请链路的团伙<br>❌ 实时性弱 | 在线邻居特征<br>✅ 可以实时判断新账户是否连接到高风险子图<br>❌ 在线图查询成本高，热点实体容易放大延迟 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Failover 和降级策略 | fail open<br>✅ 业务可用性高<br>❌ 攻击窗口变大 | fail closed<br>✅ 安全优先<br>❌ 可能导致大面积用户不可用 | cached decision / fallback model<br>✅ Feature Store 或模型故障时仍可给出保守决策<br>❌ 缓存过期和策略一致性要管理 | I would assume workers can crash after doing partial work. The system should use leases or visibility timeouts, bounded retries, DLQs, and idempotent handlers so recovery is a normal execution path rather than an exceptional manual repair. |
