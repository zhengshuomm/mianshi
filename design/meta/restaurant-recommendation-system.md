...
| --- | --- | --- | --- | --- |
| 召回策略：Geo-first vs Personalized-first | Geo-first<br>✅ 候选一定可服务，召回规模小，延迟低<br>❌ 个性化弱，可能错过用户喜欢但稍远的餐馆 | Personalized-first<br>✅ 个性化强，能推荐用户真正喜欢的类别<br>❌ 容易召回不可配送/太远/已关店的餐馆，浪费排序资源 | Hybrid multi-recall<br>✅ Geo、个性化、热门、新店、促销各自有 quota，覆盖更全面<br>❌ 候选去重、quota 调参和召回归因更复杂 | I would choose the geo index based on update frequency, boundary behavior, and query radius rather than implementation convenience. For high-scale dynamic location, hierarchical cells such as H3/S2 plus geo-sharding are usually easier to operate than a single relational geo index. |
| 排序目标：CTR vs CVR vs Long-term Satisfaction | 优化 CTR<br>✅ 样本多，反馈快，模型容易训练<br>❌ 容易标题党/图片党，和业务收入及满意度不完全一致 | 优化 CVR/Order<br>✅ 更接近业务收益<br>❌ 订单样本少，反馈慢 | 多目标排序<br>✅ 同时考虑点击、下单、复购、取消率、等待时间、差评风险<br>❌ 目标权重和校准复杂 | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| 实时可用性：强过滤 vs 排序特征 | 把可用性当 hard filter<br>✅ 不会推荐用户无法下单的餐馆<br>❌ 如果状态延迟或误报，会直接损失曝光 | 把可用性当 ranking feature<br>✅ 排序更平滑，不会因为短期波动完全消失<br>❌ 如果特征过期，用户体验会变差 | hard filter + soft penalty<br>✅ 不可服务的过滤掉，可服务但体验差的降权<br>❌ 需要清晰定义哪些是硬约束、哪些是软约束 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| 地理索引：PostGIS vs Geohash/H3/S2 vs Redis GEO | PostGIS<br>✅ 地理能力强，支持复杂空间查询<br>❌ 高 QPS 在线推荐压力大，需要 cache/read replica | Geohash/H3/S2<br>✅ 易分片，可预计算 cell -> restaurants，查询快<br>❌ 边界问题，需要查邻居 cell | Redis GEO<br>✅ 实现简单，速度快<br>❌ 本质是 sorted set，复杂 polygon 和频繁大规模更新会吃力 | I would make search freshness eventually consistent, but permissions, deletion, and visibility must be enforced at read time or by a strongly maintained ACL filter. That gives us a scalable index without turning stale search results into a security bug. |
| 冷启动：新用户 vs 新餐馆 | 热门兜底<br>✅ 稳定，转化不差<br>❌ 个性化弱，头部效应更严重 | 内容/属性召回<br>✅ 可根据 cuisine、价格、位置、图片、菜单做匹配<br>❌ 内容质量不稳定，无法完全代表真实体验 | 受控探索<br>✅ 可以收集反馈，避免系统只推老餐馆<br>❌ 探索过多会降低短期转化 | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| 探索利用：只推最高分 vs Bandit | 纯 exploitation<br>✅ 指标稳定，用户体验可控<br>❌ 反馈闭环偏置严重，长尾无法学习 | 固定比例探索<br>✅ 实现简单，容易解释<br>❌ 探索效率低，可能浪费流量 | Contextual bandit<br>✅ 根据不确定性和上下文分配探索流量<br>❌ 实现和评估复杂，需要严格 guardrail | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| 训练数据偏差：position bias 和 selection bias | 直接用点击/订单训练<br>✅ 简单，数据量大<br>❌ 模型会强化现有排序偏差 | 加入 position feature<br>✅ 能部分校正位置影响<br>❌ 不能完全消除曝光选择偏差 | 随机探索 + debias 方法<br>✅ 可以估计 propensity，做 IPS/DR 等反事实评估<br>❌ 需要牺牲少量流量做探索，分析复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 缓存和一致性：推荐结果缓存 vs 实时重排 | 缓存完整推荐列表<br>✅ 延迟低，成本低<br>❌ 个性化弱，状态容易过期 | 完全实时计算<br>✅ 最新、最个性化<br>❌ 成本高，峰值压力大 | 缓存候选 + 实时 rerank<br>✅ 召回成本低，最终排序仍能利用实时特征<br>❌ 工程复杂，需要候选 cache 失效和版本管理 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
