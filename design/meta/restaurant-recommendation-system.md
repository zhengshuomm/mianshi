...
| --- | --- | --- | --- | --- |
| 召回策略：Geo-first vs Personalized-first | Geo-first<br>✅ 候选一定可服务，召回规模小，延迟低<br>❌ 个性化弱，可能错过用户喜欢但稍远的餐馆 | Personalized-first<br>✅ 个性化强，能推荐用户真正喜欢的类别<br>❌ 容易召回不可配送/太远/已关店的餐馆，浪费排序资源 | Hybrid multi-recall<br>✅ Geo、个性化、热门、新店、促销各自有 quota，覆盖更全面<br>❌ 候选去重、quota 调参和召回归因更复杂 | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| 排序目标：CTR vs CVR vs Long-term Satisfaction | 优化 CTR<br>✅ 样本多，反馈快，模型容易训练<br>❌ 容易标题党/图片党，和业务收入及满意度不完全一致 | 优化 CVR/Order<br>✅ 更接近业务收益<br>❌ 订单样本少，反馈慢 | 多目标排序<br>✅ 同时考虑点击、下单、复购、取消率、等待时间、差评风险<br>❌ 目标权重和校准复杂 | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| 实时可用性：强过滤 vs 排序特征 | 把可用性当 hard filter<br>✅ 不会推荐用户无法下单的餐馆<br>❌ 如果状态延迟或误报，会直接损失曝光 | 把可用性当 ranking feature<br>✅ 排序更平滑，不会因为短期波动完全消失<br>❌ 如果特征过期，用户体验会变差 | hard filter + soft penalty<br>✅ 不可服务的过滤掉，可服务但体验差的降权<br>❌ 需要清晰定义哪些是硬约束、哪些是软约束 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 地理索引：PostGIS vs Geohash/H3/S2 vs Redis GEO | PostGIS<br>✅ 地理能力强，支持复杂空间查询<br>❌ 高 QPS 在线推荐压力大，需要 cache/read replica | Geohash/H3/S2<br>✅ 易分片，可预计算 cell -> restaurants，查询快<br>❌ 边界问题，需要查邻居 cell | Redis GEO<br>✅ 实现简单，速度快<br>❌ 本质是 sorted set，复杂 polygon 和频繁大规模更新会吃力 | Search freshness 可以最终一致，但 permission、delete、visibility 不能最终一致。索引可以异步更新和重建，但查询时必须做 ACL/delete 校验，避免 stale index 变成安全问题。 |
| 冷启动：新用户 vs 新餐馆 | 热门兜底<br>✅ 稳定，转化不差<br>❌ 个性化弱，头部效应更严重 | 内容/属性召回<br>✅ 可根据 cuisine、价格、位置、图片、菜单做匹配<br>❌ 内容质量不稳定，无法完全代表真实体验 | 受控探索<br>✅ 可以收集反馈，避免系统只推老餐馆<br>❌ 探索过多会降低短期转化 | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| 探索利用：只推最高分 vs Bandit | 纯 exploitation<br>✅ 指标稳定，用户体验可控<br>❌ 反馈闭环偏置严重，长尾无法学习 | 固定比例探索<br>✅ 实现简单，容易解释<br>❌ 探索效率低，可能浪费流量 | Contextual bandit<br>✅ 根据不确定性和上下文分配探索流量<br>❌ 实现和评估复杂，需要严格 guardrail | 我会把它同时看成 ML quality 问题和 serving system 问题。Staff 级别要讲 feature freshness、training-serving consistency、bias/guardrail metrics，以及模型或 feature store 故障时的安全 fallback。 |
| 训练数据偏差：position bias 和 selection bias | 直接用点击/订单训练<br>✅ 简单，数据量大<br>❌ 模型会强化现有排序偏差 | 加入 position feature<br>✅ 能部分校正位置影响<br>❌ 不能完全消除曝光选择偏差 | 随机探索 + debias 方法<br>✅ 可以估计 propensity，做 IPS/DR 等反事实评估<br>❌ 需要牺牲少量流量做探索，分析复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 缓存和一致性：推荐结果缓存 vs 实时重排 | 缓存完整推荐列表<br>✅ 延迟低，成本低<br>❌ 个性化弱，状态容易过期 | 完全实时计算<br>✅ 最新、最个性化<br>❌ 成本高，峰值压力大 | 缓存候选 + 实时 rerank<br>✅ 召回成本低，最终排序仍能利用实时特征<br>❌ 工程复杂，需要候选 cache 失效和版本管理 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
