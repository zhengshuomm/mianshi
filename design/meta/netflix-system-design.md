...
| --- | --- | --- | --- | --- |
| 视频分发：源站直出 vs CDN vs 自建 Edge Cache | 源站/Object Store 直出<br>✅ 实现简单，内容一致性好<br>❌ 延迟高，带宽成本高，源站容易被打爆 | 第三方 CDN<br>✅ 上线快，覆盖广，弹性好<br>❌ 成本高 | 自建 Edge Cache / Open Connect 类方案<br>✅ 降低带宽成本，提升 ISP 侧播放质量，可预热热门内容<br>❌ 建设和运维成本高，需要容量预测和内容放置策略 | 面试基础设计用 CDN；如果讨论 Netflix 规模，进一步讲自建 edge cache，按地区预热热门 titles 和前几个 segments。 |
| 视频编码：单一版本 vs Adaptive Bitrate | 单一分辨率/码率<br>✅ 转码简单，存储成本低<br>❌ 弱网卡顿，高端设备画质差 | 多 bitrate ladder + HLS/DASH<br>✅ 客户端可根据网络选择合适 segment，降低 buffering<br>❌ 转码成本和存储成本增加 | per-title encoding<br>✅ 不同内容使用不同 bitrate ladder，动画/动作片分别优化<br>❌ 编码策略复杂，需要质量评估 pipeline | 用 HLS/DASH + adaptive bitrate；高阶讨论 per-title encoding 和 codec 选择，如 H.264 兼容性好，AV1 压缩率高但编码和设备支持成本更高。 |
| Manifest 设计和授权 | 静态 public manifest<br>✅ CDN 缓存友好，实现简单<br>❌ 无法按用户、地区、设备、订阅权限控制 | 动态 manifest<br>✅ 可根据 region、device codec、DRM、订阅等级返回不同版本<br>❌ 缓存命中率低，Playback Service 压力更大 | 静态 manifest + signed token/filter<br>✅ manifest 可缓存，segment URL 通过短 TTL token 控权<br>❌ 权限变化和 token 过期处理更复杂 | 使用动态生成或模板化 manifest，结合短 TTL signed URL 和 DRM license。热门 manifest 可以按 device/region 维度缓存。 |
| Playback session：强校验 vs 缓存授权 | 每次播放都强查所有依赖<br>✅ 权限最新，逻辑直接<br>❌ 依赖多，延迟高 | Entitlement cache<br>✅ 低延迟，依赖故障时可短时间继续播放<br>❌ 订阅取消、版权下架等变更有短暂不一致 | session token + 短 TTL<br>✅ 播放期间减少重复校验，token 过期后重新校验<br>❌ token 泄露和重放要防护 | 创建 session 时强校验，播放过程中使用短 TTL session token + DRM license。高风险变更通过 token 失效列表或短 TTL 控制窗口。 |
| 播放进度一致性：同步写 vs 异步事件 | 每个 heartbeat 同步写 DB<br>✅ 进度最新<br>❌ 写压力巨大，影响播放链路 | 客户端定期批量上报，服务端异步聚合<br>✅ 降低写量，削峰填谷<br>❌ 进度可能滞后，客户端 crash 前最后几秒可能丢失 | 关键事件同步 + heartbeat 异步<br>✅ start/pause/stop/complete 更准确，普通 heartbeat...<br>❌ 实现比单一路径复杂 | 使用关键事件同步或准同步，heartbeat 异步聚合。Continue watching 允许几秒误差，不应影响播放质量。 |
| 推荐系统与播放系统隔离 | 推荐和播放共享服务/数据库<br>✅ 实现快，数据复用简单<br>❌ 推荐流量或训练数据问题可能拖垮播放 | 服务隔离，播放链路只依赖必要 metadata<br>✅ 推荐故障不影响播放<br>❌ 需要同步 title availability 和 metadata 派生数据 | 播放路径专用缓存和降级<br>✅ Catalog/Entitlement 短暂故障时仍可播放已授权内容<br>❌ 缓存一致性和版权撤销更难 | 播放系统和推荐系统强隔离。推荐可以失败降级，播放 session、manifest、DRM、CDN 是更高优先级。 |
| QoE 监控和 CDN 调度 | 只监控服务端错误率<br>✅ 简单<br>❌ 看不到客户端卡顿和 ISP/CDN 问题 | 客户端 QoE 事件<br>✅ 真实反映播放体验<br>❌ 事件量大，有采样、延迟和丢失问题 | QoE 驱动 CDN steering<br>✅ 根据地区、ISP、设备动态选择更好的边缘节点<br>❌ 调度系统复杂，错误调度会放大事故 | 收集客户端 QoE：startup time、rebuffering ratio、average bitrate、error code、CDN node。成熟阶段用 QoE 做 CDN steering 和内容预热。 |
| 转码 pipeline：大任务调度和幂等 | 单 worker 顺序处理整个视频<br>✅ 简单<br>❌ 慢，失败重试成本高 | 按 segment 并行处理<br>✅ 并行度高，失败只重试单个 segment<br>❌ 任务编排、合并、质量检查复杂 | workflow engine 编排<br>✅ 每步状态清晰，可重试、可恢复、可观测<br>❌ 引入 Temporal/Step Functions/Airflow 等组件复杂度 | 用 workflow engine 编排：upload -> split -> transcode segments -> QC -> manifest -> publish。每个任务幂等，输出用 content_version 管理。 |
