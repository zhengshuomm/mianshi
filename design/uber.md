...
| --- | --- | --- | --- | --- |
| Geo index：Quadtree vs Geohash vs S2/H3 | Quadtree<br>✅ 能自适应热点区域<br>❌ 实现复杂 | Geohash<br>✅ 容易按 prefix 分 shard<br>❌ 高纬度区域形变 | S2/H3 cell<br>✅ 层级清晰，邻居计算成熟<br>❌ 引入额外库和概念复杂度 | 简化面试可用 geohash。<br>Staff+ 可以说生产系统更倾向 S2/H3 或动态 quadtree。 |
| Location store：Redis Geo vs PostGIS | Redis Geo 做 hot path<br>✅ `GEOADD/GEOSEARCH` 简单高效<br>❌ 内存成本高 | PostgreSQL + PostGIS 做 hot path<br>✅ 持久化强，查询表达力强<br>❌ 高频 update + nearby query 压力大 | Redis hot path + PostGIS durable path<br>✅ Redis 扛实时查询，PostGIS 扛持久化和恢复<br>❌ 双写/异步同步带来一致性复杂度 | 用 Redis Geo 做实时 nearby query。<br>PostGIS 存 durable last location 和历史轨迹。 |
| Redis Geo 怎么 shard | 单 Redis key 存全城司机<br>✅ 查询简单<br>❌ 热点 key 明显 | 按 geohash prefix/cell 拆 key<br>✅ 写入按 cell 分散<br>❌ 边界查询需要 scatter-gather | 按 city + dynamic hot cell 拆分<br>✅ 能针对热点区域进一步拆分<br>❌ routing 和 rebalancing 复杂 | 使用 `drivers:{region}:{cell_id}` 作为 Redis key。<br>查询时查当前 cell 和相邻 cell。 |
| 位置更新频率：固定频率 vs adaptive interval | 固定频率上报<br>✅ 服务端逻辑简单<br>❌ 静止或低速司机浪费更新 | 客户端 adaptive interval<br>✅ 静止/低速少上报，高速/转向频繁多上报<br>❌ 客户端逻辑更复杂 | 服务端动态下发策略<br>✅ 服务端可根据供需、区域、ride state 调整频率<br>❌ 策略系统复杂 | 用 adaptive client interval。<br>接单中、接近 pickup/dropoff、方向变化大时提高频率。 |
| 司机分配：Redis lock vs DB 状态机 vs durable workflow | Redis distributed lock，TTL 10 秒<br>✅ 快，适合高并发匹配<br>❌ Redis lock 不是最终一致状态 | DB driver state / ride offer 状态机<br>✅ 状态可靠，可审计<br>❌ DB 写压力高 | Durable execution framework，如 Temporal / Step Functions<br>✅ timeout、retry、状态恢复天然支持<br>❌ 引入平台依赖 | Redis lock 做短期 reservation，TTL 约 10 秒。<br>Ride DB 记录最终 offer/ride state。 |
| 司机不响应：同步等待 vs delayed queue | API 同步等待司机响应<br>✅ 流程直观<br>❌ API 长时间占用连接 | Delayed queue timeout<br>✅ 10 秒后自动触发重新匹配<br>❌ 需要处理 accept 和 timeout race | Temporal/Step Functions<br>✅ workflow state 和 timer 持久化<br>❌ 系统复杂度和平台成本更高 | 简单场景用 delayed queue。<br>复杂 ride lifecycle 用 durable workflow。 |
| 高峰期和 Surge：普通 queue vs priority queue | 普通 FIFO queue<br>✅ 简单公平<br>❌ 高价值/紧急请求无法优先 | 按 geo shard 的 priority queue<br>✅ 不同区域隔离<br>❌ 公平性和优先级策略复杂 | Surge pricing + load shedding<br>✅ 通过价格调节供需<br>❌ 用户体验敏感，产品和合规风险 | Matching queue 按 geo shard 隔离。<br>高峰期使用 priority queue + surge。 |
| Geo-sharding：单城市服务 vs 全局 geo shard | 按城市 shard<br>✅ 简单，运营边界清楚<br>❌ 城市内部热点仍然需要细分 | 按 geohash/S2 cell shard<br>✅ 分片更细，扩展性更好<br>❌ shard routing、neighbor lookup、rebalancing... | 城市 + cell 混合<br>✅ 城市级运营简单，热点区域可细分<br>❌ 需要统一 geo routing library | 用城市 + cell 混合。<br>大部分请求只打一个 geo shard。 |
