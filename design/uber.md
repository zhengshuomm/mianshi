...
| --- | --- | --- | --- | --- |
| Geo index：Quadtree vs Geohash vs S2/H3 | Quadtree<br>✅ 能自适应热点区域<br>❌ 实现复杂 | Geohash<br>✅ 容易按 prefix 分 shard<br>❌ 高纬度区域形变 | S2/H3 cell<br>✅ 层级清晰，邻居计算成熟<br>❌ 引入额外库和概念复杂度 | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| Location store：Redis Geo vs PostGIS | Redis Geo 做 hot path<br>✅ `GEOADD/GEOSEARCH` 简单高效<br>❌ 内存成本高 | PostgreSQL + PostGIS 做 hot path<br>✅ 持久化强，查询表达力强<br>❌ 高频 update + nearby query 压力大 | Redis hot path + PostGIS durable path<br>✅ Redis 扛实时查询，PostGIS 扛持久化和恢复<br>❌ 双写/异步同步带来一致性复杂度 | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| Redis Geo 怎么 shard | 单 Redis key 存全城司机<br>✅ 查询简单<br>❌ 热点 key 明显 | 按 geohash prefix/cell 拆 key<br>✅ 写入按 cell 分散<br>❌ 边界查询需要 scatter-gather | 按 city + dynamic hot cell 拆分<br>✅ 能针对热点区域进一步拆分<br>❌ routing 和 rebalancing 复杂 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 位置更新频率：固定频率 vs adaptive interval | 固定频率上报<br>✅ 服务端逻辑简单<br>❌ 静止或低速司机浪费更新 | 客户端 adaptive interval<br>✅ 静止/低速少上报，高速/转向频繁多上报<br>❌ 客户端逻辑更复杂 | 服务端动态下发策略<br>✅ 服务端可根据供需、区域、ride state 调整频率<br>❌ 策略系统复杂 | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| 司机分配：Redis lock vs DB 状态机 vs durable workflow | Redis distributed lock，TTL 10 秒<br>✅ 快，适合高并发匹配<br>❌ Redis lock 不是最终一致状态 | DB driver state / ride offer 状态机<br>✅ 状态可靠，可审计<br>❌ DB 写压力高 | Durable execution framework，如 Temporal / Step Functions<br>✅ timeout、retry、状态恢复天然支持<br>❌ 引入平台依赖 | 我会把长流程建模成显式状态机，而不是一串 best-effort RPC。跨服务流程需要可见的 retry、timeout 和 compensation；当状态和恢复逻辑变复杂时，workflow engine 是合理的演进。 |
...
| 高峰期和 Surge：普通 queue vs priority queue | 普通 FIFO queue<br>✅ 简单公平<br>❌ 高价值/紧急请求无法优先 | 按 geo shard 的 priority queue<br>✅ 不同区域隔离<br>❌ 公平性和优先级策略复杂 | Surge pricing + load shedding<br>✅ 通过价格调节供需<br>❌ 用户体验敏感，产品和合规风险 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
| Geo-sharding：单城市服务 vs 全局 geo shard | 按城市 shard<br>✅ 简单，运营边界清楚<br>❌ 城市内部热点仍然需要细分 | 按 geohash/S2 cell shard<br>✅ 分片更细，扩展性更好<br>❌ shard routing、neighbor lookup、rebalancing... | 城市 + cell 混合<br>✅ 城市级运营简单，热点区域可细分<br>❌ 需要统一 geo routing library | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
