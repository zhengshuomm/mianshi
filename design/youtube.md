...
| --- | --- | --- | --- | --- |
| Codec 选择：H.264 vs H.265/HEVC vs VP9 vs AV1 | H.264<br>✅ 平台支持最好<br>❌ 压缩效率不如 H.265/VP9/AV1 | H.265/HEVC 或 VP9<br>✅ 比 H.264 压缩效率更好<br>❌ HEVC 有授权/兼容性问题 | AV1<br>✅ 压缩效率很好<br>❌ 编码时间长 | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
| Manifest：Primary manifest vs Media manifest | 单一 manifest 列所有 segment<br>✅ 简单<br>❌ 多 resolution/bitrate/codec 时文件大且不清晰 | Primary manifest + media manifest<br>✅ Primary manifest 列出所有 available versions<br>❌ Manifest 生成和版本管理更复杂 | Manifest metadata 完全放 DB，播放时动态生成<br>✅ 灵活，可动态加签、过滤版本<br>❌ Playback Service 压力变大 | 我会对会改变行为的 artifact 做版本化：model、config、manifest、policy、index schema。Versioning 带来 canary、rollback、shadow comparison 和 auditability，比一次性快速发布更重要。 |
| Video processing：整文件转码 vs segment 并行转码 | 整文件转码<br>✅ 实现简单<br>❌ 大文件处理慢 | Split into segments 后并行转码<br>✅ 可以并行处理<br>❌ 需要处理 GOP/keyframe 边界 | 分阶段 pipeline<br>✅ Split、transcode、thumbnail、audio、transcript...<br>❌ 编排复杂，需要任务状态机 | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
| Upload：普通上传 vs resumable multipart upload | 单请求上传到 app server<br>✅ 简单<br>❌ App server 带宽压力大 | S3 Multipart Upload<br>✅ Client 直传 object storage<br>❌ 需要管理 upload session、part etag、complete | 自研 resumable upload protocol<br>✅ 灵活<br>❌ 实现和兼容成本高 | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
| Adaptive bitrate streaming：固定码率 vs ABR | 单一码率播放<br>✅ 存储和处理简单<br>❌ 网络差时卡顿 | HLS/DASH adaptive bitrate streaming<br>✅ Player 根据带宽和 buffer 动态切换清晰度<br>❌ 需要生成多种 rendition 和 manifest | 按设备/网络选择有限 rendition<br>✅ 控制转码和存储成本<br>❌ 播放体验不如完整 ABR ladder | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
| CDN 和热点视频：Origin serving vs CDN cache | App server/origin 直接 serve video<br>✅ 简单<br>❌ 带宽成本和延迟不可接受 | CDN serve manifest 和 segments<br>✅ 就近分发，低延迟<br>❌ CDN miss 会回源 | 多 CDN + pre-warm<br>✅ 降低单 CDN 风险<br>❌ 多 CDN 调度复杂，成本更高 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| Processing failure：同步处理 vs 队列化状态机 | 同步处理完整 pipeline<br>✅ 简单<br>❌ 大文件耗时长 | 队列化任务 + 状态机<br>✅ 每个 stage 可重试<br>❌ 状态管理复杂 | Workflow engine<br>✅ 任务依赖、重试、超时、补偿更清晰<br>❌ 引入额外平台和成本 | 我会把长流程建模成显式状态机，而不是一串 best-effort RPC。跨服务流程需要可见的 retry、timeout 和 compensation；当状态和恢复逻辑变复杂时，workflow engine 是合理的演进。 |
