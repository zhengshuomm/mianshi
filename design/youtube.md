...
| --- | --- | --- | --- | --- |
| Codec 选择：H.264 vs H.265/HEVC vs VP9 vs AV1 | H.264<br>✅ 平台支持最好<br>❌ 压缩效率不如 H.265/VP9/AV1 | H.265/HEVC 或 VP9<br>✅ 比 H.264 压缩效率更好<br>❌ HEVC 有授权/兼容性问题 | AV1<br>✅ 压缩效率很好<br>❌ 编码时间长 | 默认生成 H.264 保兼容。<br>热门/高清视频额外生成 VP9/HEVC/AV1。 |
| Manifest：Primary manifest vs Media manifest | 单一 manifest 列所有 segment<br>✅ 简单<br>❌ 多 resolution/bitrate/codec 时文件大且不清晰 | Primary manifest + media manifest<br>✅ Primary manifest 列出所有 available versions<br>❌ Manifest 生成和版本管理更复杂 | Manifest metadata 完全放 DB，播放时动态生成<br>✅ 灵活，可动态加签、过滤版本<br>❌ Playback Service 压力变大 | 使用 HLS/DASH 两层 manifest。<br>DB 存 rendition/segment metadata，manifest 文件放 object storage/CDN。 |
| Video processing：整文件转码 vs segment 并行转码 | 整文件转码<br>✅ 实现简单<br>❌ 大文件处理慢 | Split into segments 后并行转码<br>✅ 可以并行处理<br>❌ 需要处理 GOP/keyframe 边界 | 分阶段 pipeline<br>✅ Split、transcode、thumbnail、audio、transcript...<br>❌ 编排复杂，需要任务状态机 | 用 segment 并行转码 + staged processing pipeline。<br>每个 segment/rendition task 幂等，可重试。 |
| Upload：普通上传 vs resumable multipart upload | 单请求上传到 app server<br>✅ 简单<br>❌ App server 带宽压力大 | S3 Multipart Upload<br>✅ Client 直传 object storage<br>❌ 需要管理 upload session、part etag、complete | 自研 resumable upload protocol<br>✅ 灵活<br>❌ 实现和兼容成本高 | 用 S3 Multipart Upload。<br>服务端记录 client chunks 的 fingerprint/status，支持 resume。 |
| Adaptive bitrate streaming：固定码率 vs ABR | 单一码率播放<br>✅ 存储和处理简单<br>❌ 网络差时卡顿 | HLS/DASH adaptive bitrate streaming<br>✅ Player 根据带宽和 buffer 动态切换清晰度<br>❌ 需要生成多种 rendition 和 manifest | 按设备/网络选择有限 rendition<br>✅ 控制转码和存储成本<br>❌ 播放体验不如完整 ABR ladder | 使用 HLS/DASH ABR。<br>冷门视频生成少量常用 rendition；热门视频补齐更完整 ladder。 |
| CDN 和热点视频：Origin serving vs CDN cache | App server/origin 直接 serve video<br>✅ 简单<br>❌ 带宽成本和延迟不可接受 | CDN serve manifest 和 segments<br>✅ 就近分发，低延迟<br>❌ CDN miss 会回源 | 多 CDN + pre-warm<br>✅ 降低单 CDN 风险<br>❌ 多 CDN 调度复杂，成本更高 | Manifest 和 segments 都走 CDN。<br>热门视频 pre-warm CDN。 |
| Processing failure：同步处理 vs 队列化状态机 | 同步处理完整 pipeline<br>✅ 简单<br>❌ 大文件耗时长 | 队列化任务 + 状态机<br>✅ 每个 stage 可重试<br>❌ 状态管理复杂 | Workflow engine<br>✅ 任务依赖、重试、超时、补偿更清晰<br>❌ 引入额外平台和成本 | 用 queue + processing state machine。<br>复杂 DAG 可用 workflow engine。 |
