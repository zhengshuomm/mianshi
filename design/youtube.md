...
|---|---|---|---|
| Codec 选择：H.264 vs H.265/HEVC vs VP9 vs AV1 | A: H.264<br>B: H.265/HEVC 或 VP9<br>C: AV1 | A ✅ 平台支持最好。 编码速度快。 播放兼容性强。 ❌ 压缩效率不如 H.265/VP9/AV1。 同等质量下带宽和存储成本更高。<br>B ✅ 比 H.264 压缩效率更好。 高分辨率下带宽收益明显。 ❌ HEVC 有授权/兼容性问题。 VP9 支持也不是所有平台一致。 编码更慢。<br>C ✅ 压缩效率很好。 适合大规模播放摊薄编码成本。 ❌ 编码时间长。 旧设备支持不足。 | 默认生成 H.264 保兼容。<br>热门/高清视频额外生成 VP9/HEVC/AV1。<br>冷门视频不要一开始生成所有 codec，避免转码成本浪费。 |
| Manifest：Primary manifest vs Media manifest | A: 单一 manifest 列所有 segment<br>B: Primary manifest + media manifest<br>C: Manifest metadata 完全放 DB，播放时动态生成 | A ✅ 简单。 ❌ 多 resolution/bitrate/codec 时文件大且不清晰。<br>B ✅ Primary manifest 列出所有 available versions。 Media manifest 列出某个 version 的 segment links。 播放器可以动态切换 bitrate。 ❌ Manifest 生成和版本管理更复杂。<br>C ✅ 灵活，可动态加签、过滤版本。 ❌ Playback Service 压力变大。 CDN cache 效果变差。 | 使用 HLS/DASH 两层 manifest。<br>DB 存 rendition/segment metadata，manifest 文件放 object storage/CDN。<br>私有视频可以在 manifest/segment URL 上加签。 |
| Video processing：整文件转码 vs segment 并行转码 | A: 整文件转码<br>B: Split into segments 后并行转码<br>C: 分阶段 pipeline | A ✅ 实现简单。 不需要处理 segment 拼接一致性。 ❌ 大文件处理慢。 失败后重试成本高。<br>B ✅ 可以并行处理。 单个 segment 失败可局部重试。 更适合 HLS/DASH。 ❌ 需要处理 GOP/keyframe 边界。 Segment 输出一致性和 manifest 生成更复杂。<br>C ✅ Split、transcode、thumbnail、audio、transcript、safety 可独立扩展。 失败和重试更清晰。 ❌ 编排复杂，需要任务状态机。 | 用 segment 并行转码 + staged processing pipeline。<br>每个 segment/rendition task 幂等，可重试。 |
| Upload：普通上传 vs resumable multipart upload | A: 单请求上传到 app server<br>B: S3 Multipart Upload<br>C: 自研 resumable upload protocol | A ✅ 简单。 ❌ App server 带宽压力大。 大文件失败后要重传。<br>B ✅ Client 直传 object storage。 支持分片并行和断点续传。 App server 不承载大流量。 ❌ 需要管理 upload session、part etag、complete。<br>C ✅ 灵活。 ❌ 实现和兼容成本高。 | 用 S3 Multipart Upload。<br>服务端记录 client chunks 的 fingerprint/status，支持 resume。<br>Complete 阶段校验 checksum 后进入 processing。 |
| Adaptive bitrate streaming：固定码率 vs ABR | A: 单一码率播放<br>B: HLS/DASH adaptive bitrate streaming<br>C: 按设备/网络选择有限 rendition | A ✅ 存储和处理简单。 ❌ 网络差时卡顿。 网络好时画质不够。<br>B ✅ Player 根据带宽和 buffer 动态切换清晰度。 降低 rebuffer，提高观看体验。 ❌ 需要生成多种 rendition 和 manifest。 存储和转码成本更高。<br>C ✅ 控制转码和存储成本。 ❌ 播放体验不如完整 ABR ladder。 | 使用 HLS/DASH ABR。<br>冷门视频生成少量常用 rendition；热门视频补齐更完整 ladder。 |
| CDN 和热点视频：Origin serving vs CDN cache | A: App server/origin 直接 serve video<br>B: CDN serve manifest 和 segments<br>C: 多 CDN + pre-warm | A ✅ 简单。 ❌ 带宽成本和延迟不可接受。 热门视频会打爆 origin。<br>B ✅ 就近分发，低延迟。 热点由 CDN 吸收。 App server 不走视频流量。 ❌ CDN miss 会回源。 Cache invalidation 和签名 URL 需要设计。<br>C ✅ 降低单 CDN 风险。 热门内容预热减少首波回源。 ❌ 多 CDN 调度复杂，成本更高。 | Manifest 和 segments 都走 CDN。<br>热门视频 pre-warm CDN。<br>Origin object storage 做保护和限流。 |
| Processing failure：同步处理 vs 队列化状态机 | A: 同步处理完整 pipeline<br>B: 队列化任务 + 状态机<br>C: Workflow engine | A ✅ 简单。 ❌ 大文件耗时长。 失败重试困难。<br>B ✅ 每个 stage 可重试。 失败可定位到具体 rendition/segment。 Worker 可按 codec/resolution 水平扩展。 ❌ 状态管理复杂。<br>C ✅ 任务依赖、重试、超时、补偿更清晰。 ❌ 引入额外平台和成本。 | 用 queue + processing state machine。<br>复杂 DAG 可用 workflow engine。<br>上传 complete 不代表视频 ready，要等必要 rendition 和 manifest 完成。 |
