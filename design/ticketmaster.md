...
| --- | --- | --- | --- | --- |
| 库存模型：assigned seat vs general admission | Assigned seat，每个 seat 一条库存记录<br>✅ 精确控制每个座位<br>❌ 热门 event 下 seat-level contention 高 | General admission，按 ticket type 维护计数<br>✅ 写入简单，只需要扣减 counter<br>❌ 无法支持精确选座 | Section-level inventory<br>✅ 比 seat-level 更容易扩展<br>❌ 最终座位分配可能带来用户体验问题 | 选座 event 用 assigned seat。<br>GA event 用 counter。 |
| 防 oversell：DB lock vs conditional write vs single-writer | DB transaction + row lock<br>✅ 语义清晰，正确性强<br>❌ 热点 seat/section 下锁竞争高 | Conditional write / compare-and-set<br>✅ 只有状态仍是 `AVAILABLE` 才能更新成功<br>❌ 复杂事务和跨多座位 hold 更难 | 按 event/section 单 writer actor<br>✅ 避免并发写冲突<br>❌ 单 partition 吞吐有限 | 基础设计用 DB row lock 或 conditional write。<br>热门 event 演进到按 `event_id/section_id` partition 的 single-writer 或 queue-based processor。 |
| Hold 票策略：短 TTL hold vs 直接支付扣库存 | 短 TTL hold<br>✅ 用户有时间支付<br>❌ 恶意用户可能大量 hold 不支付 | 不 hold，支付成功后再抢库存<br>✅ 库存不被无效占用<br>❌ 用户支付后可能发现没票，体验很差 | 排队后限时 hold + 用户风控<br>✅ 控制并发和恶意占票<br>❌ 产品和风控复杂度更高 | 用短 TTL hold。<br>热门 event 加 waiting room、per-user hold limit、风控校验。 |
| 开售高峰：直接放流量 vs waiting room | 直接让所有用户进购买页<br>✅ 实现简单，用户路径短<br>❌ 热门 event 会造成库存服务雪崩 | Waiting room / virtual queue<br>✅ 平滑流量，保护库存服务<br>❌ 用户等待体验复杂 | Lottery / invite-based sale<br>✅ 极大降低瞬时峰值<br>❌ 不是所有业务都能接受 | 普通 event 直接购买。<br>热门 event 使用 waiting room。 |
| Seat map availability：强实时 vs 缓存近似 | 强实时读 Inventory DB<br>✅ 展示准确<br>❌ 热门 event 下读压力巨大 | 缓存 availability summary<br>✅ 读路径快，保护 Inventory DB<br>❌ 显示可能短暂过期 | 实时推送 seat changes<br>✅ 用户看到的 seat map 更接近实时<br>❌ 对热门 event 推送量大 | 浏览层用缓存/近似 availability。<br>`POST /holds` 时做强校验。 |
| 支付和出票：同步链路 vs 异步补偿 | 支付成功后同步出票和发通知<br>✅ 用户路径直观<br>❌ 出票/通知失败会拖慢甚至破坏支付链路 | 支付成功后发布事件，异步出票和通知<br>✅ 支付主链路短<br>❌ 用户可能短暂看到 `payment succeeded, ticket pendin... | Outbox + worker<br>✅ 避免 DB 成功但 event 丢失<br>❌ 实现更复杂 | 用 payment callback 幂等更新 order。<br>用 outbox/event 异步出票。 |
| 反黄牛和公平性：限流 vs 身份校验 vs 队列策略 | 基础 rate limit<br>✅ 实现简单，成本低<br>❌ 对分布式 bot 效果有限 | Verified account / payment method / phone<br>✅ 提高作弊成本<br>❌ 增加用户摩擦 | Waiting room + bot scoring + per-user quota<br>✅ 同时保护系统和公平性<br>❌ 风控系统复杂 | 普通 event 用 rate limit。<br>热门 event 用 waiting room + verified user + quota。 |
