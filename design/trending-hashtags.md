...
| --- | --- | --- | --- | --- |
| 精确统计 vs 近似统计 | 精确 count<br>✅ 准确，可 debug<br>❌ 高流量 hashtag 写热点明显，unique user 统计成本高 | Count-Min Sketch / approximate heavy hitters<br>✅ 内存低，吞吐高<br>❌ 有误差，不能准确解释每个 tag 的 count | Hybrid<br>✅ 用 approximate 找候选，再对候选精确计数<br>❌ 架构复杂 | I would first clarify fixed versus arbitrary windows, exact versus approximate results, and whether K is fixed. Fixed hot windows can be precomputed into Redis, while arbitrary historical windows belong in OLAP; the expensive part is always expiring old data correctly. |
| Sliding Window 怎么维护 | 每个事件更新所有窗口<br>✅ 查询快<br>❌ 写放大大 | minute buckets + query/worker 聚合<br>✅ 存储清晰，窗口可组合<br>❌ 查询时聚合多 bucket 成本高 | bucket + rolling TopK<br>✅ 实时榜单快，历史也可重建<br>❌ 窗口过期时需要从 score 中移除旧 bucket 的贡献 | I would first clarify fixed versus arbitrary windows, exact versus approximate results, and whether K is fixed. Fixed hot windows can be precomputed into Redis, while arbitrary historical windows belong in OLAP; the expensive part is always expiring old data correctly. |
| TopK 数据结构 | Redis Sorted Set<br>✅ 实现简单，TopN 查询快<br>❌ 更新频繁时内存和写压力大 | Flink keyed state + heap<br>✅ 实时计算强，窗口语义好<br>❌ 服务在线 query 还要 sink 到 serving store | Druid/ClickHouse<br>✅ 支持 ad-hoc group by 和历史窗口<br>❌ 实时首页查询延迟和成本可能高于 Redis serving view | I would first clarify whether the query requires fixed windows, arbitrary windows, exact counts, or approximate trends. Fixed hot windows can be precomputed; arbitrary historical windows belong in OLAP, and late or duplicate events require event-time processing plus reconciliation. |
| Hot Hashtag / Hot Partition | 按 hashtag 分区<br>✅ 同 hashtag 聚合简单<br>❌ 超级热点 hashtag 会打爆单 partition | 局部聚合后全局合并<br>✅ 先在多个 partition 做 partial count，再汇总<br>❌ 需要二阶段聚合，延迟增加 | hashtag + random shard<br>✅ 热点 tag 被拆散<br>❌ 读取/合并时要聚合多个 shard | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| Trending Score：count vs velocity vs acceleration | 纯 count<br>✅ 简单可解释<br>❌ 长期热门话题一直霸榜，不代表正在 trending | 增长率 / velocity<br>✅ 更能发现新趋势<br>❌ 小基数 tag 容易因为少量增长冲上榜 | 综合 score<br>✅ 平衡热度、新鲜度、增长和质量<br>❌ 解释和调参复杂 | I would first clarify whether the query requires fixed windows, arbitrary windows, exact counts, or approximate trends. Fixed hot windows can be precomputed; arbitrary historical windows belong in OLAP, and late or duplicate events require event-time processing plus reconciliation. |
| 去重和反作弊 | 只按 post count<br>✅ 简单<br>❌ bot 和刷屏极易操纵榜单 | unique user count<br>✅ 降低单用户刷量<br>❌ 需要近似或精确 UV 统计 | trust-weighted count<br>✅ 账号信誉、内容质量、互动质量共同影响趋势<br>❌ 模型复杂，可能有偏差 | I would avoid promising end-to-end exactly-once. The stronger Staff-level framing is at-least-once delivery with deterministic idempotency keys, idempotent sinks, and replayable raw events; that gives us failure recovery without pretending the broker solves external side effects. |
| Region / Language / Personalization | 全球一个榜<br>✅ 简单<br>❌ 不同地区语言差异巨大，用户体验差 | 按 region/lang 维护榜单<br>✅ 结果更相关<br>❌ 榜单数量增加，存储和计算放大 | 个性化 trending<br>✅ 更贴近用户兴趣<br>❌ 不能为每个用户实时计算完整 TopK | I would avoid global strong consistency unless it protects a concrete invariant. A home-region or regional-shard model keeps the write path understandable; cross-region replication can be asynchronous with conflict rules and reconciliation for derived views. |
| Reconciliation 和可解释性 | 只信实时流<br>✅ 架构简单<br>❌ late events、重复事件、反作弊延迟都会影响榜单 | 离线重算<br>✅ 准确，可审计<br>❌ 延迟高 | 实时 + 离线校准<br>✅ 实时体验和最终准确性兼顾<br>❌ 同一窗口排名可能被修正 | I would keep a durable raw event log for replay, reconciliation, and audit. For money, inventory, billing, and analytics, realtime results are provisional until an offline or independent reconciliation path verifies and corrects them. |
