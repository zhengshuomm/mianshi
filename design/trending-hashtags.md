...
| --- | --- | --- | --- | --- |
| 精确统计 vs 近似统计 | 精确 count<br>✅ 准确，可 debug<br>❌ 高流量 hashtag 写热点明显，unique user 统计成本高 | Count-Min Sketch / approximate heavy hitters<br>✅ 内存低，吞吐高<br>❌ 有误差，不能准确解释每个 tag 的 count | Hybrid<br>✅ 用 approximate 找候选，再对候选精确计数<br>❌ 架构复杂 | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| Sliding Window 怎么维护 | 每个事件更新所有窗口<br>✅ 查询快<br>❌ 写放大大 | minute buckets + query/worker 聚合<br>✅ 存储清晰，窗口可组合<br>❌ 查询时聚合多 bucket 成本高 | bucket + rolling TopK<br>✅ 实时榜单快，历史也可重建<br>❌ 窗口过期时需要从 score 中移除旧 bucket 的贡献 | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| TopK 数据结构 | Redis Sorted Set<br>✅ 实现简单，TopN 查询快<br>❌ 更新频繁时内存和写压力大 | Flink keyed state + heap<br>✅ 实时计算强，窗口语义好<br>❌ 服务在线 query 还要 sink 到 serving store | Druid/ClickHouse<br>✅ 支持 ad-hoc group by 和历史窗口<br>❌ 实时首页查询延迟和成本可能高于 Redis serving view | 我会先问清固定窗口还是任意窗口、精确统计还是近似趋势。固定热窗口可以预计算；任意历史窗口应该走 OLAP；late/duplicate event 需要 event-time processing 和 reconciliation。 |
| Hot Hashtag / Hot Partition | 按 hashtag 分区<br>✅ 同 hashtag 聚合简单<br>❌ 超级热点 hashtag 会打爆单 partition | 局部聚合后全局合并<br>✅ 先在多个 partition 做 partial count，再汇总<br>❌ 需要二阶段聚合，延迟增加 | hashtag + random shard<br>✅ 热点 tag 被拆散<br>❌ 读取/合并时要聚合多个 shard | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Trending Score：count vs velocity vs acceleration | 纯 count<br>✅ 简单可解释<br>❌ 长期热门话题一直霸榜，不代表正在 trending | 增长率 / velocity<br>✅ 更能发现新趋势<br>❌ 小基数 tag 容易因为少量增长冲上榜 | 综合 score<br>✅ 平衡热度、新鲜度、增长和质量<br>❌ 解释和调参复杂 | 我会先问清固定窗口还是任意窗口、精确统计还是近似趋势。固定热窗口可以预计算；任意历史窗口应该走 OLAP；late/duplicate event 需要 event-time processing 和 reconciliation。 |
| 去重和反作弊 | 只按 post count<br>✅ 简单<br>❌ bot 和刷屏极易操纵榜单 | unique user count<br>✅ 降低单用户刷量<br>❌ 需要近似或精确 UV 统计 | trust-weighted count<br>✅ 账号信誉、内容质量、互动质量共同影响趋势<br>❌ 模型复杂，可能有偏差 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| Region / Language / Personalization | 全球一个榜<br>✅ 简单<br>❌ 不同地区语言差异巨大，用户体验差 | 按 region/lang 维护榜单<br>✅ 结果更相关<br>❌ 榜单数量增加，存储和计算放大 | 个性化 trending<br>✅ 更贴近用户兴趣<br>❌ 不能为每个用户实时计算完整 TopK | 除非保护明确的业务 invariant，否则我会避免 global strong consistency。home region 或 regional shard 能让写路径更清楚；跨区复制可以异步，并为冲突规则和 reconciliation 留出设计空间。 |
| Reconciliation 和可解释性 | 只信实时流<br>✅ 架构简单<br>❌ late events、重复事件、反作弊延迟都会影响榜单 | 离线重算<br>✅ 准确，可审计<br>❌ 延迟高 | 实时 + 离线校准<br>✅ 实时体验和最终准确性兼顾<br>❌ 同一窗口排名可能被修正 | 我会保留 durable raw event log，用于 replay、reconciliation 和 audit。钱、库存、计费、analytics 的实时结果都应该被视为 provisional，最终由离线或独立对账链路校正。 |
