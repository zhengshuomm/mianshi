...
|---|---|---|---|
| Head-based Sampling vs Tail-based Sampling | A: Head-based sampling<br>B: Tail-based sampling<br>C: Hybrid | A ✅ 实现简单；下游只看 sampling flag；成本可控。 ❌ 入口时不知道请求是否会变慢或失败，可能漏掉关键 trace。<br>B ✅ 可以基于完整 trace 结果决定保留，排障价值高。 ❌ processor 要暂存 spans，内存/延迟/复杂度更高。<br>C ✅ head sampling 控制基础成本，tail sampling 强保留高价值 traces。 ❌ 采样规则和解释更复杂。 | 默认 head-based sampling。<br>对 errors、slow traces、VIP tenants 做 tail-based 或 always sample。<br>采样决策必须记录在 trace summary 里，避免分析误解。 |
| Context Propagation：同步调用 vs 异步消息 | A: 只支持 HTTP/gRPC headers<br>B: 支持 message headers<br>C: Trace links | A ✅ 实现直观。 ❌ 消息队列、异步任务、cron job 链路会断。<br>B ✅ 生产系统常见异步链路可串起来。 ❌ fanout/fanin 关系复杂，trace tree 可能不再是简单树。<br>C ✅ 可表达一个 span 关联多个 parent traces。 ❌ UI 和查询模型更复杂。 | 同步调用传播 `traceparent`。<br>MQ headers 传播 trace context。<br>fanout/fanin 用 span links，避免强行伪造成单父节点。 |
| 存储模型：Raw Spans vs Trace Summary | A: 只存 raw spans<br>B: 只存 summary<br>C: Raw spans + summary index | A ✅ 数据完整。 ❌ 搜索慢，按 error/latency/service 查询要扫大量数据。<br>B ✅ 成本低，查询快。 ❌ 无法做单 trace 排障。<br>C ✅ trace_id 查详情快，按条件搜索也快。 ❌ 双写 raw store 和 index，有一致性和成本问题。 | Raw span store 是详情源。<br>Trace summary/search index 是 derived view。<br>index 丢了可从 raw spans replay 重建。 |
| Tag Cardinality 和 PII | A: 所有 tags 都可索引<br>B: 限制可索引 tags<br>C: 高 cardinality tags 只存 raw，不建索引 | A ✅ 查询灵活。 ❌ `user_id/request_id/order_id` 会让索引爆炸，成本不可控。<br>B ✅ 控制 index cardinality 和查询成本。 ❌ 某些 ad-hoc debug 查询不方便。<br>C ✅ 保留 debug 信息，同时保护索引。 ❌ 无法快速按这些字段搜索，需依赖 logs 或额外索引。 | 索引低 cardinality tags：service、operation、status、region、endpoint_template。<br>user_id、email、token、raw URL、request payload 不进索引，PII 需脱敏。<br>与 logs 关联用 trace_id，而不是把所有业务字段塞进 span tags。 |
| Ingestion Reliability：同步上报 vs Agent Buffer vs Queue | A: 业务请求同步上报 backend<br>B: SDK -> local agent buffer<br>C: Agent -> Kafka durable queue | A ✅ 实现简单。 ❌ tracing backend 故障会拖慢业务请求。<br>B ✅ 业务请求只写本地 buffer；agent batch/压缩/重试。 ❌ 机器 crash 时 buffer 可能丢。<br>C ✅ 后端存储不可用时可缓冲；processor 可重放。 ❌ 增加链路延迟和运维复杂度。 | SDK 异步发 agent。<br>Agent 发 collector，collector 写 Kafka。<br>对低优先级 spans 可丢，对 error/slow traces 尽量保留。 |
| Query Path：trace_id 查详情 vs 条件搜索 | A: 所有查询都走同一个数据库<br>B: Raw store + search index<br>C: Columnar store 统一承载 | A ✅ 架构简单。 ❌ trace_id point lookup 和 tag/time range search 的访问模式差异很大。<br>B ✅ trace_id 详情和条件搜索各自优化。 ❌ index 可能落后 raw store，查询结果短暂不一致。<br>C ✅ 压缩好，聚合强，能同时查明细和聚合。 ❌ 全文/任意 tag 搜索体验可能弱于专门搜索引擎。 | trace_id 查 raw store。<br>条件搜索查 index，再回 raw store 拉完整 trace。<br>UI 要展示数据 freshness，避免用户误以为没有 trace。 |
| Service Graph 和 Metrics 派生 | A: 只展示单 trace<br>B: 从 spans 派生 RED metrics<br>C: Service dependency graph | A ✅ 排查单请求有效。 ❌ 无法看服务整体依赖和趋势。<br>B ✅ 自动得到 request rate、error rate、duration。 ❌ 采样会影响 metrics 准确性，不能完全替代 metrics pipeline。<br>C ✅ 发现下游依赖、慢调用、错误传播路径。 ❌ 异步调用和采样会让图不完整。 | Traces 派生 service graph 和近似 RED metrics。<br>核心 SLO 仍以 metrics 系统为准。<br>Trace-derived metrics 要标记 sampling rate。 |
| Multi-tenant 和成本控制 | A: 所有服务共享同一采样率和 retention<br>B: 按 service/tenant 配采样和 retention<br>C: 动态采样 | A ✅ 简单。 ❌ 高 QPS 服务会吃掉存储预算，低 QPS 关键服务反而数据不足。<br>B ✅ 成本可控，关键业务优先。 ❌ 配置治理复杂。<br>C ✅ 错误升高时提高采样，正常时降低成本。 ❌ 实现复杂，分析要考虑动态采样偏差。 | 按 tenant/service 设置 ingestion quota、sampling rate、retention。<br>error/slow traces override sampling。<br>对超额 tenant 做降采样而不是拖垮全局系统。 |
