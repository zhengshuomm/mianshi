...
| --- | --- | --- | --- | --- |
| Head-based Sampling vs Tail-based Sampling | Head-based sampling<br>✅ 实现简单<br>❌ 入口时不知道请求是否会变慢或失败，可能漏掉关键 trace | Tail-based sampling<br>✅ 可以基于完整 trace 结果决定保留，排障价值高<br>❌ processor 要暂存 spans，内存/延迟/复杂度更高 | Hybrid<br>✅ head sampling 控制基础成本，tail sampling 强保留高价值...<br>❌ 采样规则和解释更复杂 | 默认 head-based sampling。<br>对 errors、slow traces、VIP tenants 做 tail-based 或 always sample。 |
| Context Propagation：同步调用 vs 异步消息 | 只支持 HTTP/gRPC headers<br>✅ 实现直观<br>❌ 消息队列、异步任务、cron job 链路会断 | 支持 message headers<br>✅ 生产系统常见异步链路可串起来<br>❌ fanout/fanin 关系复杂，trace tree 可能不再是简单树 | Trace links<br>✅ 可表达一个 span 关联多个 parent traces<br>❌ UI 和查询模型更复杂 | 同步调用传播 `traceparent`。<br>MQ headers 传播 trace context。 |
| 存储模型：Raw Spans vs Trace Summary | 只存 raw spans<br>✅ 数据完整<br>❌ 搜索慢，按 error/latency/service 查询要扫大量数据 | 只存 summary<br>✅ 成本低，查询快<br>❌ 无法做单 trace 排障 | Raw spans + summary index<br>✅ trace_id 查详情快，按条件搜索也快<br>❌ 双写 raw store 和 index，有一致性和成本问题 | Raw span store 是详情源。<br>Trace summary/search index 是 derived view。 |
| Tag Cardinality 和 PII | 所有 tags 都可索引<br>✅ 查询灵活<br>❌ `user_id/request_id/order_id` 会让索引爆炸，成本不可控 | 限制可索引 tags<br>✅ 控制 index cardinality 和查询成本<br>❌ 某些 ad-hoc debug 查询不方便 | 高 cardinality tags 只存 raw，不建索引<br>✅ 保留 debug 信息，同时保护索引<br>❌ 无法快速按这些字段搜索，需依赖 logs 或额外索引 | 索引低 cardinality tags：service、operation、status、region、endpoint_template。<br>user_id、email、token、raw URL、request payload 不进索引，PII 需脱敏。 |
| Ingestion Reliability：同步上报 vs Agent Buffer vs Queue | 业务请求同步上报 backend<br>✅ 实现简单<br>❌ tracing backend 故障会拖慢业务请求 | SDK -> local agent buffer<br>✅ 业务请求只写本地 buffer<br>❌ 机器 crash 时 buffer 可能丢 | Agent -> Kafka durable queue<br>✅ 后端存储不可用时可缓冲<br>❌ 增加链路延迟和运维复杂度 | SDK 异步发 agent。<br>Agent 发 collector，collector 写 Kafka。 |
| Query Path：trace_id 查详情 vs 条件搜索 | 所有查询都走同一个数据库<br>✅ 架构简单<br>❌ trace_id point lookup 和 tag/time range sea... | Raw store + search index<br>✅ trace_id 详情和条件搜索各自优化<br>❌ index 可能落后 raw store，查询结果短暂不一致 | Columnar store 统一承载<br>✅ 压缩好，聚合强，能同时查明细和聚合<br>❌ 全文/任意 tag 搜索体验可能弱于专门搜索引擎 | trace_id 查 raw store。<br>条件搜索查 index，再回 raw store 拉完整 trace。 |
| Service Graph 和 Metrics 派生 | 只展示单 trace<br>✅ 排查单请求有效<br>❌ 无法看服务整体依赖和趋势 | 从 spans 派生 RED metrics<br>✅ 自动得到 request rate、error rate、duration<br>❌ 采样会影响 metrics 准确性，不能完全替代 metrics pipeline | Service dependency graph<br>✅ 发现下游依赖、慢调用、错误传播路径<br>❌ 异步调用和采样会让图不完整 | Traces 派生 service graph 和近似 RED metrics。<br>核心 SLO 仍以 metrics 系统为准。 |
| Multi-tenant 和成本控制 | 所有服务共享同一采样率和 retention<br>✅ 简单<br>❌ 高 QPS 服务会吃掉存储预算，低 QPS 关键服务反而数据不足 | 按 service/tenant 配采样和 retention<br>✅ 成本可控，关键业务优先<br>❌ 配置治理复杂 | 动态采样<br>✅ 错误升高时提高采样，正常时降低成本<br>❌ 实现复杂，分析要考虑动态采样偏差 | 按 tenant/service 设置 ingestion quota、sampling rate、retention。<br>error/slow traces override sampling。 |
