...
| --- | --- | --- | --- | --- |
| 发消息协议：WebSocket send vs HTTP POST | WebSocket 发送和接收都走同一条连接<br>✅ 延迟低<br>❌ 横向扩展和 retry 语义更复杂 | HTTP POST 发送，WebSocket 接收<br>✅ HTTP retry、LB、auth、observability 成熟<br>❌ 比纯 WebSocket 多一点请求开销 | 客户端同时支持两者<br>✅ WebSocket 可用时低延迟，不可用时 HTTP fallback<br>❌ 客户端和服务端逻辑复杂 | 即使用 WebSocket 做低延迟通道，持久化消息创建也要有清晰、幂等的写入语义。send success、persisted、delivered 是三个不同状态，重试和补齐逻辑也应该分开设计。 |
| Pub/Sub：Redis PubSub vs Redis Streams vs Kafka | Redis Pub/Sub<br>✅ 非常轻量，延迟低<br>❌ At-most-once | Redis Streams<br>✅ 支持持久化、consumer group、ack<br>❌ 大规模跨 region replay、生态和吞吐不如 Kafka | Kafka<br>✅ 持久化、高吞吐、可回放<br>❌ 订阅每个用户一个 topic 不现实 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Delivery guarantee：只 push vs inbox | 只通过 WebSocket push<br>✅ 延迟低，系统简单<br>❌ Client disconnected 或 server failure 时会丢 | 维护 inbox / delivery store<br>✅ 每个接收者有 pending message entry<br>❌ 写放大，尤其群聊成员多时 | 只用 message store + client cursor<br>✅ 写路径轻<br>❌ 很难精确知道每条消息是否送达设备 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| DB write 和 Pub/Sub 一致性：2PC vs Saga vs Outbox/CDC | Two-phase commit<br>✅ 强一致语义<br>❌ 复杂、慢、可用性差 | Saga<br>✅ 适合长事务<br>❌ 对“DB 写 + 发事件”这个场景有点重 | Transactional outbox / CDC<br>✅ DB 写 message 和 outbox 在同一事务<br>❌ 事件有延迟 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| Message ordering：全局顺序 vs thread 顺序 vs sender/receiver counter | 全局顺序<br>✅ 顺序模型统一<br>❌ 全局 sequencer 是瓶颈 | 每个 thread 一个递增 sequence<br>✅ 用户看到的对话内顺序稳定<br>❌ 跨 thread 没顺序 | 发送方/接收方各维护 counter<br>✅ 可帮助解决多设备同步和去重<br>❌ 与 thread global order 合并时复杂 | 除非产品明确要求，否则我不会引入全局顺序。通常 per-entity ordering 就足够，例如 per channel、thread、auction、order；这样既保护用户可见顺序，又能按实体分区扩展和恢复。 |
| Service discovery：ZooKeeper consistent hashing vs LB | ZooKeeper 管 chat server membership + consistent hash ring<br>✅ Membership 一致<br>❌ ZooKeeper 不适合高频负载更新 | Redis / KV 存 connection routing<br>✅ 查询快，写入简单<br>❌ 需要处理 KV 故障和过期误差 | Envoy/Nginx LB + routing service<br>✅ LB 处理连接分发<br>❌ 需要额外 routing lookup | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Multi-region：同步复制 vs 异步复制 | 全球同步写<br>✅ 跨 region 数据一致性强<br>❌ 延迟高，影响 `<200ms` | Home region + 异步复制<br>✅ 写入延迟低<br>❌ 跨 region 消息和 presence 有延迟 | Thread region / conversation owner region<br>✅ 一个 thread 固定写入一个 region，顺序清晰<br>❌ 远端用户发消息要跨 region | 除非保护明确的业务 invariant，否则我会避免 global strong consistency。home region 或 regional shard 能让写路径更清楚；跨区复制可以异步，并为冲突规则和 reconciliation 留出设计空间。 |
| Unread count：存 counter vs 实时计算 | 每个 user_thread 存 unread_count<br>✅ 读快<br>❌ 每条消息都要更新接收者 counter | 用 latest_seq - last_read_seq 实时计算<br>✅ 写路径轻<br>❌ 删除、mute、mention-only 会让逻辑变复杂 | 混合<br>✅ 小会话精确，大群可扩展<br>❌ 实现复杂，产品语义要清晰 | Unread count 是 UX 派生计数，不是消息持久化 invariant。小会话可以精确计数，大 channel 应该用 seq 或最终一致 counter，保证 unread 更新不会阻塞消息写入。 |
| Media attachment：经过 chat server vs direct upload | 通过 chat server 上传<br>✅ 权限校验直接<br>❌ Chat server 带宽压力大 | Object storage pre-signed URL<br>✅ Client 直传 S3/Object Store<br>❌ 需要 upload session、扫描、状态回调 | 异步处理 media<br>✅ 发送消息不等待重处理<br>❌ 用户可能先看到 processing 状态 | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
