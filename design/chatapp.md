...
|---|---|---|---|
| 发消息协议：WebSocket send vs HTTP POST | A: WebSocket 发送和接收都走同一条连接<br>B: HTTP POST 发送，WebSocket 接收<br>C: 客户端同时支持两者 | A ✅ 延迟低。 双向通信自然。 ❌ 横向扩展和 retry 语义更复杂。 连接断开时发送失败，需要自定义重试和幂等。<br>B ✅ HTTP retry、LB、auth、observability 成熟。 写路径更容易做幂等和限流。 WebSocket 专注实时下发。 ❌ 比纯 WebSocket 多一点请求开销。<br>C ✅ WebSocket 可用时低延迟，不可用时 HTTP fallback。 ❌ 客户端和服务端逻辑复杂。 | 用 HTTP POST 发消息，WebSocket 收消息。<br>如果面试官强调 `<200ms` 极致低延迟，可以补充 WebSocket send + HTTP fallback。 |
| Pub/Sub：Redis PubSub vs Redis Streams vs Kafka | A: Redis Pub/Sub<br>B: Redis Streams<br>C: Kafka | A ✅ 非常轻量，延迟低。 实现简单。 ❌ At-most-once。 没有订阅者时消息丢失。 不适合 delivery guarantee。<br>B ✅ 支持持久化、consumer group、ack。 类似轻量 Kafka。 ❌ 大规模跨 region replay、生态和吞吐不如 Kafka。<br>C ✅ 持久化、高吞吐、可回放。 适合 CDC/outbox 后的 fanout pipeline。 ❌ 订阅每个用户一个 topic 不现实。 Kafka subscription 不适合作为在线连接路由。 | 不要用 Kafka 做 per-user subscription。<br>Message event 用 Kafka/Redis Streams。<br>在线 connection routing 用 Redis/routing service。<br>可靠性靠 DB + inbox，不靠 Pub/Sub 本身。 |
| Delivery guarantee：只 push vs inbox | A: 只通过 WebSocket push<br>B: 维护 inbox / delivery store<br>C: 只用 message store + client cursor | A ✅ 延迟低，系统简单。 ❌ Client disconnected 或 server failure 时会丢。<br>B ✅ 每个接收者有 pending message entry。 Client ack 后删除或标记 delivered。 离线用户上线后可补齐。 ❌ 写放大，尤其群聊成员多时。 Inbox 清理和状态维护更复杂。<br>C ✅ 写路径轻。 通过 `last_seen_seq` 补齐。 ❌ 很难精确知道每条消息是否送达设备。 | 1:1 / 小群用 inbox 保证 delivery。<br>大群可用 message store + cursor，避免巨大写放大。<br>Client ack 是删除 inbox entry 或更新 delivery state 的关键。 |
| DB write 和 Pub/Sub 一致性：2PC vs Saga vs Outbox/CDC | A: Two-phase commit<br>B: Saga<br>C: Transactional outbox / CDC | A ✅ 强一致语义。 ❌ 复杂、慢、可用性差。 不适合高吞吐聊天主链路。<br>B ✅ 适合长事务。 ❌ 对“DB 写 + 发事件”这个场景有点重。<br>C ✅ DB 写 message 和 outbox 在同一事务。 CDC/Event Relay 异步发布事件。 避免 DB 成功但事件丢失。 ❌ 事件有延迟。 需要 outbox relay、幂等 consumer。 | 用 DB-first + outbox/CDC。<br>Fanout、notification、push 都从 CDC event 触发。<br>这是保证消息可靠和系统可扩展的关键。 |
| Message ordering：全局顺序 vs thread 顺序 vs sender/receiver counter | A: 全局顺序<br>B: 每个 thread 一个递增 sequence<br>C: 发送方/接收方各维护 counter | A ✅ 顺序模型统一。 ❌ 全局 sequencer 是瓶颈。 对 chat 没必要。<br>B ✅ 用户看到的对话内顺序稳定。 可按 `thread_id` 分区。 ❌ 跨 thread 没顺序。 热门大群 thread 会成为热点。<br>C ✅ 可帮助解决多设备同步和去重。 对每个用户 inbox 排序友好。 ❌ 与 thread global order 合并时复杂。 | 对话内用 `thread_id + message_seq`。<br>用户 inbox / device sync 可维护 per-user receive cursor。<br>不做全局顺序。 |
| Service discovery：ZooKeeper consistent hashing vs LB | A: ZooKeeper 管 chat server membership + consistent hash ring<br>B: Redis / KV 存 connection routing<br>C: Envoy/Nginx LB + routing service | A ✅ Membership 一致。 Server 增减时迁移范围可控。 ❌ ZooKeeper 不适合高频负载更新。 不应把每个用户连接状态都频繁写 ZK。<br>B ✅ 查询快，写入简单。 TTL 可清理断开连接。 ❌ 需要处理 KV 故障和过期误差。<br>C ✅ LB 处理连接分发。 Routing service 处理 user -> connection。 ❌ 需要额外 routing lookup。 | ZooKeeper 只管理服务 membership / consistent hash ring。<br>Redis/KV 管高频 connection routing。<br>负载监控用 Prometheus/Consul/Eureka，不要用 ZooKeeper 高频写负载指标。 |
| Multi-region：同步复制 vs 异步复制 | A: 全球同步写<br>B: Home region + 异步复制<br>C: Thread region / conversation owner region | A ✅ 跨 region 数据一致性强。 ❌ 延迟高，影响 `<200ms`。 可用性和复杂度差。<br>B ✅ 写入延迟低。 架构可扩展。 Kafka MirrorMaker 可异步同步 topic。 ❌ 跨 region 消息和 presence 有延迟。 冲突处理和顺序要定义清楚。<br>C ✅ 一个 thread 固定写入一个 region，顺序清晰。 ❌ 远端用户发消息要跨 region。 | 用 home region 或 thread owner region。<br>跨 DC 数据异步复制。<br>Global Presence Service 告诉本 region 用户在哪，查询时顺便同步 presence cache。 |
| Unread count：存 counter vs 实时计算 | A: 每个 user_thread 存 unread_count<br>B: 用 latest_seq - last_read_seq 实时计算<br>C: 混合 | A ✅ 读快。 展示会话列表简单。 ❌ 每条消息都要更新接收者 counter。 群聊写放大明显。<br>B ✅ 写路径轻。 ❌ 删除、mute、mention-only 会让逻辑变复杂。<br>C ✅ 小会话精确，大群可扩展。 ❌ 实现复杂，产品语义要清晰。 | 1:1、小群存 unread counter。<br>大群用 last_read_seq 或只维护 mention count。<br>Unread 不阻塞消息主链路。 |
| Media attachment：经过 chat server vs direct upload | A: 通过 chat server 上传<br>B: Object storage pre-signed URL<br>C: 异步处理 media | A ✅ 权限校验直接。 ❌ Chat server 带宽压力大。 大文件会影响消息服务。<br>B ✅ Client 直传 S3/Object Store。 App server 不承载大流量。 ❌ 需要 upload session、扫描、状态回调。<br>C ✅ 发送消息不等待重处理。 ❌ 用户可能先看到 processing 状态。 | 附件用 pre-signed URL 直传。<br>消息里引用 `attachment_id`。<br>Media scan/thumbnail/transcode 异步完成。 |
