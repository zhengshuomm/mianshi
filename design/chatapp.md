...
| --- | --- | --- | --- | --- |
| 发消息协议：WebSocket send vs HTTP POST | WebSocket 发送和接收都走同一条连接<br>✅ 延迟低<br>❌ 横向扩展和 retry 语义更复杂 | HTTP POST 发送，WebSocket 接收<br>✅ HTTP retry、LB、auth、observability 成熟<br>❌ 比纯 WebSocket 多一点请求开销 | 客户端同时支持两者<br>✅ WebSocket 可用时低延迟，不可用时 HTTP fallback<br>❌ 客户端和服务端逻辑复杂 | I would keep durable message creation on an idempotent HTTP or clearly acknowledged write path, even if WebSocket is used for low-latency delivery. The key is that send success, persistence, and delivery are separate states with separate retry behavior. |
| Pub/Sub：Redis PubSub vs Redis Streams vs Kafka | Redis Pub/Sub<br>✅ 非常轻量，延迟低<br>❌ At-most-once | Redis Streams<br>✅ 支持持久化、consumer group、ack<br>❌ 大规模跨 region replay、生态和吞吐不如 Kafka | Kafka<br>✅ 持久化、高吞吐、可回放<br>❌ 订阅每个用户一个 topic 不现实 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Delivery guarantee：只 push vs inbox | 只通过 WebSocket push<br>✅ 延迟低，系统简单<br>❌ Client disconnected 或 server failure 时会丢 | 维护 inbox / delivery store<br>✅ 每个接收者有 pending message entry<br>❌ 写放大，尤其群聊成员多时 | 只用 message store + client cursor<br>✅ 写路径轻<br>❌ 很难精确知道每条消息是否送达设备 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| DB write 和 Pub/Sub 一致性：2PC vs Saga vs Outbox/CDC | Two-phase commit<br>✅ 强一致语义<br>❌ 复杂、慢、可用性差 | Saga<br>✅ 适合长事务<br>❌ 对“DB 写 + 发事件”这个场景有点重 | Transactional outbox / CDC<br>✅ DB 写 message 和 outbox 在同一事务<br>❌ 事件有延迟 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| Message ordering：全局顺序 vs thread 顺序 vs sender/receiver counter | 全局顺序<br>✅ 顺序模型统一<br>❌ 全局 sequencer 是瓶颈 | 每个 thread 一个递增 sequence<br>✅ 用户看到的对话内顺序稳定<br>❌ 跨 thread 没顺序 | 发送方/接收方各维护 counter<br>✅ 可帮助解决多设备同步和去重<br>❌ 与 thread global order 合并时复杂 | I would not introduce global ordering unless the product explicitly requires it. Per-entity ordering, such as per channel, thread, auction, or order, usually preserves the user-visible invariant and lets us partition and recover independently. |
| Service discovery：ZooKeeper consistent hashing vs LB | ZooKeeper 管 chat server membership + consistent hash ring<br>✅ Membership 一致<br>❌ ZooKeeper 不适合高频负载更新 | Redis / KV 存 connection routing<br>✅ 查询快，写入简单<br>❌ 需要处理 KV 故障和过期误差 | Envoy/Nginx LB + routing service<br>✅ LB 处理连接分发<br>❌ 需要额外 routing lookup | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Multi-region：同步复制 vs 异步复制 | 全球同步写<br>✅ 跨 region 数据一致性强<br>❌ 延迟高，影响 `<200ms` | Home region + 异步复制<br>✅ 写入延迟低<br>❌ 跨 region 消息和 presence 有延迟 | Thread region / conversation owner region<br>✅ 一个 thread 固定写入一个 region，顺序清晰<br>❌ 远端用户发消息要跨 region | I would avoid global strong consistency unless it protects a concrete invariant. A home-region or regional-shard model keeps the write path understandable; cross-region replication can be asynchronous with conflict rules and reconciliation for derived views. |
| Unread count：存 counter vs 实时计算 | 每个 user_thread 存 unread_count<br>✅ 读快<br>❌ 每条消息都要更新接收者 counter | 用 latest_seq - last_read_seq 实时计算<br>✅ 写路径轻<br>❌ 删除、mute、mention-only 会让逻辑变复杂 | 混合<br>✅ 小会话精确，大群可扩展<br>❌ 实现复杂，产品语义要清晰 | I would treat unread count as a derived UX counter, not part of the message durability invariant. Small conversations can maintain precise counters, while large channels should use sequence-based or eventually consistent counters so unread updates never block message writes. |
| Media attachment：经过 chat server vs direct upload | 通过 chat server 上传<br>✅ 权限校验直接<br>❌ Chat server 带宽压力大 | Object storage pre-signed URL<br>✅ Client 直传 S3/Object Store<br>❌ 需要 upload session、扫描、状态回调 | 异步处理 media<br>✅ 发送消息不等待重处理<br>❌ 用户可能先看到 processing 状态 | I would keep the control plane and data plane separate. Metadata, authorization, and manifest generation protect correctness, while CDN/object storage carries bulk traffic; heavy processing such as transcoding should be an idempotent workflow. |
