...
| --- | --- | --- | --- | --- |
| Server-side Redirect vs Client-side Tracking | Client-side tracking pixel / JS 上报<br>✅ 实现简单，对用户跳转路径影响小<br>❌ 用户或浏览器可以绕过 | Server-side redirect<br>✅ 用户必须先经过 tracking endpoint，才能跳到 advertiser...<br>❌ 增加一次网络跳转 | - | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 去重：Redis SETNX vs DB Unique Key vs Stream Dedup | Redis `SETNX impression_id`<br>✅ 快<br>❌ Redis 丢数据或 failover 可能导致少量重复 | DB unique constraint<br>✅ 正确性强<br>❌ 写入延迟高 | Streaming dedup<br>✅ 点击路径最快<br>❌ 实时 dashboard 可能短暂显示重复，需要后续修正 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| Cassandra vs Druid vs ClickHouse | Cassandra<br>✅ 写入吞吐高<br>❌ 不适合任意维度 group by | Druid<br>✅ 天然按时间分区<br>❌ 不适合非常高 QPS 的点查服务 | ClickHouse<br>✅ 查询能力强<br>❌ 实时 upsert / 去重语义需要小心 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 实时数据和离线数据如何合并 | 一个 DB 存所有数据<br>✅ 架构简单，没有 merge 问题<br>❌ 实时写、历史分析、replay、修正都压在一个系统上 | Online DB 存实时，Offline DB 存历史<br>✅ 实时路径快<br>❌ Query API 需要合并 recent + historical | Druid real-time + batch segments<br>✅ Druid query engine 可以同时查 real-time data 和...<br>❌ 实时 segment 和 batch segment 时间区间重叠时，要靠 vers... | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Exactly-once：能做到什么，不能承诺什么 | At-least-once + 幂等 sink<br>✅ 简单可靠<br>❌ sink 不幂等就会重复计数 | Kafka transactional producer<br>✅ 支持 transaction abort rollback<br>❌ 只能解决 Kafka 内部事务，不自动解决外部 DB 写入 exactly-once | Flink checkpoint + transactional / idempotent sink<br>✅ checkpoint 可以保存 offset 和 state<br>❌ 外部 sink 必须支持 2PC、事务、幂等 key 或 overwrite | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| Delayed Event / Late Arrival | 严格按 event time window 关闭<br>✅ 结果产出快<br>❌ 迟到事件会被丢弃或进入 correction path | Watermark + allowed lateness<br>✅ 能吸收常见网络延迟和客户端延迟<br>❌ dashboard 会有一段时间不断修正 | 离线 reconciliation 修正<br>✅ 准确性最高<br>❌ 不是实时结果 | 我不会把 TTL 当成 correctness 机制。TTL 适合清理；正确性要靠 read path 检查 expires_at，再配合可恢复的 scheduler 或 delayed queue，且 worker 执行必须幂等。 |
| Druid Roll-up 和 Reindex/Compaction | Kafka indexing task 实时 ingestion<br>✅ 从 Kafka 消费，生成 real-time segments，能快速查询<br>❌ 实时 segment 小且多，需要后续 compaction | Batch ingestion<br>✅ 数据更完整，segment 更规整<br>❌ 有延迟，不能替代实时 dashboard | Compaction + version overwrite<br>✅ 把小 segment 合并，降低 query overhead<br>❌ 要管理 interval、version、roll-up 粒度，避免重复计数 | 我会按数据用途设计 retention：raw log 用于 replay，compacted state 用于 serving，cold storage 用于 audit。compaction/GC 可以省成本，但不能删掉恢复、合规或用户可见正确性所需的数据。 |
| 查询服务：实时高 QPS vs 分析型查询 | 直接查 Druid / ClickHouse<br>✅ 灵活，维度丰富<br>❌ 高 QPS 下成本高，容易影响集群稳定性 | 预聚合结果存 Online DB<br>✅ 低延迟，高 QPS 友好<br>❌ 查询维度固定，灵活性差 | Cache + OLAP fallback<br>✅ 保护 OLAP engine，降低延迟<br>❌ cache invalidation 和 freshness 需要控制 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
