...
| --- | --- | --- | --- | --- |
| Redis Sorted Set vs DynamoDB + GSI | Redis Sorted Set<br>✅ 原生支持 `ZADD/ZREVRANGE/ZREVRANK`<br>❌ 内存成本高 | DynamoDB + GSI(score as sort key)<br>✅ 持久化强，扩展性好<br>❌ GSI 按 score 排序会受到 partition key 设计限制 | DynamoDB 做 source of truth + Redis 做 leaderboard read model<br>✅ DynamoDB 保可靠分数<br>❌ 双写/异步更新带来最终一致 | 选方案 C。<br>Staff+ 点：Redis 是 read model，DynamoDB/ScoreDB 是 source of truth。 |
| Redis scale：单 ZSET vs sharded ZSET vs threshold TopK | 单 board 单 ZSET<br>✅ 实现最简单<br>❌ 单 key 过大，内存和 CPU 热点明显 | 按 user_id consistent hash 分多个 ZSET shard<br>✅ 写入分散<br>❌ 全局 Top K 需要 merge 多个 shard | 只把达到一定分数阈值的用户写入 Redis<br>✅ 大幅降低 Redis 内存<br>❌ 长尾用户无法直接查精确 rank | 如果只查 Top K，用 threshold TopK 或 sharded local TopN。<br>如果要 around-me 和精确 rank，单 Redis ZSET 简单但规模有限，要明确成本。 |
| Sharding：按 score range vs consistent hash by user_id | 按 score range shard<br>✅ 查询 Top K 直观，从最高 score range 开始查<br>❌ Leaderboard 分数频繁变化，用户会跨 shard 移动 | 按 user_id consistent hash shard<br>✅ 写入分散稳定<br>❌ 全局 Top K 需要各 shard 上报 local Top N 后 merge | 混合：user_id shard + score histogram/rank bucket<br>✅ 写入稳定<br>❌ 实现更复杂 | 高频更新的 leaderboard 不建议按 score range shard。<br>按 user_id consistent hash 做写扩展，定期汇总 local Top N 和 score histogram。 |
| Global Top K：每个 shard 上报 Top N vs 全量聚合 | 每次查询 scatter-gather 所有 shard<br>✅ 简单，实时性好<br>❌ 查询 fanout 大 | 每个 shard 定期上报 local Top N，Aggregator merge<br>✅ 查询快，直接读 global Top K<br>❌ 全局榜单有延迟 | 流式二阶段聚合<br>✅ 比定期 batch 更新更实时<br>❌ Aggregator 复杂，且可能成为热点 | 高 QPS 查询用 local Top N 上报 + global merge。<br>N 应该大于 K，例如每 shard 上报 `K * safety_factor`。 |
| Rank 是否存 DB：读优化 vs 写优化 | 不存 rank，读时实时计算<br>✅ 写路径简单<br>❌ 分片后实时 global rank 难算 | 存 rank / percentile 到 RankDB<br>✅ 读很快<br>❌ 每次分数变化都可能影响很多人的 rank | 只存头部用户精确 rank，长尾存 percentile/bucket<br>✅ 成本可控<br>❌ 语义不完全统一 | 不要实时存所有用户精确 rank。<br>Top users 更新完 Redis 后可以写精确 rank。 |
| Percentile：score range histogram vs batch recompute | score range histogram<br>✅ 读快，存储小<br>❌ 精度取决于 bucket 设计 | 定期 batch recompute percentile<br>✅ 结果更稳定<br>❌ 不实时 | Redis rank 实时计算 percentile<br>✅ 实时准确<br>❌ 规模受限 | 大规模下用 histogram + batch 校准。<br>头部用户给精确 rank，长尾用户给 percentile。 |
| 分数更新语义：delta vs absolute score | 事件里传 `score_delta`<br>✅ 语义自然<br>❌ 重复消费会重复加分 | 下游 leaderboard 只接收 `absolute_score`<br>✅ `ZADD absolute_score user_id` 幂等<br>❌ Score Service 必须先聚合出权威分数 | delta + event_id dedup<br>✅ 可以保留 delta 流<br>❌ Dedup state 成本高 | Source of truth 记录 delta event。<br>Leaderboard read model 使用 absolute score 更新。 |
