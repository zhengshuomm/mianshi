...
| --- | --- | --- | --- | --- |
| Redis Sorted Set vs DynamoDB + GSI | Redis Sorted Set<br>✅ 原生支持 `ZADD/ZREVRANGE/ZREVRANK`<br>❌ 内存成本高 | DynamoDB + GSI(score as sort key)<br>✅ 持久化强，扩展性好<br>❌ GSI 按 score 排序会受到 partition key 设计限制 | DynamoDB 做 source of truth + Redis 做 leaderboard read model<br>✅ DynamoDB 保可靠分数<br>❌ 双写/异步更新带来最终一致 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Redis scale：单 ZSET vs sharded ZSET vs threshold TopK | 单 board 单 ZSET<br>✅ 实现最简单<br>❌ 单 key 过大，内存和 CPU 热点明显 | 按 user_id consistent hash 分多个 ZSET shard<br>✅ 写入分散<br>❌ 全局 Top K 需要 merge 多个 shard | 只把达到一定分数阈值的用户写入 Redis<br>✅ 大幅降低 Redis 内存<br>❌ 长尾用户无法直接查精确 rank | I would first define the business invariant, such as no oversell, one winner, or one successful payment capture. The critical transition should be protected with a conditional write or state machine, and all external side effects should be idempotent and reconciled asynchronously. |
| Sharding：按 score range vs consistent hash by user_id | 按 score range shard<br>✅ 查询 Top K 直观，从最高 score range 开始查<br>❌ Leaderboard 分数频繁变化，用户会跨 shard 移动 | 按 user_id consistent hash shard<br>✅ 写入分散稳定<br>❌ 全局 Top K 需要各 shard 上报 local Top N 后 merge | 混合：user_id shard + score histogram/rank bucket<br>✅ 写入稳定<br>❌ 实现更复杂 | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| Global Top K：每个 shard 上报 Top N vs 全量聚合 | 每次查询 scatter-gather 所有 shard<br>✅ 简单，实时性好<br>❌ 查询 fanout 大 | 每个 shard 定期上报 local Top N，Aggregator merge<br>✅ 查询快，直接读 global Top K<br>❌ 全局榜单有延迟 | 流式二阶段聚合<br>✅ 比定期 batch 更新更实时<br>❌ Aggregator 复杂，且可能成为热点 | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| Rank 是否存 DB：读优化 vs 写优化 | 不存 rank，读时实时计算<br>✅ 写路径简单<br>❌ 分片后实时 global rank 难算 | 存 rank / percentile 到 RankDB<br>✅ 读很快<br>❌ 每次分数变化都可能影响很多人的 rank | 只存头部用户精确 rank，长尾存 percentile/bucket<br>✅ 成本可控<br>❌ 语义不完全统一 | I would separate the write path for scores from the read model for ranks. Redis sorted sets are excellent for hot Top-K serving, but durable score history, anti-cheat review, and shard-level aggregation should remain recoverable from the source of truth. |
| Percentile：score range histogram vs batch recompute | score range histogram<br>✅ 读快，存储小<br>❌ 精度取决于 bucket 设计 | 定期 batch recompute percentile<br>✅ 结果更稳定<br>❌ 不实时 | Redis rank 实时计算 percentile<br>✅ 实时准确<br>❌ 规模受限 | I would separate the write path for scores from the read model for ranks. Redis sorted sets are excellent for hot Top-K serving, but durable score history, anti-cheat review, and shard-level aggregation should remain recoverable from the source of truth. |
| 分数更新语义：delta vs absolute score | 事件里传 `score_delta`<br>✅ 语义自然<br>❌ 重复消费会重复加分 | 下游 leaderboard 只接收 `absolute_score`<br>✅ `ZADD absolute_score user_id` 幂等<br>❌ Score Service 必须先聚合出权威分数 | delta + event_id dedup<br>✅ 可以保留 delta 流<br>❌ Dedup state 成本高 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
