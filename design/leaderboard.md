...
|---|---|---|---|
| Redis Sorted Set vs DynamoDB + GSI | A: Redis Sorted Set<br>B: DynamoDB + GSI(score as sort key)<br>C: DynamoDB 做 source of truth + Redis 做 leaderboard read model | A ✅ 原生支持 `ZADD/ZREVRANGE/ZREVRANK`。 查询简单、延迟低。 ❌ 内存成本高。 单个大榜单会有热点和 shard 问题。 Redis 不是理想 source of truth。<br>B ✅ 持久化强，扩展性好。 条件写、幂等、source of truth 语义更好。 ❌ GSI 按 score 排序会受到 partition key 设计限制。 分数频繁变化会导致索引频繁更新。 全局 Top K 仍然需要跨 partition merge。<br>C ✅ DynamoDB 保可靠分数。 Redis 提供低延迟查询。 Redis 可从 DB/event log 重建。 ❌ 双写/异步更新带来最终一致。 | 选方案 C。<br>Staff+ 点：Redis 是 read model，DynamoDB/ScoreDB 是 source of truth。 |
| Redis scale：单 ZSET vs sharded ZSET vs threshold TopK | A: 单 board 单 ZSET<br>B: 按 user_id consistent hash 分多个 ZSET shard<br>C: 只把达到一定分数阈值的用户写入 Redis | A ✅ 实现最简单。 `ZREVRANK` 和 around-me 很方便。 ❌ 单 key 过大，内存和 CPU 热点明显。 Redis cluster 对单 key 不能自动拆分。<br>B ✅ 写入分散。 每个 shard 可独立维护 local Top N。 ❌ 全局 Top K 需要 merge 多个 shard。 精确 global rank 很难实时计算。<br>C ✅ 大幅降低 Redis 内存。 Top K 查询快。 ❌ 长尾用户无法直接查精确 rank。 阈值选择和新用户突然冲榜要处理。 | 如果只查 Top K，用 threshold TopK 或 sharded local TopN。<br>如果要 around-me 和精确 rank，单 Redis ZSET 简单但规模有限，要明确成本。 |
| Sharding：按 score range vs consistent hash by user_id | A: 按 score range shard<br>B: 按 user_id consistent hash shard<br>C: 混合：user_id shard + score histogram/rank bucket | A ✅ 查询 Top K 直观，从最高 score range 开始查。 Percentile/rank bucket 容易理解。 ❌ Leaderboard 分数频繁变化，用户会跨 shard 移动。 高分区容易成为热点。 Rebalancing 很麻烦。<br>B ✅ 写入分散稳定。 用户不会因为 score 变化频繁迁移 shard。 ❌ 全局 Top K 需要各 shard 上报 local Top N 后 merge。 精确 global rank/percentile 需要额外结构。<br>C ✅ 写入稳定。 Percentile 可用批量 histogram 近似。 ❌ 实现更复杂。 Rank 精度取决于 bucket 粒度。 | 高频更新的 leaderboard 不建议按 score range shard。<br>按 user_id consistent hash 做写扩展，定期汇总 local Top N 和 score histogram。 |
| Global Top K：每个 shard 上报 Top N vs 全量聚合 | A: 每次查询 scatter-gather 所有 shard<br>B: 每个 shard 定期上报 local Top N，Aggregator merge<br>C: 流式二阶段聚合 | A ✅ 简单，实时性好。 ❌ 查询 fanout 大。 Shard 多时延迟和失败率上升。<br>B ✅ 查询快，直接读 global Top K。 写入和查询解耦。 ❌ 全局榜单有延迟。 如果 local N 太小，可能漏掉全局 Top K 边缘用户。<br>C ✅ 比定期 batch 更新更实时。 可以持续维护 global heap。 ❌ Aggregator 复杂，且可能成为热点。 | 高 QPS 查询用 local Top N 上报 + global merge。<br>N 应该大于 K，例如每 shard 上报 `K * safety_factor`。<br>对极高实时性再做流式二阶段聚合。 |
| Rank 是否存 DB：读优化 vs 写优化 | A: 不存 rank，读时实时计算<br>B: 存 rank / percentile 到 RankDB<br>C: 只存头部用户精确 rank，长尾存 percentile/bucket | A ✅ 写路径简单。 不需要维护大量 rank 更新。 ❌ 分片后实时 global rank 难算。 大规模时读延迟高。<br>B ✅ 读很快。 Percentile 可以批量更新，适合 dashboard/profile 展示。 ❌ 每次分数变化都可能影响很多人的 rank。 实时维护精确 rank 写放大巨大。<br>C ✅ 成本可控。 头部体验准确，长尾也能显示进度感。 ❌ 语义不完全统一。 边界用户可能需要特殊处理。 | 不要实时存所有用户精确 rank。<br>Top users 更新完 Redis 后可以写精确 rank。<br>长尾用户用批处理 percentile/rank bucket。 |
| Percentile：score range histogram vs batch recompute | A: score range histogram<br>B: 定期 batch recompute percentile<br>C: Redis rank 实时计算 percentile | A ✅ 读快，存储小。 可以按 score bucket 聚合。 ❌ 精度取决于 bucket 设计。 分数分布变化时 bucket 可能不均匀。<br>B ✅ 结果更稳定。 可以扫全量数据生成 rank bucket。 ❌ 不实时。 大榜单成本高。<br>C ✅ 实时准确。 ❌ 规模受限。 Sharding 后不再简单。 | 大规模下用 histogram + batch 校准。<br>头部用户给精确 rank，长尾用户给 percentile。 |
| 分数更新语义：delta vs absolute score | A: 事件里传 `score_delta`<br>B: 下游 leaderboard 只接收 `absolute_score`<br>C: delta + event_id dedup | A ✅ 语义自然。 便于审计分数来源。 ❌ 重复消费会重复加分。 幂等处理更复杂。<br>B ✅ `ZADD absolute_score user_id` 幂等。 重复消费不会重复加分。 ❌ Score Service 必须先聚合出权威分数。<br>C ✅ 可以保留 delta 流。 ❌ Dedup state 成本高。 过期和重放逻辑复杂。 | Source of truth 记录 delta event。<br>Leaderboard read model 使用 absolute score 更新。 |
