...
| --- | --- | --- | --- | --- |
| Redis Sorted Set vs DynamoDB + GSI | Redis Sorted Set<br>✅ 原生支持 `ZADD/ZREVRANGE/ZREVRANK`<br>❌ 内存成本高 | DynamoDB + GSI(score as sort key)<br>✅ 持久化强，扩展性好<br>❌ GSI 按 score 排序会受到 partition key 设计限制 | DynamoDB 做 source of truth + Redis 做 leaderboard read model<br>✅ DynamoDB 保可靠分数<br>❌ 双写/异步更新带来最终一致 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Redis scale：单 ZSET vs sharded ZSET vs threshold TopK | 单 board 单 ZSET<br>✅ 实现最简单<br>❌ 单 key 过大，内存和 CPU 热点明显 | 按 user_id consistent hash 分多个 ZSET shard<br>✅ 写入分散<br>❌ 全局 Top K 需要 merge 多个 shard | 只把达到一定分数阈值的用户写入 Redis<br>✅ 大幅降低 Redis 内存<br>❌ 长尾用户无法直接查精确 rank | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| Sharding：按 score range vs consistent hash by user_id | 按 score range shard<br>✅ 查询 Top K 直观，从最高 score range 开始查<br>❌ Leaderboard 分数频繁变化，用户会跨 shard 移动 | 按 user_id consistent hash shard<br>✅ 写入分散稳定<br>❌ 全局 Top K 需要各 shard 上报 local Top N 后 merge | 混合：user_id shard + score histogram/rank bucket<br>✅ 写入稳定<br>❌ 实现更复杂 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Global Top K：每个 shard 上报 Top N vs 全量聚合 | 每次查询 scatter-gather 所有 shard<br>✅ 简单，实时性好<br>❌ 查询 fanout 大 | 每个 shard 定期上报 local Top N，Aggregator merge<br>✅ 查询快，直接读 global Top K<br>❌ 全局榜单有延迟 | 流式二阶段聚合<br>✅ 比定期 batch 更新更实时<br>❌ Aggregator 复杂，且可能成为热点 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Rank 是否存 DB：读优化 vs 写优化 | 不存 rank，读时实时计算<br>✅ 写路径简单<br>❌ 分片后实时 global rank 难算 | 存 rank / percentile 到 RankDB<br>✅ 读很快<br>❌ 每次分数变化都可能影响很多人的 rank | 只存头部用户精确 rank，长尾存 percentile/bucket<br>✅ 成本可控<br>❌ 语义不完全统一 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| Percentile：score range histogram vs batch recompute | score range histogram<br>✅ 读快，存储小<br>❌ 精度取决于 bucket 设计 | 定期 batch recompute percentile<br>✅ 结果更稳定<br>❌ 不实时 | Redis rank 实时计算 percentile<br>✅ 实时准确<br>❌ 规模受限 | 我会把 score 写路径和 rank/read model 分开。Redis ZSET 很适合热 TopK serving，但分数历史、反作弊审核、shard 聚合和重建能力要回到 durable source of truth。 |
| 分数更新语义：delta vs absolute score | 事件里传 `score_delta`<br>✅ 语义自然<br>❌ 重复消费会重复加分 | 下游 leaderboard 只接收 `absolute_score`<br>✅ `ZADD absolute_score user_id` 幂等<br>❌ Score Service 必须先聚合出权威分数 | delta + event_id dedup<br>✅ 可以保留 delta 流<br>❌ Dedup state 成本高 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
