...
| --- | --- | --- | --- | --- | --- |
| Token Bucket vs Leaky Bucket vs Fixed Window vs Sliding Window | Token Bucket<br>✅ 用户体验好<br>❌ 实现比 fixed window 复杂 | Leaky Bucket<br>✅ 请求处理速率稳定<br>❌ burst 支持弱 | Fixed Window<br>✅ 实现简单，Redis `INCR + EXPIRE` 即可<br>❌ 窗口边界问题明显，两个窗口交界可能放过 2 倍请求 | Sliding Window / Sliding Window Count<br>✅ 比 fixed window 平滑，边界误差小<br>❌ 精确 sliding log 存储成本高 | 通用 API limit 用 token bucket。<br>保护下游稳定吞吐用 leaky bucket。 |
| 分布式状态：本地计数 vs Redis 集中计数 | 每个 gateway 本地计数<br>✅ 极低延迟，不依赖外部 Redis<br>❌ 多个 gateway 会各自放量，总量可能超出限制 | Redis 分布式计数<br>✅ 全局单 region 内较准确<br>❌ 每个请求多一次 Redis 调用 | Local cache + Redis periodic sync<br>✅ 降低 Redis 压力<br>❌ 准确性下降，故障时可能 over-allow | - | 默认 Redis 分布式计数。<br>极热 key 用本地小额度预取，周期性向 Redis 扣减。 |
| Redis 分片：Consistent Hashing | 所有 key 进单 Redis<br>✅ 简单<br>❌ 容量、QPS、hot key 很快成为瓶颈 | 按 key consistent hashing 到 Redis shards<br>✅ 扩容迁移少量 key<br>❌ ring 变更期间短窗口状态可能迁移不一致 | Redis Cluster<br>✅ 运维成熟，自动分槽<br>❌ multi-key 脚本受 hash slot 限制 | - | 只操作单个 rate key 的规则，consistent hashing 或 Redis Cluster 都可。<br>rate key 构造必须稳定：`rule_id + dimension + identifier`。 |
| 跨 DC Global Rate Limit | 每次请求同步调用 Global Quota Service<br>✅ 全局最准确<br>❌ 跨 DC 延迟高 | Local quota service + periodic sync<br>✅ 请求只打本地，低延迟高可用<br>❌ 不同 DC 可能短暂超用，需要控制 quota drift | Static quota split<br>✅ 简单，无需频繁协调<br>❌ 某个 DC 流量突增时可能本地限流，但其他 DC 额度闲置 | - | 使用 local quota service + global periodic sync。<br>Global 根据各 DC 近期使用率动态分配 quota slice。 |
| Hot Key 和高 QPS API Key | 所有请求直接打同一个 Redis key<br>✅ 准确<br>❌ 大客户 API key 或热门 IP 会打爆单 Redis shard | 本地 token lease<br>✅ Gateway 从 Redis/Quota Service 预取一小批 token，...<br>❌ gateway crash 可能丢未用 token | 分桶 counter<br>✅ 把一个 hot key 拆成多个 sub-key<br>❌ 读写多个 bucket 才能算总量，准确性/延迟 trade-off | - | 普通 key 直接 Redis Lua。<br>高 QPS tenant/API key 使用 token lease。 |
| Fail Open vs Fail Closed | Redis/Quota service 故障时 fail open<br>✅ 不因为限流系统故障阻断业务<br>❌ 下游可能被洪峰打垮 | fail closed<br>✅ 保护核心系统<br>❌ 限流依赖故障会导致大量误拒 | degraded local limit<br>✅ Redis 故障时回退到本地保守限流<br>❌ 全局准确性下降 | - | 默认 degraded local limit。<br>低风险 GET 可 fail open。 |
| Sliding Window 实现 | Sliding log<br>✅ 精确记录窗口内每个请求 timestamp<br>❌ 高 QPS 下内存巨大 | Sliding window counter<br>✅ 只存当前窗口和上一窗口 counter，成本低<br>❌ 是近似值 | Bucketed counters<br>✅ 把窗口拆成多个小 bucket，例如 60 个 1s bucket<br>❌ 读时要聚合多个 bucket，写入和清理更复杂 | - | 高 QPS 不用 sliding log。<br>常规用 sliding window counter 或 bucketed counters。 |
| 配置和可观测性 | 硬编码规则<br>✅ 简单<br>❌ 无法动态调整，事故时反应慢 | 中心化配置 + 本地 cache<br>✅ 动态发布<br>❌ 配置版本、回滚、灰度要管理 | 策略引擎<br>✅ 表达能力强<br>❌ 策略评估成本和复杂度高 | - | 规则存在 Config DB，gateway 本地 cache。<br>支持灰度、版本、回滚。 |
