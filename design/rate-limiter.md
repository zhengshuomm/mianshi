...
|---|---|---|---|
| Token Bucket vs Leaky Bucket vs Fixed Window vs Sliding Window | A: Token Bucket<br>B: Leaky Bucket<br>C: Fixed Window<br>D: Sliding Window / Sliding Window Count | A ✅ 用户体验好；可配置 refill rate 和 bucket capacity。 ❌ 实现比 fixed window 复杂；需要存 last_refill_time 和 tokens。<br>B ✅ 请求处理速率稳定。 ❌ burst 支持弱；队列满时直接拒绝或增加等待。<br>C ✅ 实现简单，Redis `INCR + EXPIRE` 即可。 ❌ 窗口边界问题明显，两个窗口交界可能放过 2 倍请求。<br>D ✅ 比 fixed window 平滑，边界误差小。 ❌ 精确 sliding log 存储成本高；sliding count 是近似。 | 通用 API limit 用 token bucket。<br>保护下游稳定吞吐用 leaky bucket。<br>简单 rule 或低价值 endpoint 用 fixed window。<br>付费 API 配额或严格窗口用 sliding window count。 |
| 分布式状态：本地计数 vs Redis 集中计数 | A: 每个 gateway 本地计数<br>B: Redis 分布式计数<br>C: Local cache + Redis periodic sync | A ✅ 极低延迟，不依赖外部 Redis。 ❌ 多个 gateway 会各自放量，总量可能超出限制。<br>B ✅ 全局单 region 内较准确；Lua 原子操作。 ❌ 每个请求多一次 Redis 调用；Redis 热点和故障会影响请求路径。<br>C ✅ 降低 Redis 压力。 ❌ 准确性下降，故障时可能 over-allow。 | 默认 Redis 分布式计数。<br>极热 key 用本地小额度预取，周期性向 Redis 扣减。<br>关键付费 quota 走更严格的 Redis/global quota。 |
| Redis 分片：Consistent Hashing | A: 所有 key 进单 Redis<br>B: 按 key consistent hashing 到 Redis shards<br>C: Redis Cluster | A ✅ 简单。 ❌ 容量、QPS、hot key 很快成为瓶颈。<br>B ✅ 扩容迁移少量 key；不同 user/API key 分散到不同 shard。 ❌ ring 变更期间短窗口状态可能迁移不一致。<br>C ✅ 运维成熟，自动分槽。 ❌ multi-key 脚本受 hash slot 限制；复杂规则要设计 key tag。 | 只操作单个 rate key 的规则，consistent hashing 或 Redis Cluster 都可。<br>rate key 构造必须稳定：`rule_id + dimension + identifier`。<br>匿名用户用 IP，登录用户用 user_id，API 请求用 API key。 |
| 跨 DC Global Rate Limit | A: 每次请求同步调用 Global Quota Service<br>B: Local quota service + periodic sync<br>C: Static quota split | A ✅ 全局最准确。 ❌ 跨 DC 延迟高；global service 故障会影响所有地区。<br>B ✅ 请求只打本地，低延迟高可用。 ❌ 不同 DC 可能短暂超用，需要控制 quota drift。<br>C ✅ 简单，无需频繁协调。 ❌ 某个 DC 流量突增时可能本地限流，但其他 DC 额度闲置。 | 使用 local quota service + global periodic sync。<br>Global 根据各 DC 近期使用率动态分配 quota slice。<br>对金融/计费强一致 quota，可使用 global check 或保守预留。 |
| Hot Key 和高 QPS API Key | A: 所有请求直接打同一个 Redis key<br>B: 本地 token lease<br>C: 分桶 counter | A ✅ 准确。 ❌ 大客户 API key 或热门 IP 会打爆单 Redis shard。<br>B ✅ Gateway 从 Redis/Quota Service 预取一小批 token，本地消耗。 ❌ gateway crash 可能丢未用 token；会产生轻微 underuse/overuse。<br>C ✅ 把一个 hot key 拆成多个 sub-key。 ❌ 读写多个 bucket 才能算总量，准确性/延迟 trade-off。 | 普通 key 直接 Redis Lua。<br>高 QPS tenant/API key 使用 token lease。<br>Hot key 检测后自动切换策略。 |
| Fail Open vs Fail Closed | A: Redis/Quota service 故障时 fail open<br>B: fail closed<br>C: degraded local limit | A ✅ 不因为限流系统故障阻断业务。 ❌ 下游可能被洪峰打垮。<br>B ✅ 保护核心系统。 ❌ 限流依赖故障会导致大量误拒。<br>C ✅ Redis 故障时回退到本地保守限流。 ❌ 全局准确性下降。 | 默认 degraded local limit。<br>低风险 GET 可 fail open。<br>高风险写接口 fail closed 或 conservative local cap。 |
| Sliding Window 实现 | A: Sliding log<br>B: Sliding window counter<br>C: Bucketed counters | A ✅ 精确记录窗口内每个请求 timestamp。 ❌ 高 QPS 下内存巨大。<br>B ✅ 只存当前窗口和上一窗口 counter，成本低。 ❌ 是近似值。<br>C ✅ 把窗口拆成多个小 bucket，例如 60 个 1s bucket。 ❌ 读时要聚合多个 bucket，写入和清理更复杂。 | 高 QPS 不用 sliding log。<br>常规用 sliding window counter 或 bucketed counters。<br>Strict quota 用 token bucket + audit/reconciliation。 |
| 配置和可观测性 | A: 硬编码规则<br>B: 中心化配置 + 本地 cache<br>C: 策略引擎 | A ✅ 简单。 ❌ 无法动态调整，事故时反应慢。<br>B ✅ 动态发布；请求热路径低延迟。 ❌ 配置版本、回滚、灰度要管理。<br>C ✅ 表达能力强。 ❌ 策略评估成本和复杂度高。 | 规则存在 Config DB，gateway 本地 cache。<br>支持灰度、版本、回滚。<br>监控 allowed/throttled、rule hit、Redis latency、hot key、quota drift。 |
