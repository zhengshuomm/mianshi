...
| --- | --- | --- | --- | --- | --- |
| Token Bucket vs Leaky Bucket vs Fixed Window vs Sliding Window | Token Bucket<br>✅ 用户体验好<br>❌ 实现比 fixed window 复杂 | Leaky Bucket<br>✅ 请求处理速率稳定<br>❌ burst 支持弱 | Fixed Window<br>✅ 实现简单，Redis `INCR + EXPIRE` 即可<br>❌ 窗口边界问题明显，两个窗口交界可能放过 2 倍请求 | Sliding Window / Sliding Window Count<br>✅ 比 fixed window 平滑，边界误差小<br>❌ 精确 sliding log 存储成本高 | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| 分布式状态：本地计数 vs Redis 集中计数 | 每个 gateway 本地计数<br>✅ 极低延迟，不依赖外部 Redis<br>❌ 多个 gateway 会各自放量，总量可能超出限制 | Redis 分布式计数<br>✅ 全局单 region 内较准确<br>❌ 每个请求多一次 Redis 调用 | Local cache + Redis periodic sync<br>✅ 降低 Redis 压力<br>❌ 准确性下降，故障时可能 over-allow | - | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Redis 分片：Consistent Hashing | 所有 key 进单 Redis<br>✅ 简单<br>❌ 容量、QPS、hot key 很快成为瓶颈 | 按 key consistent hashing 到 Redis shards<br>✅ 扩容迁移少量 key<br>❌ ring 变更期间短窗口状态可能迁移不一致 | Redis Cluster<br>✅ 运维成熟，自动分槽<br>❌ multi-key 脚本受 hash slot 限制 | - | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| 跨 DC Global Rate Limit | 每次请求同步调用 Global Quota Service<br>✅ 全局最准确<br>❌ 跨 DC 延迟高 | Local quota service + periodic sync<br>✅ 请求只打本地，低延迟高可用<br>❌ 不同 DC 可能短暂超用，需要控制 quota drift | Static quota split<br>✅ 简单，无需频繁协调<br>❌ 某个 DC 流量突增时可能本地限流，但其他 DC 额度闲置 | - | 除非保护明确的业务 invariant，否则我会避免 global strong consistency。home region 或 regional shard 能让写路径更清楚；跨区复制可以异步，并为冲突规则和 reconciliation 留出设计空间。 |
| Hot Key 和高 QPS API Key | 所有请求直接打同一个 Redis key<br>✅ 准确<br>❌ 大客户 API key 或热门 IP 会打爆单 Redis shard | 本地 token lease<br>✅ Gateway 从 Redis/Quota Service 预取一小批 token，...<br>❌ gateway crash 可能丢未用 token | 分桶 counter<br>✅ 把一个 hot key 拆成多个 sub-key<br>❌ 读写多个 bucket 才能算总量，准确性/延迟 trade-off | - | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
| Fail Open vs Fail Closed | Redis/Quota service 故障时 fail open<br>✅ 不因为限流系统故障阻断业务<br>❌ 下游可能被洪峰打垮 | fail closed<br>✅ 保护核心系统<br>❌ 限流依赖故障会导致大量误拒 | degraded local limit<br>✅ Redis 故障时回退到本地保守限流<br>❌ 全局准确性下降 | - | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Sliding Window 实现 | Sliding log<br>✅ 精确记录窗口内每个请求 timestamp<br>❌ 高 QPS 下内存巨大 | Sliding window counter<br>✅ 只存当前窗口和上一窗口 counter，成本低<br>❌ 是近似值 | Bucketed counters<br>✅ 把窗口拆成多个小 bucket，例如 60 个 1s bucket<br>❌ 读时要聚合多个 bucket，写入和清理更复杂 | - | 我会先问清楚固定窗口还是任意窗口、精确还是近似、K 是否固定。固定热窗口可以预聚合到 Redis；任意历史窗口应该走 OLAP；真正难点是旧数据淘汰和 late/duplicate event。 |
| 配置和可观测性 | 硬编码规则<br>✅ 简单<br>❌ 无法动态调整，事故时反应慢 | 中心化配置 + 本地 cache<br>✅ 动态发布<br>❌ 配置版本、回滚、灰度要管理 | 策略引擎<br>✅ 表达能力强<br>❌ 策略评估成本和复杂度高 | - | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
