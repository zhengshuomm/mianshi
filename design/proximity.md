...
|---|---|---|---|
| GeoHash vs Quadtree | A: GeoHash<br>B: Quadtree<br>C: H3 / S2 | A ✅ 实现简单；字符串 prefix 可用于范围查询；位置更新容易，只需重新计算 geohash。 ❌ cell size 固定，不能按人口密度动态调整；边界问题明显，很近的位置可能没有 shared prefix。<br>B ✅ 可以按 density 动态拆分，热点区域 cell 更小，稀疏区域 cell 更大。 ❌ 实现复杂；移动实体更新 index 更难；树结构分裂/合并要维护一致性。<br>C ✅ 成熟库支持多 resolution 和邻居查询；比手写 geohash/quadtree 更可运维。 ❌ 需要理解 cell resolution、边界覆盖和索引建模。 | 面试基础版可以讲 GeoHash。<br>Staff+ 版本建议指出 H3/S2 更适合生产系统。<br>高频移动实体优先选 fixed grid/H3/S2，而不是复杂动态 quadtree。 |
| 边界问题：只查当前 cell vs current + neighbours | A: 只查当前 GeoHash prefix<br>B: 查 current cell + neighboring cells<br>C: 按圆形范围覆盖 cells | A ✅ 查询简单、低延迟。 ❌ 用户在 cell 边缘时会漏掉隔壁 cell 的近邻。<br>B ✅ 解决大部分边界漏召回问题。 ❌ 查询 cell 数增加，候选集变大。<br>C ✅ 召回更稳定，适合 H3/S2 k-ring 或 covering API。 ❌ 实现比固定 8 邻居复杂。 | GeoHash 至少查 current + 8 neighbours。<br>H3/S2 用 k-ring 或 region covering。<br>最后必须精确距离过滤，因为 cell 覆盖只是候选召回。 |
| 不够结果时怎么办：扩大半径 vs 降低 precision | A: 固定半径返回不足结果<br>B: 逐步扩大 search radius<br>C: GeoHash remove last digit / lower precision | A ✅ 语义清晰。 ❌ 稀疏区域体验差，可能返回很少甚至空。<br>B ✅ 用户体验好。 ❌ 查询成本可能逐步升高，远距离结果相关性下降。<br>C ✅ 实现简单，prefix 更短覆盖更大区域。 ❌ 候选集可能突然变很大，尤其城市中心。 | 查询先按目标 radius 找 cells。<br>结果不足时逐步扩大 radius 或降低 resolution。<br>设置最大 radius，避免稀疏区域查询无限放大。 |
| Redis GEO 是否适合 | A: Redis GEO<br>B: Redis GEO per region shard<br>C: H3/S2 + NoSQL/KV | A ✅ API 简单，`GEOADD` / `GEOSEARCH` 上手快；延迟低。 ❌ 本质是 sorted set，频繁更新写会有压力；复杂 sharding 和多维过滤要自己做。<br>B ✅ 可以按 city/geohash prefix 分散写入。 ❌ 跨边界查询要 scatter-gather 多个 shard；rebalance 复杂。<br>C ✅ cell id 天然作为 shard key；邻居查询明确；写更新路径更可控。 ❌ 需要自己实现距离过滤、候选聚合和 TTL。 | Redis GEO 适合原型或中小规模。<br>高频移动位置更新更推荐 H3/S2 + KV/NoSQL + TTL。<br>生产系统常把 Redis 用作 cache，而不是唯一 geo index source。 |
| 高频位置更新：同步写索引 vs 异步索引 | A: 每次位置更新同步更新所有索引<br>B: 异步 location update stream<br>C: 客户端 adaptive update interval | A ✅ 查询最新。 ❌ 司机/设备高频更新会导致写放大和尾延迟。<br>B ✅ 写入解耦；Indexer 可以 batch、去重、丢弃旧版本。 ❌ nearby query 可能短暂读到旧位置。<br>C ✅ 静止或慢速时降低更新频率，高速/转向时提高频率。 ❌ 客户端逻辑更复杂，可能被恶意或异常客户端绕过。 | Source DB 保存 latest location。<br>Geo index 异步更新，并用 timestamp/version 防旧位置覆盖新位置。<br>客户端用 adaptive interval，服务端加最小上报间隔和位置变化阈值。 |
| 静态 POI vs 动态实体 | A: 统一索引<br>B: 静态/动态分离<br>C: 多层索引 | A ✅ 实现简单。 ❌ 静态店铺和动态司机的更新频率完全不同，混在一起影响性能。<br>B ✅ 静态 POI 可离线构建和缓存；动态实体支持 TTL 和高频更新。 ❌ 查询时需要合并两个来源。<br>C ✅ 不同实体使用不同存储和 freshness 策略。 ❌ Query Service 需要做结果融合和排序。 | 静态 POI 用离线/低频索引，可缓存。<br>动态实体用实时 geo index + TTL。<br>Query Service 统一做距离排序和业务过滤。 |
| Sharding：按 entity_id vs 按 geo cell | A: 按 `entity_id` shard<br>B: 按 geo cell shard<br>C: 双写 latest store + geo index | A ✅ 更新某个实体容易；负载相对均匀。 ❌ nearby query 需要跨很多 shard 扫描，不适合范围查找。<br>B ✅ 查询附近只访问相关 cells；空间局部性好。 ❌ 城市中心 cell 热点明显；实体跨 cell 移动需要删除旧 entry。<br>C ✅ entity lookup 和 nearby query 各自优化。 ❌ 两份数据要处理一致性和重建。 | Source DB 按 `entity_id`。<br>Geo Index 按 cell id / region。<br>热点 cell 可加 sub-shard，例如 `cell_id + hash(entity_id) % N`。 |
| H3/S2 高阶方案 | A: H3<br>B: S2<br>C: 时空数据库 / OLAP | A ✅ 多 resolution；k-ring 查询方便；Uber 生态经验多。 ❌ hexagon 边界仍需距离过滤；resolution 选择影响召回和成本。<br>B ✅ Google 常用；层级结构强；覆盖任意区域能力好。 ❌ 实现和调参比 GeoHash 更复杂。<br>C ✅ GeoMesa、ClickHouse+H3 适合大规模 geo/time query。 ❌ 不一定适合毫秒级在线 dispatch 主路径。 | 在线 nearby 主路径使用 H3/S2 + KV/NoSQL。<br>历史分析、热区统计、运营报表使用 ClickHouse + H3 或 GeoMesa。<br>不要用一个数据库同时承担高频在线匹配和复杂历史分析。 |
