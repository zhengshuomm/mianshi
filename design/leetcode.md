...
| --- | --- | --- | --- | --- |
| 执行隔离：VM vs Container vs Serverless | VM<br>✅ 完整 OS 隔离，攻击逃逸难度更高<br>❌ 资源开销大，启动慢，弹性扩缩容成本高 | Docker Container<br>✅ 轻量、启动快、资源利用率高，便于按语言打 image<br>❌ 共享 host kernel，隔离弱于 VM | Serverless Function<br>✅ 自动扩缩容，运维低，空闲成本低<br>❌ 资源和运行时限制强 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Container 安全限制 | 只用普通 Docker<br>✅ 实现简单<br>❌ 用户代码可能访问网络、fork bomb、读 host mount、打 kernel... | Hardened Container<br>✅ 成本低、启动快、安全性足够可控<br>❌ 仍然共享 kernel，需要持续 patch 和 defense-in-depth | Container + MicroVM<br>✅ 既有 container 镜像生态，又有 VM 级隔离<br>❌ 启动和运维复杂度上升 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Client 获取结果：Polling vs SSE | Polling<br>✅ 实现简单<br>❌ 有几秒延迟 | SSE<br>✅ server-to-client 单向推送简单<br>❌ 兼容性差于普通 HTTP，不支持 IE | WebSocket<br>✅ 双向低延迟<br>❌ 连接管理和扩展复杂 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Code/Test Storage：DB vs S3 | 代码直接存在 DB<br>✅ 读取简单<br>❌ 大文件、多文件项目会让 DB 膨胀 | 代码放 S3，DB 存 pointer<br>✅ 成本低，适合大对象<br>❌ 多一次读取 | Hybrid<br>✅ 小提交快，大提交省成本<br>❌ 两条读取路径 | 我会按访问模式和恢复语义选择存储，而不是只说能 scale。Staff 级别要讲清 write amplification、read latency、replication consistency，以及系统如何从 WAL、snapshot 或 replay 恢复。 |
| Queue 和 Runner 扩展 | 单全局 queue<br>✅ 简单<br>❌ contest burst 可能饿死普通提交 | 按语言/优先级分 queue<br>✅ 不同 runtime 独立扩展<br>❌ 调度器要处理公平性和容量分配 | 独立 contest pool<br>✅ 隔离峰值，避免影响普通用户<br>❌ 资源利用率可能降低 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
| At-least-once 和结果幂等 | 假设 evaluation 只执行一次<br>✅ 实现简单<br>❌ queue retry、runner crash 都会导致重复执行 | 幂等更新 submission result<br>✅ 重复评测不会破坏最终状态<br>❌ 需要状态机和 version/CAS | dedup execution<br>✅ 减少重复执行浪费<br>❌ 需要 execution lease，runner failure 后要恢复 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
