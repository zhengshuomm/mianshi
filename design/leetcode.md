...
| --- | --- | --- | --- | --- |
| 执行隔离：VM vs Container vs Serverless | VM<br>✅ 完整 OS 隔离，攻击逃逸难度更高<br>❌ 资源开销大，启动慢，弹性扩缩容成本高 | Docker Container<br>✅ 轻量、启动快、资源利用率高，便于按语言打 image<br>❌ 共享 host kernel，隔离弱于 VM | Serverless Function<br>✅ 自动扩缩容，运维低，空闲成本低<br>❌ 资源和运行时限制强 | 普通 LeetCode 使用 container runner。<br>高安全场景可用 microVM，比如 Firecracker 类方案。 |
| Container 安全限制 | 只用普通 Docker<br>✅ 实现简单<br>❌ 用户代码可能访问网络、fork bomb、读 host mount、打 kernel... | Hardened Container<br>✅ 成本低、启动快、安全性足够可控<br>❌ 仍然共享 kernel，需要持续 patch 和 defense-in-depth | Container + MicroVM<br>✅ 既有 container 镜像生态，又有 VM 级隔离<br>❌ 启动和运维复杂度上升 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| Client 获取结果：Polling vs SSE | Polling<br>✅ 实现简单<br>❌ 有几秒延迟 | SSE<br>✅ server-to-client 单向推送简单<br>❌ 兼容性差于普通 HTTP，不支持 IE | WebSocket<br>✅ 双向低延迟<br>❌ 连接管理和扩展复杂 | LeetCode submission result 默认 polling。<br>如果要展示逐 test case 进度或编译日志，可以加 SSE。 |
| Code/Test Storage：DB vs S3 | 代码直接存在 DB<br>✅ 读取简单<br>❌ 大文件、多文件项目会让 DB 膨胀 | 代码放 S3，DB 存 pointer<br>✅ 成本低，适合大对象<br>❌ 多一次读取 | Hybrid<br>✅ 小提交快，大提交省成本<br>❌ 两条读取路径 | 小函数代码直接存在 Submission DB。<br>大文件/多文件代码和 judge artifact 放 S3。 |
| Queue 和 Runner 扩展 | 单全局 queue<br>✅ 简单<br>❌ contest burst 可能饿死普通提交 | 按语言/优先级分 queue<br>✅ 不同 runtime 独立扩展<br>❌ 调度器要处理公平性和容量分配 | 独立 contest pool<br>✅ 隔离峰值，避免影响普通用户<br>❌ 资源利用率可能降低 | queue 按语言和优先级拆分。<br>ECS/K8s autoscaling 根据 queue lag、CPU、memory 扩 runner。 |
| At-least-once 和结果幂等 | 假设 evaluation 只执行一次<br>✅ 实现简单<br>❌ queue retry、runner crash 都会导致重复执行 | 幂等更新 submission result<br>✅ 重复评测不会破坏最终状态<br>❌ 需要状态机和 version/CAS | dedup execution<br>✅ 减少重复执行浪费<br>❌ 需要 execution lease，runner failure 后要恢复 | Queue 默认 at-least-once。<br>每个 submission 有状态机：`queued -> running -> success/failed`。 |
| Leaderboard：Redis ZSET vs DB | DB 实时算排名<br>✅ source of truth 简单<br>❌ 排名查询和排序成本高，高峰扛不住 | Redis Sorted Set<br>✅ 更新和 top-N 查询快<br>❌ Redis 不是最终事实源 | Redis + periodic reconciliation<br>✅ 实时体验好，同时能修正错误<br>❌ 实现多一层 batch/rebuild | Redis ZSET 服务实时 leaderboard。<br>SubmissionDB 是 source of truth。 |
| Monitoring / Alert | 只看 API latency/error<br>✅ 容易实现<br>❌ 看不到 judge pipeline 是否堆积 | queue + runner 指标<br>✅ 能及时发现 contest backlog、某语言 runner 挂掉<br>❌ 需要按语言、题目、contest 维度拆指标 | sandbox 安全审计<br>✅ 能发现越权尝试、异常 syscall、资源攻击<br>❌ 日志量大，需要采样和规则 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
