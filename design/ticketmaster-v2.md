...
| --- | --- | --- | --- | --- |
| 防 Double Booking：Distributed Lock vs DB Conditional Update | Distributed lock with TTL<br>✅ 快速拦截竞争<br>❌ 网络分区、锁过期、client pause 都可能导致边界问题 | DB conditional update / row lock<br>✅ 状态和更新在同一个系统内，正确性清楚<br>❌ 热门座位/票区会有 write contention | Queue serialization per seat/section<br>✅ 同一分区串行处理，减少 DB 冲突<br>❌ 增加排队延迟 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| 两种抢票模式：先排队再看票 vs 都能看票再异步下单 | 先进入虚拟队列，然后才能看票/买票<br>✅ 保护搜索、库存、订单系统<br>❌ 用户体验有等待 | 所有人都能看票，下单后进入 Kafka 异步处理<br>✅ 用户能浏览票况<br>❌ 很多用户看到票但最终失败，心理落差大 | Hybrid<br>✅ 普通活动直接看票<br>❌ 活动模式切换和容量估算更复杂 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| Virtual Queue + WebSocket/SSE | Polling 排队状态<br>✅ 实现简单，HTTP 兼容好<br>❌ 用户频繁刷新造成重复请求 | WebSocket<br>✅ 实时推送位置和抢票状态<br>❌ 连接管理复杂，占用服务资源 | SSE<br>✅ 比 WebSocket 简单<br>❌ 断线重连、浏览器连接数限制、兼容性需要考虑 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| DB 和 Cache 一致性 | Write-through cache<br>✅ 写 DB 同时更新 cache，看起来一致<br>❌ 并发写、失败重试、跨服务更新仍会导致 cache 不一致 | Cache-aside + TTL<br>✅ 简单，cache 失效可回源<br>❌ 短时间 stale | Event-driven invalidation / incremental update<br>✅ 库存变化后推送 cache 更新或失效<br>❌ 事件丢失/乱序会导致 cache drift，需要 TTL/rebuild 兜底 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 搜索加速：CDN / Query Cache / Adaptive Cache | CDN 缓存活动页和静态资源<br>✅ 极大减轻 origin 压力<br>❌ 个性化和实时库存不适合 CDN 长缓存 | Query Result Cache<br>✅ 直接复用非个性化查询结果<br>❌ 缓存结果不能包含用户专属价格/权限 | Adaptive / Tiered Cache<br>✅ 热门 query 短 TTL 保新鲜，冷门 query 长 TTL 省成本<br>❌ TTL 策略和 invalidation 更复杂 | 我不会让 cache 单独承担 correctness，除非有 versioned write 或 single-writer 保证。默认选择是先写 source of truth，再异步更新 cache；读到 stale 时要能回源，并对关键实体做 reconciliation。 |
| 异步处理和客户端结果通知 | 同步 reserve 后直接返回结果<br>✅ 用户体验直接，状态简单<br>❌ 高峰时 API tail latency 高，库存 DB 压力大 | 请求入 Kafka，异步处理<br>✅ MQ 削峰，worker 可水平扩展<br>❌ 用户需要等待结果 | 同步轻量预检 + 异步最终确认<br>✅ 无 queue token、重复请求、售罄可快速失败<br>❌ 状态机更复杂 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 支付 Hold 和超时释放 | reserve 后永久占票直到支付<br>✅ 实现简单<br>❌ 恶意或犹豫用户会占住票 | 短 TTL hold<br>✅ 用户有支付时间，未支付自动释放<br>❌ 支付成功和过期释放存在竞态 | 支付成功后再抢票<br>✅ 不占库存<br>❌ 付款后没票会导致退款和投诉 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| 热点 Sharding 和容量保护 | 按 event_id shard<br>✅ 同一个活动数据 locality 好<br>❌ 超级热门 event 会形成单 shard 热点 | 按 event_id + section/seat range shard<br>✅ 把热门活动拆到多个 shard<br>❌ 跨 section 查询和统计复杂 | Virtual queue admission control<br>✅ 从入口把并发限制在系统可承受范围内<br>❌ 用户等待，队列系统本身要高可用 | 我会按最热实体设计，而不是平均 QPS。先用自然分片；遇到热点再引入 salting、per-entity single writer、shard-local TopN 或 admission limit，只在被验证的瓶颈上增加复杂度。 |
