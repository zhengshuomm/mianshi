...
|---|---|---|---|
| 防 Double Booking：Distributed Lock vs DB Conditional Update | A: Distributed lock with TTL<br>B: DB conditional update / row lock<br>C: Queue serialization per seat/section | A ✅ 快速拦截竞争；TTL 可避免死锁。 ❌ 网络分区、锁过期、client pause 都可能导致边界问题；不能单独保证不超卖。<br>B ✅ 状态和更新在同一个系统内，正确性清楚。 ❌ 热门座位/票区会有 write contention。<br>C ✅ 同一分区串行处理，减少 DB 冲突。 ❌ 增加排队延迟；分区热点仍然存在。 | Redis lock with TTL 可以作为优化。<br>最终必须用 DB 条件更新：`available -> held -> sold`。<br>面试里要明确 lock 不是 source of truth。 |
| 两种抢票模式：先排队再看票 vs 都能看票再异步下单 | A: 先进入虚拟队列，然后才能看票/买票<br>B: 所有人都能看票，下单后进入 Kafka 异步处理<br>C: Hybrid | A ✅ 保护搜索、库存、订单系统；用户不会反复刷新。 ❌ 用户体验有等待；队列公平性和断线恢复复杂。<br>B ✅ 用户能浏览票况；下单写入通过 MQ 削峰。 ❌ 很多用户看到票但最终失败，心理落差大；库存展示可能 stale。<br>C ✅ 普通活动直接看票；热点活动进入 waiting room；放行后可看实时票况。 ❌ 活动模式切换和容量估算更复杂。 | 默认 hybrid。<br>超热门 event 使用 virtual queue gating。<br>普通 event 允许浏览，reserve 时强一致验证。 |
| Virtual Queue + WebSocket/SSE | A: Polling 排队状态<br>B: WebSocket<br>C: SSE | A ✅ 实现简单，HTTP 兼容好。 ❌ 用户频繁刷新造成重复请求；位置更新延迟大。<br>B ✅ 实时推送位置和抢票状态；减少用户刷新；可感知连接状态。 ❌ 连接管理复杂，占用服务资源。<br>C ✅ 比 WebSocket 简单；适合 status stream。 ❌ 断线重连、浏览器连接数限制、兼容性需要考虑。 | Waiting room 可用 WebSocket 或 SSE。<br>WebSocket/SSE 负责体验和减少重复请求，不负责库存正确性。<br>客户端断线后用 queue_id 查询当前位置。 |
| DB 和 Cache 一致性 | A: Write-through cache<br>B: Cache-aside + TTL<br>C: Event-driven invalidation / incremental update | A ✅ 写 DB 同时更新 cache，看起来一致。 ❌ 并发写、失败重试、跨服务更新仍会导致 cache 不一致；不能保证强一致。<br>B ✅ 简单，cache 失效可回源。 ❌ 短时间 stale；热门 key 失效可能打爆 DB。<br>C ✅ 库存变化后推送 cache 更新或失效。 ❌ 事件丢失/乱序会导致 cache drift，需要 TTL/rebuild 兜底。 | 搜索和票区展示可用 cache-aside + TTL + incremental updates。<br>Reserve 前必须查 DB source of truth。<br>Cache 只优化读体验，不参与 correctness。 |
| 搜索加速：CDN / Query Cache / Adaptive Cache | A: CDN 缓存活动页和静态资源<br>B: Query Result Cache<br>C: Adaptive / Tiered Cache | A ✅ 极大减轻 origin 压力。 ❌ 个性化和实时库存不适合 CDN 长缓存。<br>B ✅ 直接复用非个性化查询结果。 ❌ 缓存结果不能包含用户专属价格/权限。<br>C ✅ 热门 query 短 TTL 保新鲜，冷门 query 长 TTL 省成本。 ❌ TTL 策略和 invalidation 更复杂。 | CDN 缓静态内容。<br>Search query cache 缓非个性化结果。<br>热门 query 短 TTL，冷门 query 长 TTL，库存变化用 incremental update。 |
| 异步处理和客户端结果通知 | A: 同步 reserve 后直接返回结果<br>B: 请求入 Kafka，异步处理<br>C: 同步轻量预检 + 异步最终确认 | A ✅ 用户体验直接，状态简单。 ❌ 高峰时 API tail latency 高，库存 DB 压力大。<br>B ✅ MQ 削峰，worker 可水平扩展。 ❌ 用户需要等待结果；accepted 不代表抢到票。<br>C ✅ 无 queue token、重复请求、售罄可快速失败。 ❌ 状态机更复杂。 | 热门抢票用异步处理。<br>客户端通过 polling 或 WebSocket/SSE 获取结果。<br>最终状态以 Reservation DB 为准。 |
| 支付 Hold 和超时释放 | A: reserve 后永久占票直到支付<br>B: 短 TTL hold<br>C: 支付成功后再抢票 | A ✅ 实现简单。 ❌ 恶意或犹豫用户会占住票。<br>B ✅ 用户有支付时间，未支付自动释放。 ❌ 支付成功和过期释放存在竞态。<br>C ✅ 不占库存。 ❌ 付款后没票会导致退款和投诉。 | `available -> held -> sold/available`。<br>Reservation 有 `expires_at`。<br>支付和 expiration 都使用 CAS/version，保证只有一个成功。 |
| 热点 Sharding 和容量保护 | A: 按 event_id shard<br>B: 按 event_id + section/seat range shard<br>C: Virtual queue admission control | A ✅ 同一个活动数据 locality 好。 ❌ 超级热门 event 会形成单 shard 热点。<br>B ✅ 把热门活动拆到多个 shard。 ❌ 跨 section 查询和统计复杂。<br>C ✅ 从入口把并发限制在系统可承受范围内。 ❌ 用户等待，队列系统本身要高可用。 | 默认 event_id shard。<br>热门 event 进一步按 section/seat range 拆。<br>更重要的是 virtual queue 控制进入 reserve 的并发。 |
