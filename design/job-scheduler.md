...
| --- | --- | --- | --- | --- |
| Due Job 发现：DB Polling vs Delayed Message Queue | Scheduler poll DB<br>✅ job schedule durable<br>❌ poll interval 会影响调度精度 | 创建 job 时直接放 delayed message<br>✅ 到点自动投递，scheduler 逻辑简单<br>❌ 很多 MQ 对超长 delay 支持不好 | Hybrid<br>✅ DB 存完整 schedule<br>❌ 两层状态，需要处理 DB 和 queue 的一致性 | 用 DB 作为 schedule source of truth。<br>Scheduler 提前扫描未来 N 秒，把 execution 放入 queue。 |
| Schedule Table 设计：job_id 分区 vs time_bucket 分区 | `PK=job_id, SK=next_execution`<br>✅ 查询单个 job 简单<br>❌ 找 due jobs 很难，需要 GSI 或全局扫描 | `PK=time_bucket, SK=next_execution#job_id`<br>✅ 天然支持扫描当前时间窗口<br>❌ 热门时间 bucket 会成为热点，比如整点大量任务 | `PK=time_bucket#shard_id, SK=next_execution#job_id`<br>✅ scheduler 可以并发扫多个 shard<br>❌ 查询某个用户或 job 的 schedule 需要额外 GSI | schedule 表主查询是 “哪些 job 到点了”，所以主键应围绕 time bucket 设计。<br>用 `shard_id = hash(job_id) % N` 打散同一时间 bucket。 |
| 调度准确性：Poll Interval vs Lookahead vs Delay Queue | 短 poll interval<br>✅ 容易实现<br>❌ DB 扫描压力大 | lookahead window<br>✅ 提前把未来任务拿出来，减少临界时间扫描压力<br>❌ 提前 dispatch 后，如果 job 被取消或修改，需要 queue 侧再校验... | delay queue 精准释放<br>✅ 触发时间更准确<br>❌ 取消/修改复杂 | DB poller 每几秒扫一个 lookahead window。<br>execution message 带 `scheduled_at` 和 `schedule_version`。 |
| Scheduler 和 Worker 是否分离 | Scheduler 直接执行任务<br>✅ 架构简单<br>❌ 慢任务会阻塞调度 | Scheduler 只 dispatch，Worker 执行<br>✅ 调度和执行独立扩展<br>❌ 需要 queue、execution state、幂等和 retry | 多级 scheduler<br>✅ 全局 scheduler 只分发 shard，本地 scheduler 管细粒度执行<br>❌ 架构复杂，debug 成本高 | 面试里明确说 separating query and task execution。<br>Scheduler 是控制面，Worker 是执行面。 |
| Worker Failure：Health Check Pull vs Job Leasing vs Visibility Timeout | health check pull<br>✅ 实现直观<br>❌ worker 活着不代表 job 还在正常执行 | job leasing<br>✅ 每个 execution 有 `lease_until`，过期即可重试<br>❌ 需要实现 lease CAS、heartbeat、reaper，相当于分布式锁 | SQS visibility timeout<br>✅ native 支持<br>❌ 长任务需要 heartbeat extend visibility | 如果用 SQS，优先使用 visibility timeout + heartbeat。<br>如果自建执行状态，用 job lease，但要用 CAS 防止两个 worker 同时拿到同一个 execution。 |
| Retry：Visible Failure vs Invisible Failure | visible failure<br>✅ 可以立即按照 retry policy 处理<br>❌ 只能覆盖主动失败，不能覆盖 crash、OOM、网络隔离 | invisible failure<br>✅ 通过 lease timeout / visibility timeout 可以恢复<br>❌ 恢复有延迟 | DLQ<br>✅ 隔离坏任务，避免无限重试拖垮系统<br>❌ 需要人工或自动修复流程，否则 DLQ 会变成垃圾堆 | visible failure 立即 retry with exponential backoff。<br>invisible failure 等 lease timeout 后 retry。 |
| At-least-once 和幂等 | 假设 worker 只执行一次<br>✅ 业务代码简单<br>❌ queue redelivery、worker timeout、scheduler... | deduplication table<br>✅ 通过 `job_id + execution_id` 或业务 idempotency...<br>❌ 需要业务方配合定义幂等键 | job 本身设计成幂等<br>✅ 最稳健，系统层重复投递不会破坏业务<br>❌ 不是所有业务天然幂等，需要额外状态 | Scheduler 承诺 at-least-once，不承诺 exactly-once。<br>Worker 执行前写 execution state，业务侧使用 idempotency key。 |
| Worker Runtime：Kubernetes/EC2 vs Lambda | EC2 / Kubernetes workers<br>✅ 成本可控<br>❌ 需要容量规划和运维 | Lambda<br>✅ 自动扩缩容<br>❌ 有执行时间限制、cold start、并发限制 | Hybrid<br>✅ 长任务走 K8s，短突发任务走 Lambda<br>❌ 执行环境和 observability 更复杂 | 默认 K8s/EC2 worker pool。<br>短小、突发、无状态任务可以支持 Lambda executor。 |
