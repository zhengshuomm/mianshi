...
| --- | --- | --- | --- | --- |
| Due Job 发现：DB Polling vs Delayed Message Queue | Scheduler poll DB<br>✅ job schedule durable<br>❌ poll interval 会影响调度精度 | 创建 job 时直接放 delayed message<br>✅ 到点自动投递，scheduler 逻辑简单<br>❌ 很多 MQ 对超长 delay 支持不好 | Hybrid<br>✅ DB 存完整 schedule<br>❌ 两层状态，需要处理 DB 和 queue 的一致性 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Schedule Table 设计：job_id 分区 vs time_bucket 分区 | `PK=job_id, SK=next_execution`<br>✅ 查询单个 job 简单<br>❌ 找 due jobs 很难，需要 GSI 或全局扫描 | `PK=time_bucket, SK=next_execution#job_id`<br>✅ 天然支持扫描当前时间窗口<br>❌ 热门时间 bucket 会成为热点，比如整点大量任务 | `PK=time_bucket#shard_id, SK=next_execution#job_id`<br>✅ scheduler 可以并发扫多个 shard<br>❌ 查询某个用户或 job 的 schedule 需要额外 GSI | I would not use TTL as the correctness mechanism. TTL is cleanup; correctness should come from an explicit expires_at check plus a recoverable scheduler or delayed queue that can tolerate duplicate execution and worker failure. |
| 调度准确性：Poll Interval vs Lookahead vs Delay Queue | 短 poll interval<br>✅ 容易实现<br>❌ DB 扫描压力大 | lookahead window<br>✅ 提前把未来任务拿出来，减少临界时间扫描压力<br>❌ 提前 dispatch 后，如果 job 被取消或修改，需要 queue 侧再校验... | delay queue 精准释放<br>✅ 触发时间更准确<br>❌ 取消/修改复杂 | I would put admission control in front of the expensive or scarce resource, not after it is overloaded. A bounded queue with timeout, priority, and load shedding gives predictable tail latency and makes overload behavior explicit instead of accidental. |
| Scheduler 和 Worker 是否分离 | Scheduler 直接执行任务<br>✅ 架构简单<br>❌ 慢任务会阻塞调度 | Scheduler 只 dispatch，Worker 执行<br>✅ 调度和执行独立扩展<br>❌ 需要 queue、execution state、幂等和 retry | 多级 scheduler<br>✅ 全局 scheduler 只分发 shard，本地 scheduler 管细粒度执行<br>❌ 架构复杂，debug 成本高 | I would not use TTL as the correctness mechanism. TTL is cleanup; correctness should come from an explicit expires_at check plus a recoverable scheduler or delayed queue that can tolerate duplicate execution and worker failure. |
| Worker Failure：Health Check Pull vs Job Leasing vs Visibility Timeout | health check pull<br>✅ 实现直观<br>❌ worker 活着不代表 job 还在正常执行 | job leasing<br>✅ 每个 execution 有 `lease_until`，过期即可重试<br>❌ 需要实现 lease CAS、heartbeat、reaper，相当于分布式锁 | SQS visibility timeout<br>✅ native 支持<br>❌ 长任务需要 heartbeat extend visibility | I would assume workers can crash after doing partial work. The system should use leases or visibility timeouts, bounded retries, DLQs, and idempotent handlers so recovery is a normal execution path rather than an exceptional manual repair. |
| Retry：Visible Failure vs Invisible Failure | visible failure<br>✅ 可以立即按照 retry policy 处理<br>❌ 只能覆盖主动失败，不能覆盖 crash、OOM、网络隔离 | invisible failure<br>✅ 通过 lease timeout / visibility timeout 可以恢复<br>❌ 恢复有延迟 | DLQ<br>✅ 隔离坏任务，避免无限重试拖垮系统<br>❌ 需要人工或自动修复流程，否则 DLQ 会变成垃圾堆 | I would assume workers can crash after doing partial work. The system should use leases or visibility timeouts, bounded retries, DLQs, and idempotent handlers so recovery is a normal execution path rather than an exceptional manual repair. |
| At-least-once 和幂等 | 假设 worker 只执行一次<br>✅ 业务代码简单<br>❌ queue redelivery、worker timeout、scheduler... | deduplication table<br>✅ 通过 `job_id + execution_id` 或业务 idempotency...<br>❌ 需要业务方配合定义幂等键 | job 本身设计成幂等<br>✅ 最稳健，系统层重复投递不会破坏业务<br>❌ 不是所有业务天然幂等，需要额外状态 | I would avoid promising end-to-end exactly-once. The stronger Staff-level framing is at-least-once delivery with deterministic idempotency keys, idempotent sinks, and replayable raw events; that gives us failure recovery without pretending the broker solves external side effects. |
| Worker Runtime：Kubernetes/EC2 vs Lambda | EC2 / Kubernetes workers<br>✅ 成本可控<br>❌ 需要容量规划和运维 | Lambda<br>✅ 自动扩缩容<br>❌ 有执行时间限制、cold start、并发限制 | Hybrid<br>✅ 长任务走 K8s，短突发任务走 Lambda<br>❌ 执行环境和 observability 更复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
