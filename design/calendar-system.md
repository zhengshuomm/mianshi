...
| --- | --- | --- | --- | --- |
| Source of Truth：Event-Centric vs User-Centric | 只存 event-centric record<br>✅ 数据模型简单，一份 event source of truth<br>❌ 读用户日历时需要 join attendees 和 events，延迟高 | 每个用户存一份 calendar view<br>✅ 用户打开日/周视图很快<br>❌ 写事件时要 fanout 更新多个用户 view，存在最终一致 | Hybrid<br>✅ Event DB 保持正确性，User View 提供低延迟读取<br>❌ 需要 outbox、materializer、rebuild 机制 | 我会把权威写路径和派生读模型分开：source of truth 负责正确性，materialized view 负责低延迟读。关键运营要求是所有 derived view 都能从 source of truth 或 durable log 重建和校验。 |
| 并发更新：Last Write Wins vs Optimistic Lock | Last Write Wins<br>✅ 实现简单<br>❌ 两个客户端同时改 event，后写会覆盖先写，用户难以理解 | Optimistic Lock with version<br>✅ 避免 lost update<br>❌ 冲突时客户端需要重新拉取并 merge | 字段级 merge<br>✅ 减少冲突，例如一个人改 title，另一个人改 reminder<br>❌ merge 规则复杂，部分字段不能安全 merge | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Recurring Event：预展开 vs 按需展开 | 预生成所有 future occurrences<br>✅ 查询快，实现直观<br>❌ 无限重复无法预生成 | 按查询窗口动态展开<br>✅ 存储少<br>❌ 每次查询都要计算，复杂 recurrence 可能慢 | 滚动窗口 materialization<br>✅ 未来几个月查询快<br>❌ 需要后台 job 持续补窗口和处理 exceptions | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Free/Busy 查询：实时扫描 vs Busy Index | 实时扫描 calendar events<br>✅ 结果最新，不需要额外索引<br>❌ 多人 meeting availability 查询会很慢 | 维护 busy index<br>✅ 查询快，适合批量用户<br>❌ event update 后 busy index 有延迟，需处理 stale | cache popular free/busy windows<br>✅ 降低 DB 压力<br>❌ cache invalidation 复杂，不能作为权限最终判断 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Reminder 调度：DB Polling vs Delay Queue vs Timer Wheel | DB polling<br>✅ durable，容易重建<br>❌ poll interval 影响准确性，扫描压力要控制 | Delay queue<br>✅ 到点投递，调度逻辑简单<br>❌ 长时间 future reminders、取消修改较难管理 | Hybrid timer table + short delay queue<br>✅ timer table 存长期提醒，短期进入 delay queue 精准触发<br>❌ 两层状态，需要去重和版本校验 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 邀请 Fanout：同步写所有人 vs 异步 Materialization | 同步写所有 attendee view<br>✅ 创建后所有人立即可见<br>❌ 大会议邀请会拖慢创建请求，甚至超时 | 异步 fanout<br>✅ 创建 event 快<br>❌ attendee view 最终一致，短时间内可能未显示 | 按需补齐<br>✅ 不为不活跃用户浪费写入<br>❌ 用户打开日历时首次加载可能变慢 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Privacy / Security：详情权限 vs Busy 权限 | 缓存里存完整 event 并直接返回<br>✅ 读路径简单<br>❌ 权限变化后容易泄露 private event 详情 | 读时做 final ACL check<br>✅ 权限变更能立即生效<br>❌ 读路径增加 policy check | 不同视图存不同 visibility snapshot<br>✅ 读路径快<br>❌ 权限变更时要重建视图 | 我会把 privacy 和 authorization 当成 correctness requirement。前端过滤只能改善体验，真正防泄露要靠服务端强校验、audit log、删除语义和 least-privilege access。 |
| Offline Sync：全量拉取 vs 增量 sync token | 每次全量拉取时间窗口<br>✅ 实现简单<br>❌ 移动端流量和延迟差，无法高效跨设备同步 | 增量 sync token<br>✅ 只拉变更，低流量，适合离线恢复<br>❌ 需要维护 change log 和 token 过期策略 | Push notification + incremental sync<br>✅ 体验好，延迟低<br>❌ push 丢失时仍需要 sync token 兜底 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
