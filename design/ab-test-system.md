...
| --- | --- | --- | --- | --- |
| 分流：Deterministic Hash vs Assignment Table | 每次请求写 assignment table<br>✅ 可以直接查用户分到哪个 variant<br>❌ 写 QPS 高 | deterministic hash<br>✅ 无状态、低延迟、稳定分流<br>❌ 需要严格管理 salt、experiment_id、traffic range，否则... | hybrid<br>✅ 热路径 hash，异步记录 exposure/assignment<br>❌ debug 时要同时看配置版本和曝光事件 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Traffic Allocation：扩流如何不打乱已有用户 | 重新随机分配所有用户<br>✅ 实现简单<br>❌ 用户会换组，污染实验结果 | 固定 bucket range<br>✅ 已有 bucket 不变<br>❌ 配置系统要维护 bucket ranges | 预留实验层 / namespace<br>✅ 可管理多个实验共享流量池，避免冲突<br>❌ 实验治理复杂 | 我会根据更新频率、边界问题和查询半径选择 geo index，而不是只看实现简单。高频动态位置通常更适合 H3/S2 这类分层 cell + geo-sharding，而不是单个关系型 geo index。 |
| 实验冲突：独立实验 vs Mutually Exclusive Layers | 所有实验独立分流<br>✅ 实现简单，流量利用高<br>❌ 同一用户可能同时进入多个互相影响的实验，结果难解释 | 互斥层<br>✅ 避免 interference，实验结果更可信<br>❌ 流量被切分，实验速度变慢 | Factorial design<br>✅ 可以估计 interaction effect<br>❌ 样本量需求大，分析复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Exposure Logging：Assignment 时记还是用户真正看到时记 | assignment 时记录 exposure<br>✅ 实现简单，assignment 和 exposure 一致<br>❌ 用户可能没有真正看到组件，导致曝光膨胀 | 真正 render/visible 时记录 exposure<br>✅ 更符合 “用户实际看到 treatment”<br>❌ 依赖客户端上报，可能丢事件或受 adblock/网络影响 | 两者都记录<br>✅ 可区分 assigned、served、viewed<br>❌ 事件模型更复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Metrics Pipeline：实时 vs 离线 | 只用实时 stream<br>✅ 延迟低，可以快速发现问题<br>❌ late events、重复事件、join 错误会影响准确性 | 只用离线 batch<br>✅ 准确、可重算、方便复杂 SQL<br>❌ 延迟高，不能快速发现坏实验 | Lambda-style 双路径<br>✅ stream 提供实时趋势，batch 提供最终可信结果<br>❌ 两套结果可能短暂不一致，需要解释 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 统计显著性和 Peeking | 每天看 p-value，显著就停<br>✅ 决策快<br>❌ continuous peeking 会增加 false positive | 固定样本量 / 固定实验周期<br>✅ 统计解释清楚<br>❌ 不够灵活，坏实验可能跑太久 | Sequential testing / Bayesian<br>✅ 允许更灵活地早停<br>❌ 实现和解释复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Guardrail Metrics 和自动停止 | 只看 primary metric<br>✅ 简单<br>❌ 可能提升点击率但损害延迟、crash、revenue、retention | 配置 guardrail metrics<br>✅ 保护用户体验和系统稳定性<br>❌ 指标多了会增加误报和决策复杂度 | 自动暂停<br>✅ 快速止损<br>❌ 规则配置错误会误停实验 | 我会把 observability 当成架构的一部分，而不是事后加 dashboard。关键链路需要 request_id、structured events、high-cardinality 控制、SLO alert，以及足够 raw data 支撑 debug/replay。 |
| Identity：匿名用户、登录用户和跨设备 | 只按 user_id 分流<br>✅ 稳定、简单<br>❌ 匿名流量无法实验 | 按 device_id/session_id 分流<br>✅ 覆盖匿名流量<br>❌ 同一用户跨设备可能进入不同 variant | Identity stitching<br>✅ 登录后尽量保持同一用户体验<br>❌ 合并规则复杂，可能影响实验样本 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 配置发布和故障降级 | Assignment Service 每次查 DB<br>✅ 配置最新<br>❌ DB 抖动会拖慢业务请求 | 本地配置缓存<br>✅ 低延迟，高可用<br>❌ 配置更新有传播延迟 | SDK 本地评估<br>✅ 无需每次请求远程 assignment<br>❌ 配置泄露风险、更新延迟、客户端环境不可信 | 我会默认 worker 可能在完成部分副作用后崩溃。系统需要 lease/visibility timeout、bounded retry、DLQ 和幂等 handler，让恢复成为正常执行路径，而不是依赖人工修数据。 |
