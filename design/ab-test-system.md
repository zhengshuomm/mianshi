...
|---|---|---|---|
| 分流：Deterministic Hash vs Assignment Table | A: 每次请求写 assignment table<br>B: deterministic hash<br>C: hybrid | A ✅ 可以直接查用户分到哪个 variant。 ❌ 写 QPS 高；assignment 热路径依赖 DB；扩展性差。<br>B ✅ 无状态、低延迟、稳定分流；服务可水平扩展。 ❌ 需要严格管理 salt、experiment_id、traffic range，否则改配置会导致用户换组。<br>C ✅ 热路径 hash，异步记录 exposure/assignment。 ❌ debug 时要同时看配置版本和曝光事件。 | Assignment 用 deterministic hash。<br>Exposure event 记录用户实际看到 variant 的事实。<br>不把 assignment DB 放进请求热路径。 |
| Traffic Allocation：扩流如何不打乱已有用户 | A: 重新随机分配所有用户<br>B: 固定 bucket range<br>C: 预留实验层 / namespace | A ✅ 实现简单。 ❌ 用户会换组，污染实验结果。<br>B ✅ 已有 bucket 不变；扩流只纳入新 bucket。 ❌ 配置系统要维护 bucket ranges。<br>C ✅ 可管理多个实验共享流量池，避免冲突。 ❌ 实验治理复杂。 | 使用 0-9999 或 0-999999 bucket space。<br>Control/Treatment 都是稳定区间。<br>扩流只扩大区间，不重排已有用户。 |
| 实验冲突：独立实验 vs Mutually Exclusive Layers | A: 所有实验独立分流<br>B: 互斥层<br>C: Factorial design | A ✅ 实现简单，流量利用高。 ❌ 同一用户可能同时进入多个互相影响的实验，结果难解释。<br>B ✅ 避免 interference，实验结果更可信。 ❌ 流量被切分，实验速度变慢。<br>C ✅ 可以估计 interaction effect。 ❌ 样本量需求大，分析复杂。 | 默认支持 mutually exclusive layers。<br>同页面/同 funnel 的实验放同一层。<br>独立系统层实验可以并行，但要在分析中记录 co-exposure。 |
| Exposure Logging：Assignment 时记还是用户真正看到时记 | A: assignment 时记录 exposure<br>B: 真正 render/visible 时记录 exposure<br>C: 两者都记录 | A ✅ 实现简单，assignment 和 exposure 一致。 ❌ 用户可能没有真正看到组件，导致曝光膨胀。<br>B ✅ 更符合 “用户实际看到 treatment”。 ❌ 依赖客户端上报，可能丢事件或受 adblock/网络影响。<br>C ✅ 可区分 assigned、served、viewed。 ❌ 事件模型更复杂。 | 后端逻辑实验：assignment/served 可作为 exposure。<br>UI 实验：记录 visible exposure。<br>指标计算要明确 denominator 使用哪种 exposure。 |
| Metrics Pipeline：实时 vs 离线 | A: 只用实时 stream<br>B: 只用离线 batch<br>C: Lambda-style 双路径 | A ✅ 延迟低，可以快速发现问题。 ❌ late events、重复事件、join 错误会影响准确性。<br>B ✅ 准确、可重算、方便复杂 SQL。 ❌ 延迟高，不能快速发现坏实验。<br>C ✅ stream 提供实时趋势，batch 提供最终可信结果。 ❌ 两套结果可能短暂不一致，需要解释。 | Guardrail 用 stream。<br>最终实验结论用 batch/warehouse。<br>Dashboard 标注数据 freshness 和是否 final。 |
| 统计显著性和 Peeking | A: 每天看 p-value，显著就停<br>B: 固定样本量 / 固定实验周期<br>C: Sequential testing / Bayesian | A ✅ 决策快。 ❌ continuous peeking 会增加 false positive。<br>B ✅ 统计解释清楚。 ❌ 不够灵活，坏实验可能跑太久。<br>C ✅ 允许更灵活地早停。 ❌ 实现和解释复杂。 | 面试里不用推公式，但要说明不能随便 peeking。<br>设置最小样本量、最短运行时间、guardrail 早停。<br>对业务结论使用预定义 primary metric。 |
| Guardrail Metrics 和自动停止 | A: 只看 primary metric<br>B: 配置 guardrail metrics<br>C: 自动暂停 | A ✅ 简单。 ❌ 可能提升点击率但损害延迟、crash、revenue、retention。<br>B ✅ 保护用户体验和系统稳定性。 ❌ 指标多了会增加误报和决策复杂度。<br>C ✅ 快速止损。 ❌ 规则配置错误会误停实验。 | 每个实验必须有 primary metric + guardrails。<br>Guardrails 包括 error rate、latency、crash、revenue drop。<br>高严重度 guardrail 可自动暂停，普通 guardrail 通知 owner。 |
| Identity：匿名用户、登录用户和跨设备 | A: 只按 user_id 分流<br>B: 按 device_id/session_id 分流<br>C: Identity stitching | A ✅ 稳定、简单。 ❌ 匿名流量无法实验。<br>B ✅ 覆盖匿名流量。 ❌ 同一用户跨设备可能进入不同 variant。<br>C ✅ 登录后尽量保持同一用户体验。 ❌ 合并规则复杂，可能影响实验样本。 | 实验配置明确 unit of randomization：user、device、session、org。<br>登录产品优先 user_id。<br>匿名到登录的迁移要记录 identity change，分析时可过滤或单独处理。 |
| 配置发布和故障降级 | A: Assignment Service 每次查 DB<br>B: 本地配置缓存<br>C: SDK 本地评估 | A ✅ 配置最新。 ❌ DB 抖动会拖慢业务请求。<br>B ✅ 低延迟，高可用。 ❌ 配置更新有传播延迟。<br>C ✅ 无需每次请求远程 assignment。 ❌ 配置泄露风险、更新延迟、客户端环境不可信。 | 服务端 Assignment Service + 本地 cache 是默认方案。<br>高性能后端可用 SDK 本地评估，但规则和 payload 要控制。<br>配置 cache 不可用时使用 last known good config，避免业务全挂。 |
