...
| --- | --- | --- | --- | --- |
| 分流：Deterministic Hash vs Assignment Table | 每次请求写 assignment table<br>✅ 可以直接查用户分到哪个 variant<br>❌ 写 QPS 高 | deterministic hash<br>✅ 无状态、低延迟、稳定分流<br>❌ 需要严格管理 salt、experiment_id、traffic range，否则... | hybrid<br>✅ 热路径 hash，异步记录 exposure/assignment<br>❌ debug 时要同时看配置版本和曝光事件 | Assignment 用 deterministic hash。<br>Exposure event 记录用户实际看到 variant 的事实。 |
| Traffic Allocation：扩流如何不打乱已有用户 | 重新随机分配所有用户<br>✅ 实现简单<br>❌ 用户会换组，污染实验结果 | 固定 bucket range<br>✅ 已有 bucket 不变<br>❌ 配置系统要维护 bucket ranges | 预留实验层 / namespace<br>✅ 可管理多个实验共享流量池，避免冲突<br>❌ 实验治理复杂 | 使用 0-9999 或 0-999999 bucket space。<br>Control/Treatment 都是稳定区间。 |
| 实验冲突：独立实验 vs Mutually Exclusive Layers | 所有实验独立分流<br>✅ 实现简单，流量利用高<br>❌ 同一用户可能同时进入多个互相影响的实验，结果难解释 | 互斥层<br>✅ 避免 interference，实验结果更可信<br>❌ 流量被切分，实验速度变慢 | Factorial design<br>✅ 可以估计 interaction effect<br>❌ 样本量需求大，分析复杂 | 默认支持 mutually exclusive layers。<br>同页面/同 funnel 的实验放同一层。 |
| Exposure Logging：Assignment 时记还是用户真正看到时记 | assignment 时记录 exposure<br>✅ 实现简单，assignment 和 exposure 一致<br>❌ 用户可能没有真正看到组件，导致曝光膨胀 | 真正 render/visible 时记录 exposure<br>✅ 更符合 “用户实际看到 treatment”<br>❌ 依赖客户端上报，可能丢事件或受 adblock/网络影响 | 两者都记录<br>✅ 可区分 assigned、served、viewed<br>❌ 事件模型更复杂 | 后端逻辑实验：assignment/served 可作为 exposure。<br>UI 实验：记录 visible exposure。 |
| Metrics Pipeline：实时 vs 离线 | 只用实时 stream<br>✅ 延迟低，可以快速发现问题<br>❌ late events、重复事件、join 错误会影响准确性 | 只用离线 batch<br>✅ 准确、可重算、方便复杂 SQL<br>❌ 延迟高，不能快速发现坏实验 | Lambda-style 双路径<br>✅ stream 提供实时趋势，batch 提供最终可信结果<br>❌ 两套结果可能短暂不一致，需要解释 | Guardrail 用 stream。<br>最终实验结论用 batch/warehouse。 |
| 统计显著性和 Peeking | 每天看 p-value，显著就停<br>✅ 决策快<br>❌ continuous peeking 会增加 false positive | 固定样本量 / 固定实验周期<br>✅ 统计解释清楚<br>❌ 不够灵活，坏实验可能跑太久 | Sequential testing / Bayesian<br>✅ 允许更灵活地早停<br>❌ 实现和解释复杂 | 面试里不用推公式，但要说明不能随便 peeking。<br>设置最小样本量、最短运行时间、guardrail 早停。 |
| Guardrail Metrics 和自动停止 | 只看 primary metric<br>✅ 简单<br>❌ 可能提升点击率但损害延迟、crash、revenue、retention | 配置 guardrail metrics<br>✅ 保护用户体验和系统稳定性<br>❌ 指标多了会增加误报和决策复杂度 | 自动暂停<br>✅ 快速止损<br>❌ 规则配置错误会误停实验 | 每个实验必须有 primary metric + guardrails。<br>Guardrails 包括 error rate、latency、crash、revenue drop。 |
| Identity：匿名用户、登录用户和跨设备 | 只按 user_id 分流<br>✅ 稳定、简单<br>❌ 匿名流量无法实验 | 按 device_id/session_id 分流<br>✅ 覆盖匿名流量<br>❌ 同一用户跨设备可能进入不同 variant | Identity stitching<br>✅ 登录后尽量保持同一用户体验<br>❌ 合并规则复杂，可能影响实验样本 | 实验配置明确 unit of randomization：user、device、session、org。<br>登录产品优先 user_id。 |
| 配置发布和故障降级 | Assignment Service 每次查 DB<br>✅ 配置最新<br>❌ DB 抖动会拖慢业务请求 | 本地配置缓存<br>✅ 低延迟，高可用<br>❌ 配置更新有传播延迟 | SDK 本地评估<br>✅ 无需每次请求远程 assignment<br>❌ 配置泄露风险、更新延迟、客户端环境不可信 | 服务端 Assignment Service + 本地 cache 是默认方案。<br>高性能后端可用 SDK 本地评估，但规则和 payload 要控制。 |
