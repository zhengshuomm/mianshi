...
| --- | --- | --- | --- | --- |
| Batching：自己做 scheduler vs 使用 vLLM runtime 内置 scheduler | 系统层自研 token-level scheduler<br>✅ 可以完全控制 prefill/decode、priority、KV cache、fa...<br>❌ 工程复杂度极高 | 使用 vLLM/TGI/TensorRT-LLM 内置 continuous batching<br>✅ 快速获得 continuous batching、paged attention、K...<br>❌ 对 runtime 内部策略可控性有限 | 系统层做 admission/router，runtime 内做 token-level scheduling<br>✅ 分工清晰：Router 管跨实例，runtime 管单实例 batch<br>❌ 需要 runtime 暴露足够健康和负载指标 | I would not present the system as reimplementing vLLM’s token scheduler. At Staff level, the clean boundary is: the platform owns routing, admission control, tenant isolation, and capacity policy, while the runtime owns continuous batching and KV-cache management inside a GPU worker. |
| Prefill 和 Decode：一起调度 vs 分离部署 | prefill/decode 在同一个 runtime<br>✅ 架构简单<br>❌ 长 prompt prefill 会拖慢短请求 decode | prefill/decode 分离<br>✅ 可以分别扩展 prefill 和 decode 资源<br>❌ KV cache transfer 复杂 | 按请求类型分池<br>✅ 比完全分离简单<br>❌ 资源池利用率可能下降 | I would start with a unified runtime because it is operationally simpler and avoids KV transfer. As traffic grows, I would isolate long-prefill workloads or split prefill/decode only when TTFT, TPOT, and GPU utilization data show that the added scheduling complexity is justified. |
| KV Cache：只放 GPU vs CPU offload vs prefix cache | KV cache 全放 GPU<br>✅ 访问最快，decode 延迟低<br>❌ 显存容易成为瓶颈 | CPU offload / swap<br>✅ 支持更长 context 或更多并发<br>❌ PCIe/NVLink 传输增加延迟 | prefix cache<br>✅ 降低重复 prefill 成本<br>❌ cache hit 依赖请求相似度 | I would treat KV cache as the scarce resource, not just GPU count. Routing should account for context length, prefix reuse, and memory pressure; prefix caching and long-context pools are optimizations, but correctness still comes from request cancellation, bounded queues, and accounting for generated tokens. |
| 路由策略：least-loaded vs model-locality vs SLA-aware | least-loaded routing<br>✅ 简单直观<br>❌ 不考虑 context length、KV cache、adapter、tenant... | model-locality / adapter-locality routing<br>✅ 避免频繁加载模型或 adapter<br>❌ 可能导致某些副本热点 | SLA-aware routing<br>✅ 可以按优先级、deadline、预计 TTFT 做调度<br>❌ 系统复杂，需要准确负载估计 | I would route by estimated token cost and locality, not by request count alone. A Staff-level router considers model or adapter residency, KV-cache pressure, tenant priority, and deadline, then uses admission control when no placement can meet the SLA. |
| Admission Control：让请求排队 vs 快速拒绝 | 无限排队<br>✅ 请求不容易被拒绝<br>❌ TTFT 不可控 | bounded queue + timeout<br>✅ 控制尾延迟<br>❌ 高峰期会拒绝部分请求 | priority queue + load shedding<br>✅ 高优先级请求有保障<br>❌ 公平性和实现复杂 | I would make overload explicit with bounded queues, token-based admission, timeout budgets, and priority tiers. That is better than accepting every request and letting users discover overload through unbounded TTFT or dropped streaming connections. |
| Streaming Protocol：SSE vs WebSocket vs gRPC streaming | SSE<br>✅ 简单，浏览器和 HTTP infra 友好<br>❌ 单向 server-to-client | WebSocket<br>✅ 双向通信灵活<br>❌ 连接管理复杂，LB/proxy 支持要更小心 | gRPC streaming<br>✅ 类型化、性能好、适合 service-to-service<br>❌ 浏览器外部 API 不如 SSE 直接 | I would use SSE for external token streaming because it fits browser and HTTP infrastructure, and gRPC streaming internally for typed service-to-service communication. WebSocket is worth it only when the product needs true bidirectional sessions, not just server-to-client tokens. |
| 模型发布：直接替换 vs canary/rollback | 直接替换<br>✅ 简单<br>❌ 回归影响全量用户 | canary rollout<br>✅ 小流量验证质量、延迟、错误率<br>❌ 需要 Router 支持版本流量切分 | shadow traffic<br>✅ 用户无感<br>❌ 成本高，相当于多跑一份推理 | I would version every model, adapter, prompt template, and safety policy, then release through shadow or canary before full rollout. The Staff-level point is that model quality, latency, cost, and safety regressions all need rollback and auditability, not just deployment automation. |
