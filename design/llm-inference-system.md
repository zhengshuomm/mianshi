...
| Prefill 和 Decode：一起调度 vs 分离部署 | prefill/decode 在同一个 runtime<br>✅ 架构简单<br>❌ 长 prompt prefill 会拖慢短请求 decode | prefill/decode 分离<br>✅ 可以分别扩展 prefill 和 decode 资源<br>❌ KV cache transfer 复杂 | 按请求类型分池<br>✅ 比完全分离简单<br>❌ 资源池利用率可能下降 | 先同 runtime。<br>规模上来后按长短上下文分池。 |
| KV Cache：只放 GPU vs CPU offload vs prefix cache | KV cache 全放 GPU<br>✅ 访问最快，decode 延迟低<br>❌ 显存容易成为瓶颈 | CPU offload / swap<br>✅ 支持更长 context 或更多并发<br>❌ PCIe/NVLink 传输增加延迟 | prefix cache<br>✅ 降低重复 prefill 成本<br>❌ cache hit 依赖请求相似度 | 默认 GPU KV cache + paged attention。<br>对共享 prompt 开 prefix cache。 |
| 路由策略：least-loaded vs model-locality vs SLA-aware | least-loaded routing<br>✅ 简单直观<br>❌ 不考虑 context length、KV cache、adapter、tenant... | model-locality / adapter-locality routing<br>✅ 避免频繁加载模型或 adapter<br>❌ 可能导致某些副本热点 | SLA-aware routing<br>✅ 可以按优先级、deadline、预计 TTFT 做调度<br>❌ 系统复杂，需要准确负载估计 | 基础用 model-locality + load。<br>多租户场景加入 SLA-aware admission control。 |
| Admission Control：让请求排队 vs 快速拒绝 | 无限排队<br>✅ 请求不容易被拒绝<br>❌ TTFT 不可控 | bounded queue + timeout<br>✅ 控制尾延迟<br>❌ 高峰期会拒绝部分请求 | priority queue + load shedding<br>✅ 高优先级请求有保障<br>❌ 公平性和实现复杂 | 用 bounded queue + tenant priority。<br>admission control 基于 estimated tokens，而不是单纯 QPS。 |
| Streaming Protocol：SSE vs WebSocket vs gRPC streaming | SSE<br>✅ 简单，浏览器和 HTTP infra 友好<br>❌ 单向 server-to-client | WebSocket<br>✅ 双向通信灵活<br>❌ 连接管理复杂，LB/proxy 支持要更小心 | gRPC streaming<br>✅ 类型化、性能好、适合 service-to-service<br>❌ 浏览器外部 API 不如 SSE 直接 | 外部 API 用 SSE。<br>内部 Gateway 到 Runtime 用 gRPC streaming。 |
| 模型发布：直接替换 vs canary/rollback | 直接替换<br>✅ 简单<br>❌ 回归影响全量用户 | canary rollout<br>✅ 小流量验证质量、延迟、错误率<br>❌ 需要 Router 支持版本流量切分 | shadow traffic<br>✅ 用户无感<br>❌ 成本高，相当于多跑一份推理 | 用 registry + canary + rollback。<br>高风险模型可以先 shadow，再 canary。 |
