...
| --- | --- | --- | --- | --- |
| Batching：自己做 scheduler vs 使用 vLLM runtime 内置 scheduler | 系统层自研 token-level scheduler<br>✅ 可以完全控制 prefill/decode、priority、KV cache、fa...<br>❌ 工程复杂度极高 | 使用 vLLM/TGI/TensorRT-LLM 内置 continuous batching<br>✅ 快速获得 continuous batching、paged attention、K...<br>❌ 对 runtime 内部策略可控性有限 | 系统层做 admission/router，runtime 内做 token-level scheduling<br>✅ 分工清晰：Router 管跨实例，runtime 管单实例 batch<br>❌ 需要 runtime 暴露足够健康和负载指标 | 我不会把系统设计成重新实现 vLLM 的 token scheduler。Staff 级别的边界是：平台层负责 routing、admission control、tenant isolation 和 capacity policy；runtime 内部负责 continuous batching 和 KV cache 管理。 |
| Prefill 和 Decode：一起调度 vs 分离部署 | prefill/decode 在同一个 runtime<br>✅ 架构简单<br>❌ 长 prompt prefill 会拖慢短请求 decode | prefill/decode 分离<br>✅ 可以分别扩展 prefill 和 decode 资源<br>❌ KV cache transfer 复杂 | 按请求类型分池<br>✅ 比完全分离简单<br>❌ 资源池利用率可能下降 | 我会先用同一个 runtime，降低运维复杂度并避免 KV cache 跨节点传输。规模上来后，再根据 TTFT、TPOT、GPU 利用率，把长 prompt 池化，最后才考虑 prefill/decode 分离。 |
| KV Cache：只放 GPU vs CPU offload vs prefix cache | KV cache 全放 GPU<br>✅ 访问最快，decode 延迟低<br>❌ 显存容易成为瓶颈 | CPU offload / swap<br>✅ 支持更长 context 或更多并发<br>❌ PCIe/NVLink 传输增加延迟 | prefix cache<br>✅ 降低重复 prefill 成本<br>❌ cache hit 依赖请求相似度 | 我会把 KV cache 当成核心稀缺资源，而不是只看 GPU 数量。Router 要理解 context length、prefix reuse 和显存压力；prefix cache 是优化，正确性仍靠取消请求、bounded queue 和 token accounting。 |
| 路由策略：least-loaded vs model-locality vs SLA-aware | least-loaded routing<br>✅ 简单直观<br>❌ 不考虑 context length、KV cache、adapter、tenant... | model-locality / adapter-locality routing<br>✅ 避免频繁加载模型或 adapter<br>❌ 可能导致某些副本热点 | SLA-aware routing<br>✅ 可以按优先级、deadline、预计 TTFT 做调度<br>❌ 系统复杂，需要准确负载估计 | 我不会只按 request count 做 least-loaded routing。更好的做法是按预估 token cost、模型/adapter locality、KV cache 压力、tenant priority 和 deadline 做路由；如果没有 placement 能满足 SLA，就应该 admission reject 或降级。 |
| Admission Control：让请求排队 vs 快速拒绝 | 无限排队<br>✅ 请求不容易被拒绝<br>❌ TTFT 不可控 | bounded queue + timeout<br>✅ 控制尾延迟<br>❌ 高峰期会拒绝部分请求 | priority queue + load shedding<br>✅ 高优先级请求有保障<br>❌ 公平性和实现复杂 | 我会把过载行为显式化：用 bounded queue、token-based admission、timeout budget 和 priority tier 控制尾延迟。比起无限接收请求，让用户在 TTFT 或 streaming 中被动超时，这种方式更可运营。 |
| Streaming Protocol：SSE vs WebSocket vs gRPC streaming | SSE<br>✅ 简单，浏览器和 HTTP infra 友好<br>❌ 单向 server-to-client | WebSocket<br>✅ 双向通信灵活<br>❌ 连接管理复杂，LB/proxy 支持要更小心 | gRPC streaming<br>✅ 类型化、性能好、适合 service-to-service<br>❌ 浏览器外部 API 不如 SSE 直接 | 外部 token streaming 我会优先用 SSE，因为它和浏览器、HTTP infra 更匹配；内部服务间用 gRPC streaming。只有产品真的需要双向长会话时，才引入 WebSocket。 |
| 模型发布：直接替换 vs canary/rollback | 直接替换<br>✅ 简单<br>❌ 回归影响全量用户 | canary rollout<br>✅ 小流量验证质量、延迟、错误率<br>❌ 需要 Router 支持版本流量切分 | shadow traffic<br>✅ 用户无感<br>❌ 成本高，相当于多跑一份推理 | 我会对 model、adapter、prompt template 和 safety policy 都做版本化，通过 shadow/canary 后再全量。Staff 级别要强调：质量、延迟、成本和安全回归都必须可回滚、可审计，而不是只关注部署成功。 |
