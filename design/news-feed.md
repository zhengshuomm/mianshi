...
| --- | --- | --- | --- | --- |
| SQL vs NoSQL | SQL<br>✅ 事务强，查询灵活，数据一致性好<br>❌ 大规模 timeline、follow graph、fanout 写入很难水平扩展 | NoSQL<br>✅ 按 `user_id / tweet_id` 分区，吞吐高，扩展简单<br>❌ 不能依赖复杂 JOIN，需要为查询模式设计表 | Hybrid<br>✅ User/account 配置可用 SQL，tweet/timeline 用 NoS...<br>❌ 多套存储带来一致性和运维复杂度 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Push vs Pull | Pull / fanout-on-read<br>✅ 写 tweet 很便宜<br>❌ 读 feed 时要查很多 followees 并 merge，延迟高 | Push / fanout-on-write<br>✅ 读 feed 很快，直接读 home timeline cache<br>❌ 明星用户发 tweet 写放大巨大 | Hybrid<br>✅ 普通用户 push，大 V pull，平衡读写成本<br>❌ 读路径要合并 pushed timeline 和 celebrity pull re... | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Active User Cache 策略 | 缓存所有用户 timeline<br>✅ 读路径简单<br>❌ 大量不活跃用户浪费内存 | 只缓存活跃用户<br>✅ 内存成本可控<br>❌ 不活跃用户回来时首次读取较慢 | 分层缓存<br>✅ hot users timeline 放内存，warm users 放 SSD/No...<br>❌ 系统复杂，cache consistency 更难 | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
| Partition / Replication | 按 `user_id` 分区 timeline<br>✅ 读某个用户 feed 很快<br>❌ 超级活跃用户或热门用户可能产生热点 | 按 `tweet_id` / `author_id` 分区 tweets<br>✅ 作者 timeline 查询自然<br>❌ 读 home feed 需要批量跨分区取 tweet body | 多副本 + read replica<br>✅ 提升读吞吐和可用性<br>❌ 复制延迟导致读到旧数据 | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
| Cache Invalidation 和 Privacy | 写时清理所有 cache<br>✅ cache 更干净<br>❌ 删除 tweet、block、private change 可能需要清理海量 tim... | 读时过滤<br>✅ 避免大规模 cache invalidation<br>❌ 读路径多一次 policy check | 异步清理 + 读时兜底<br>✅ 最终清理缓存，同时保证读时不会泄露不可见内容<br>❌ 实现复杂，需要 policy version 或 visibility check | I would keep cache off the correctness path unless it is guarded by versioned writes or a single-writer model. The safer default is source-of-truth first, then asynchronous cache updates with stale-read fallback and reconciliation for hot or critical entities. |
| Follow/Unfollow 后 Timeline 如何处理 | follow 时立即 backfill<br>✅ 体验好<br>❌ follow 大量用户时写放大明显 | unfollow 时立即删除 timeline 里的历史 tweets<br>✅ 用户不会再看到 unfollow 对象内容<br>❌ 需要扫描 timeline cache，成本高 | lazy rebuild / read-time filter<br>✅ 写路径轻<br>❌ timeline 里可能有 stale candidate，需要读时过滤 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Monitoring / Alert | 只看服务 QPS/latency/error<br>✅ 容易落地<br>❌ 看不出 feed 质量问题，比如 timeline stale | 加 pipeline freshness 指标<br>✅ 能发现 MQ lag、fanout delay、cache rebuild 慢<br>❌ 需要端到端 trace 和业务指标 | synthetic users / canary feed<br>✅ 能检测“发 tweet 后 follower 是否看到”的真实体验<br>❌ 需要维护测试账号和预期结果 | I would design observability as part of the architecture, not a dashboard added later. Each critical path needs request IDs, structured events, high-cardinality controls, SLO-oriented alerts, and enough raw data to debug or replay incidents. |
