...
| --- | --- | --- | --- | --- |
| 下单顺序：先写 DB vs 先提交 Exchange | 先提交外部 exchange，再写 DB<br>✅ 路径看起来短<br>❌ 如果 submit 成功但 DB 写失败，本地失去订单追踪，风险极高 | 先写 DB，再异步提交 exchange<br>✅ 任何订单先有本地记录，可审计、可恢复<br>❌ 用户看到 created 后，外部提交可能失败，需要状态机表达 | DB + outbox transaction<br>✅ 保证本地订单和提交任务一起持久化<br>❌ outbox relay、重复提交和幂等处理复杂 | 先写 Order DB，再通过 transactional outbox 提交外部系统。<br>订单状态从 `created -> submitting -> submitted/rejected/submit_failed`。 |
| 外部订单 ID：internal order_id vs client_order_id vs external_order_id | 只用 internal `order_id`<br>✅ 简单<br>❌ 外部 broker 无法用本地 ID 做幂等或对账 | 使用 `external_order_id`<br>✅ 对账和查询外部状态方便<br>❌ 提交前没有 external id | 使用 `client_order_id`<br>✅ 本地生成，提交前就存在<br>❌ 外部 venue 必须支持，且要保证全局唯一 | 三个 ID 都保留：<br>`order_id`：内部 source of truth。 |
| 取消订单：本地取消 vs 外部取消 | 只改本地状态为 cancelled<br>✅ 快，简单<br>❌ 如果订单已到 exchange，本地取消不代表市场取消 | 提交 cancel request 到 exchange<br>✅ 符合真实交易语义<br>❌ cancel request 和 fill 可能竞态 | cancel pending 状态机<br>✅ 准确表达 “取消请求已发出但未确认”<br>❌ 客户端体验更复杂，需要解释 pending | 未提交外部：`created -> cancelled`。<br>已提交外部：`submitted/partial_filled -> cancel_pending -> cancelled`。 |
| 失败场景：DB 写失败、提交失败、提交后处理失败 | Failure to store order<br>✅ 简单<br>❌ 复杂度更高 | Failure submit order to exchange<br>✅ 简单<br>❌ 复杂度更高 | Failure processing after exchange submission<br>✅ 简单<br>❌ 复杂度更高 | 交易系统里最重要的是区分 `failed` 和 `unknown`。<br>对未知状态不要盲目重试新订单；先用 `client_order_id` 查询外部状态。 |
| 订单状态一致性：同步更新 vs Event-driven | Execution Gateway 直接写 Order DB<br>✅ 路径短<br>❌ 外部接入逻辑和订单状态机耦合，难审计 | Execution Gateway 发标准化 order events<br>✅ Order Service 统一管理状态机<br>❌ event bus 至少一次投递，需要幂等和乱序处理 | Event sourcing<br>✅ 所有状态由事件重放得到，审计清晰<br>❌ 实现复杂，查询需要 projection | 使用 order_events 作为审计流水，Order DB 存当前状态 projection。<br>状态转移用 version/CAS。 |
| Live Price Update：Redis Pub/Sub vs Kafka vs WebSocket/SSE | Redis Pub/Sub + SSE<br>✅ 轻量、延迟低<br>❌ Redis Pub/Sub 不持久 | Kafka market data stream<br>✅ 持久化、可 replay、消费者扩展性好<br>❌ 端到端延迟和 fanout 复杂度高于 Redis | WebSocket<br>✅ 双向、低延迟<br>❌ 连接管理和扩展复杂 | 行情内部可 Kafka 持久化，在线推送用 Redis Pub/Sub + SSE/WebSocket。<br>SSE server 可以订阅多个 symbol topic，但热点 symbol 要做 topic fanout 分层。 |
| Order Updates：Polling vs SSE | Client polling<br>✅ 简单、可靠、断线无状态<br>❌ 状态变化有延迟，QPS 高 | SSE order stream<br>✅ 用户体验好，server-to-client 简单<br>❌ 长连接需要资源 | SSE + polling fallback<br>✅ 实时体验和可靠补齐兼顾<br>❌ 客户端要处理去重和状态 merge | Order updates 用 SSE 推送，同时保留 `GET /orders/{id}`。<br>SSE 只传状态变化通知，Order DB 是最终状态。 |
| Portfolio / Buying Power：强同步扣减 vs Ledger | 只存当前 balance/position<br>✅ 查询简单<br>❌ 审计困难，故障后很难解释余额变化 | ledger + projection<br>✅ 每笔 hold、release、fill、fee 都可追溯<br>❌ 读当前余额需要 projection，设计更复杂 | 同步强一致 portfolio update<br>✅ 防止超额购买<br>❌ 会增加下单延迟和锁竞争 | 下单前同步 reserve buying power。<br>fill 后通过 ledger entry 更新实际 cash/position。 |
