...
|---|---|---|---|
| Object Name vs Object ID | A: 直接用 object name 存数据<br>B: Routing service 生成 UUID `object_id`<br>C: content-addressed object id | A ✅ 模型简单。 ❌ 覆盖、版本、rename-like 行为、并发写都会复杂；object name 可能很长且分布不均。<br>B ✅ data plane 用稳定短 ID；同名对象多版本天然隔离；placement 可以和 object name 解耦。 ❌ 必须维护 metadata mapping。<br>C ✅ 相同内容可 dedup；checksum 自然校验。 ❌ 流式上传时要先算 hash；覆盖语义仍需 metadata 指向。 | 用 UUID/ULID 作为 `object_id`。<br>用户查询永远从 `bucket + object_name` 查 metadata，再定位 `object_id`。<br>`object_name` 是 namespace，`object_id` 是 data identity。 |
| Versioning：覆盖写 vs 多版本 | A: 覆盖写直接替换老数据<br>B: 每次 PUT 生成新 version<br>C: Copy-on-write + lifecycle policy | A ✅ 存储成本低，读 current 简单。 ❌ 误删/误覆盖不可恢复；并发读写可能读到中间状态。<br>B ✅ 同名 object 保留历史；publish 新版本可以原子切指针。 ❌ 存储成本上升；list versions 和 GC 更复杂。<br>C ✅ 保留近期版本，按规则清理旧版本。 ❌ lifecycle/compaction 需要小心，不能误删仍可访问版本。 | 同一个 object name 的不同 version 对应不同 `object_id`。<br>Metadata 原子更新 current pointer。<br>Lifecycle policy 后台清理过期版本。 |
| Replication vs Erasure Coding | A: Replication<br>B: Erasure Coding<br>C: Hybrid | A ✅ 读简单；恢复快；单副本丢失直接从其他副本复制。 ❌ 存储开销高，例如 3 副本就是 3x。<br>B ✅ durability 高，存储开销低于多副本。 ❌ 读取和修复更复杂；小对象 EC 开销不划算；重建需要读多个 shard。<br>C ✅ 热数据先 replication，冷却后转 EC，兼顾性能和成本。 ❌ 后台转换、元数据 layout 更新和 repair 更复杂。 | 新写入先多副本，快速 ack 和低延迟读取。<br>后台把冷大对象 compaction/转换为 EC。<br>Manifest 记录 layout type，读路径按 layout 读取。 |
| Multipart Upload | A: 单请求上传大文件<br>B: Multipart upload<br>C: Resumable upload with client checkpoint | A ✅ 实现简单。 ❌ 大文件失败要重传全部；单连接吞吐受限；服务端内存压力大。<br>B ✅ part 并行上传；失败只重传单个 part；支持断点续传。 ❌ 需要管理 upload session、part etag、complete/abort 和 orphan parts。<br>C ✅ 客户端可恢复进度。 ❌ 协议和状态管理更复杂。 | 大文件必须支持 multipart。<br>未 complete 前不能 publish object version。<br>Complete 是原子 metadata 操作：校验 parts 后生成 manifest 并切 current version。 |
| Soft Delete 和 Delete Marker | A: 立即物理删除<br>B: Soft delete<br>C: Versioned delete marker | A ✅ 释放空间快。 ❌ 误删不可恢复；并发读可能失败；跨副本删除难以原子。<br>B ✅ 删除快速，只改 metadata；后台慢慢 GC data；支持恢复窗口。 ❌ 需要 GC 和 compaction，存储短期不会释放。<br>C ✅ DELETE 不删除历史版本，只让 current view 表现为不存在。 ❌ 用户 list/delete versions 语义更复杂。 | 默认 metadata soft delete。<br>versioning bucket 写 delete marker。<br>物理 chunk 删除由 GC 在安全窗口后执行。 |
| Metadata 一致性：强一致 vs 最终一致 | A: Metadata 最终一致<br>B: 单 region 强一致 metadata<br>C: Region 内强一致，跨 region 最终一致 | A ✅ 写入延迟低，跨区域复制简单。 ❌ PUT 后立即 GET/LIST 可能看不到新对象，用户体验差。<br>B ✅ PUT 完成后 GET current version 一致；delete marker 立即生效。 ❌ metadata DB 设计复杂，跨 region 强一致成本高。<br>C ✅ 本地体验好，跨区复制成本可控。 ❌ 跨 region 读可能看到旧版本，需要版本号和 replication status。 | Region 内 metadata 强一致。<br>Data replication 可以先满足写 quorum，再异步补齐。<br>Cross-region replication 明确是 eventually consistent。 |
| Compaction / GC | A: 不做 compaction<br>B: 后台 GC<br>C: Compaction + layout rewrite | A ✅ 实现简单。 ❌ delete marker、旧版本、orphan multipart parts、小 chunk 会持续占空间。<br>B ✅ 异步释放不可达数据，不影响前台请求延迟。 ❌ 必须保证不会误删仍被 metadata 引用的数据。<br>C ✅ 合并小对象、转换 EC、降低 metadata 和存储成本。 ❌ 重写期间有新旧 layout 共存，元数据切换要原子。 | GC 使用 mark-and-sweep 思路：从 Metadata DB 标记可达 object/chunks。<br>删除前保留 safety window。<br>Compaction 生成新 manifest 后原子切换，再异步删除旧 chunks。 |
| 路由和热点对象 | A: 每次都经 Routing Service 转发数据<br>B: Routing Service 返回 storage node / pre-signed URL<br>C: CDN / cache for hot objects | A ✅ 客户端简单。 ❌ Routing Service 变成 data plane 瓶颈。<br>B ✅ 数据直传 storage nodes，metadata/control plane 更轻。 ❌ 客户端协议更复杂，安全 token 和过期要处理。<br>C ✅ 保护 origin storage，降低延迟。 ❌ cache invalidation 和 version/etag 语义要清楚。 | Metadata/control 走 Routing Service。<br>大对象 data path 尽量 client direct to storage node。<br>热对象通过 CDN 和 replica fanout 扩展读取。 |
