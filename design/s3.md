...
| --- | --- | --- | --- | --- |
| Object Name vs Object ID | 直接用 object name 存数据<br>✅ 模型简单<br>❌ 覆盖、版本、rename-like 行为、并发写都会复杂 | Routing service 生成 UUID `object_id`<br>✅ data plane 用稳定短 ID<br>❌ 必须维护 metadata mapping | content-addressed object id<br>✅ 相同内容可 dedup<br>❌ 流式上传时要先算 hash | I would choose storage based on access patterns and recovery semantics, not just scale. The key Staff-level trade-off is write amplification, read latency, replication consistency, and how quickly the system can recover from WAL, snapshot, or replay. |
| Versioning：覆盖写 vs 多版本 | 覆盖写直接替换老数据<br>✅ 存储成本低，读 current 简单<br>❌ 误删/误覆盖不可恢复 | 每次 PUT 生成新 version<br>✅ 同名 object 保留历史<br>❌ 存储成本上升 | Copy-on-write + lifecycle policy<br>✅ 保留近期版本，按规则清理旧版本<br>❌ lifecycle/compaction 需要小心，不能误删仍可访问版本 | I would version the artifact that changes behavior: model, config, manifest, policy, or index schema. Versioning enables canary, rollback, shadow comparison, and auditability, which is more important than making the first rollout fast. |
| Replication vs Erasure Coding | Replication<br>✅ 读简单<br>❌ 存储开销高，例如 3 副本就是 3x | Erasure Coding<br>✅ durability 高，存储开销低于多副本<br>❌ 读取和修复更复杂 | Hybrid<br>✅ 热数据先 replication，冷却后转 EC，兼顾性能和成本<br>❌ 后台转换、元数据 layout 更新和 repair 更复杂 | I would choose storage based on access patterns and recovery semantics, not just scale. The key Staff-level trade-off is write amplification, read latency, replication consistency, and how quickly the system can recover from WAL, snapshot, or replay. |
| Multipart Upload | 单请求上传大文件<br>✅ 实现简单<br>❌ 大文件失败要重传全部 | Multipart upload<br>✅ part 并行上传<br>❌ 需要管理 upload session、part etag、complete/abo... | Resumable upload with client checkpoint<br>✅ 客户端可恢复进度<br>❌ 协议和状态管理更复杂 | I would keep the control plane and data plane separate. Metadata, authorization, and manifest generation protect correctness, while CDN/object storage carries bulk traffic; heavy processing such as transcoding should be an idempotent workflow. |
| Soft Delete 和 Delete Marker | 立即物理删除<br>✅ 释放空间快<br>❌ 误删不可恢复 | Soft delete<br>✅ 删除快速，只改 metadata<br>❌ 需要 GC 和 compaction，存储短期不会释放 | Versioned delete marker<br>✅ DELETE 不删除历史版本，只让 current view 表现为不存在<br>❌ 用户 list/delete versions 语义更复杂 | I would define retention by data purpose: raw logs for replay, compacted state for serving, and cold storage for audit. Compaction and GC save cost, but they must not remove data required for recovery, compliance, or user-visible correctness. |
| Metadata 一致性：强一致 vs 最终一致 | Metadata 最终一致<br>✅ 写入延迟低，跨区域复制简单<br>❌ PUT 后立即 GET/LIST 可能看不到新对象，用户体验差 | 单 region 强一致 metadata<br>✅ PUT 完成后 GET current version 一致<br>❌ metadata DB 设计复杂，跨 region 强一致成本高 | Region 内强一致，跨 region 最终一致<br>✅ 本地体验好，跨区复制成本可控<br>❌ 跨 region 读可能看到旧版本，需要版本号和 replication statu... | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| Compaction / GC | 不做 compaction<br>✅ 实现简单<br>❌ delete marker、旧版本、orphan multipart parts、小... | 后台 GC<br>✅ 异步释放不可达数据，不影响前台请求延迟<br>❌ 必须保证不会误删仍被 metadata 引用的数据 | Compaction + layout rewrite<br>✅ 合并小对象、转换 EC、降低 metadata 和存储成本<br>❌ 重写期间有新旧 layout 共存，元数据切换要原子 | I would define retention by data purpose: raw logs for replay, compacted state for serving, and cold storage for audit. Compaction and GC save cost, but they must not remove data required for recovery, compliance, or user-visible correctness. |
| 路由和热点对象 | 每次都经 Routing Service 转发数据<br>✅ 客户端简单<br>❌ Routing Service 变成 data plane 瓶颈 | Routing Service 返回 storage node / pre-signed URL<br>✅ 数据直传 storage nodes，metadata/control plane...<br>❌ 客户端协议更复杂，安全 token 和过期要处理 | CDN / cache for hot objects<br>✅ 保护 origin storage，降低延迟<br>❌ cache invalidation 和 version/etag 语义要清楚 | I would design for the hottest entity, not the average QPS. Start with natural partitioning, then add salting, per-entity single writers, shard-local top-N, or admission limits only where the measured hotspot requires it. |
