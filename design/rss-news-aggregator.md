...
| --- | --- | --- | --- | --- |
| Pagination Consistency：Offset vs Cursor vs Snapshot Cursor | Offset pagination<br>✅ 实现简单<br>❌ 新闻 feed 持续插入新文章，翻页会重复或漏文章 | 基于 article_id / rank_time 的 cursor<br>✅ 比 offset 稳定<br>❌ 个性化混排多个 feed 时，一个 cursor 不够表达每个 feed 的进度 | Snapshot cursor<br>✅ 固定 `snapshot_watermark_article_id`，翻页期间新文章...<br>❌ cursor 更大，服务端/客户端要处理版本和过期 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| Low Latency Feed：Redis TTL Cache vs Real-time Cached Feeds with CDC | Redis cache with TTL<br>✅ 实现简单，读取低延迟<br>❌ TTL 内 stale | Real-time cached feeds with CDC<br>✅ Article DB 变更后主动更新 feed cache，freshness 好<br>❌ CDC/consumer 失败会导致 cache drift，需要 rebuild | Hybrid<br>✅ CDC 增量更新，TTL/rebuild 兜底<br>❌ 多一层复杂度 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 30 分钟 Freshness：Webhook + Polling | 只靠 polling<br>✅ 系统可控，不依赖 publisher 主动通知<br>❌ 延迟取决于轮询间隔 | 只靠 webhook<br>✅ 延迟低，抓取少<br>❌ webhook 可能丢、延迟、签名错误，不能保证覆盖所有 source | Webhook + polling<br>✅ webhook 加速，polling 兜底，满足 30 分钟 SLA<br>❌ 需要去重，避免 webhook 和 polling 重复入库 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| Media Content：Inline Proxy vs S3 + CDN | 直接引用 publisher image URL<br>✅ 不用存储媒体<br>❌ 外链失效、慢、被防盗链，无法统一尺寸 | 后端 proxy 图片<br>✅ 控制访问<br>❌ 后端带宽压力大 | S3 + CDN<br>✅ 低延迟、高可用、可生成 thumbnails、多尺寸<br>❌ 多了媒体处理 pipeline 和版权/过期策略 | 我会把 control plane 和 data plane 分开。metadata、authorization、manifest 保护正确性；CDN/object storage 承载大流量；transcoding 这类重任务应该是幂等、可恢复的 workflow。 |
| Personalization：预生成用户 feed vs Request-time Mixing | 为每个用户预生成完整 feed<br>✅ 读请求非常快<br>❌ 用户多时写放大巨大 | 请求时混合基础 feeds<br>✅ 不为每个用户写 feed<br>❌ 读路径要 merge 多个 feeds，cursor 更复杂 | Hybrid<br>✅ 活跃用户预计算，普通用户 request-time mixing<br>❌ 系统复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Breaking News Boost | 全局替换用户 feed 为 trending<br>✅ 最快让所有用户看到<br>❌ 破坏个性化体验，可能不相关 | 临时提升 trending 权重<br>✅ 保留用户偏好，同时提高重大新闻曝光<br>❌ ranking 规则更复杂，需要 decay | 按 topic/region 相关性 boost<br>✅ 更精准<br>❌ 需要事件分类和地域识别 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Dedup 和 Canonicalization | 按 URL 完全匹配<br>✅ 实现简单<br>❌ tracking params、http/https、移动端 URL 会导致重复 | URL canonicalization + hash<br>✅ 去掉 utm、排序 query params、normalize host/path...<br>❌ 不同文章可能 URL 相似，仍需 source_guid/title 辅助 | 内容相似度 dedup<br>✅ 能合并不同 URL 的同源新闻<br>❌ 计算成本高，误合并风险 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| Feed Index 和 Cache Consistency | 写文章时同步更新所有 feed index/cache<br>✅ 文章立即可见<br>❌ 写放大大，文章入库延迟高 | CDC 异步更新 feed index<br>✅ Article DB 写路径轻<br>❌ Feed cache 最终一致，短时间看不到新文章 | Lazy update<br>✅ 不为没人看的 feed 维护缓存<br>❌ 首次请求慢 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
