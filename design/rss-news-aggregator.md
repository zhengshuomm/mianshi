...
| --- | --- | --- | --- | --- |
| Pagination Consistency：Offset vs Cursor vs Snapshot Cursor | Offset pagination<br>✅ 实现简单<br>❌ 新闻 feed 持续插入新文章，翻页会重复或漏文章 | 基于 article_id / rank_time 的 cursor<br>✅ 比 offset 稳定<br>❌ 个性化混排多个 feed 时，一个 cursor 不够表达每个 feed 的进度 | Snapshot cursor<br>✅ 固定 `snapshot_watermark_article_id`，翻页期间新文章...<br>❌ cursor 更大，服务端/客户端要处理版本和过期 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| Low Latency Feed：Redis TTL Cache vs Real-time Cached Feeds with CDC | Redis cache with TTL<br>✅ 实现简单，读取低延迟<br>❌ TTL 内 stale | Real-time cached feeds with CDC<br>✅ Article DB 变更后主动更新 feed cache，freshness 好<br>❌ CDC/consumer 失败会导致 cache drift，需要 rebuild | Hybrid<br>✅ CDC 增量更新，TTL/rebuild 兜底<br>❌ 多一层复杂度 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
| 30 分钟 Freshness：Webhook + Polling | 只靠 polling<br>✅ 系统可控，不依赖 publisher 主动通知<br>❌ 延迟取决于轮询间隔 | 只靠 webhook<br>✅ 延迟低，抓取少<br>❌ webhook 可能丢、延迟、签名错误，不能保证覆盖所有 source | Webhook + polling<br>✅ webhook 加速，polling 兜底，满足 30 分钟 SLA<br>❌ 需要去重，避免 webhook 和 polling 重复入库 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| Media Content：Inline Proxy vs S3 + CDN | 直接引用 publisher image URL<br>✅ 不用存储媒体<br>❌ 外链失效、慢、被防盗链，无法统一尺寸 | 后端 proxy 图片<br>✅ 控制访问<br>❌ 后端带宽压力大 | S3 + CDN<br>✅ 低延迟、高可用、可生成 thumbnails、多尺寸<br>❌ 多了媒体处理 pipeline 和版权/过期策略 | I would keep the control plane and data plane separate. Metadata, authorization, and manifest generation protect correctness, while CDN/object storage carries bulk traffic; heavy processing such as transcoding should be an idempotent workflow. |
| Personalization：预生成用户 feed vs Request-time Mixing | 为每个用户预生成完整 feed<br>✅ 读请求非常快<br>❌ 用户多时写放大巨大 | 请求时混合基础 feeds<br>✅ 不为每个用户写 feed<br>❌ 读路径要 merge 多个 feeds，cursor 更复杂 | Hybrid<br>✅ 活跃用户预计算，普通用户 request-time mixing<br>❌ 系统复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Breaking News Boost | 全局替换用户 feed 为 trending<br>✅ 最快让所有用户看到<br>❌ 破坏个性化体验，可能不相关 | 临时提升 trending 权重<br>✅ 保留用户偏好，同时提高重大新闻曝光<br>❌ ranking 规则更复杂，需要 decay | 按 topic/region 相关性 boost<br>✅ 更精准<br>❌ 需要事件分类和地域识别 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| Dedup 和 Canonicalization | 按 URL 完全匹配<br>✅ 实现简单<br>❌ tracking params、http/https、移动端 URL 会导致重复 | URL canonicalization + hash<br>✅ 去掉 utm、排序 query params、normalize host/path...<br>❌ 不同文章可能 URL 相似，仍需 source_guid/title 辅助 | 内容相似度 dedup<br>✅ 能合并不同 URL 的同源新闻<br>❌ 计算成本高，误合并风险 | I would avoid promising end-to-end exactly-once. The stronger Staff-level framing is at-least-once delivery with deterministic idempotency keys, idempotent sinks, and replayable raw events; that gives us failure recovery without pretending the broker solves external side effects. |
| Feed Index 和 Cache Consistency | 写文章时同步更新所有 feed index/cache<br>✅ 文章立即可见<br>❌ 写放大大，文章入库延迟高 | CDC 异步更新 feed index<br>✅ Article DB 写路径轻<br>❌ Feed cache 最终一致，短时间看不到新文章 | Lazy update<br>✅ 不为没人看的 feed 维护缓存<br>❌ 首次请求慢 | I would make the source-of-truth write the correctness boundary, then publish changes through outbox or CDC. Cache, search index, realtime push, and read models are derived state; they can be eventually consistent as long as events are versioned, idempotent, and replayable. |
