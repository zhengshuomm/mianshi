...
|---|---|---|---|
| Pagination Consistency：Offset vs Cursor vs Snapshot Cursor | A: Offset pagination<br>B: 基于 article_id / rank_time 的 cursor<br>C: Snapshot cursor | A ✅ 实现简单。 ❌ 新闻 feed 持续插入新文章，翻页会重复或漏文章。<br>B ✅ 比 offset 稳定；`WHERE article_id < cursor` 查询高效。 ❌ 个性化混排多个 feed 时，一个 cursor 不够表达每个 feed 的进度。<br>C ✅ 固定 `snapshot_watermark_article_id`，翻页期间新文章不会插队；per_feed_offsets 保证混排稳定。 ❌ cursor 更大，服务端/客户端要处理版本和过期。 | Topic feed 用 monotonic article_id cursor。<br>Personalized feed 用 snapshot cursor + per feed offsets。<br>Refresh 首页才更新 snapshot，翻页沿用旧 snapshot。 |
| Low Latency Feed：Redis TTL Cache vs Real-time Cached Feeds with CDC | A: Redis cache with TTL<br>B: Real-time cached feeds with CDC<br>C: Hybrid | A ✅ 实现简单，读取低延迟。 ❌ TTL 内 stale；TTL 过期时可能 cache stampede。<br>B ✅ Article DB 变更后主动更新 feed cache，freshness 好。 ❌ CDC/consumer 失败会导致 cache drift，需要 rebuild。<br>C ✅ CDC 增量更新，TTL/rebuild 兜底。 ❌ 多一层复杂度。 | 热门 feed 用 Redis ZSET + CDC 实时更新。<br>Redis key 设置 TTL 或定期 rebuild 防漂移。<br>Cache miss 从 Feed Index/Article DB 回源。 |
| 30 分钟 Freshness：Webhook + Polling | A: 只靠 polling<br>B: 只靠 webhook<br>C: Webhook + polling | A ✅ 系统可控，不依赖 publisher 主动通知。 ❌ 延迟取决于轮询间隔；抓取成本高。<br>B ✅ 延迟低，抓取少。 ❌ webhook 可能丢、延迟、签名错误，不能保证覆盖所有 source。<br>C ✅ webhook 加速，polling 兜底，满足 30 分钟 SLA。 ❌ 需要去重，避免 webhook 和 polling 重复入库。 | Webhook 进入高优先级 pipeline。<br>Polling 以 source priority 调整频率，确保大部分 source 30 分钟内发现新文章。<br>Dedup 用 canonical_url_hash/source_guid。 |
| Media Content：Inline Proxy vs S3 + CDN | A: 直接引用 publisher image URL<br>B: 后端 proxy 图片<br>C: S3 + CDN | A ✅ 不用存储媒体。 ❌ 外链失效、慢、被防盗链，无法统一尺寸。<br>B ✅ 控制访问。 ❌ 后端带宽压力大。<br>C ✅ 低延迟、高可用、可生成 thumbnails、多尺寸。 ❌ 多了媒体处理 pipeline 和版权/过期策略。 | Media Processor 异步下载和生成 thumbnail。<br>S3 存 object，CDN 分发。<br>文章先入 feed，thumbnail 可后补。 |
| Personalization：预生成用户 feed vs Request-time Mixing | A: 为每个用户预生成完整 feed<br>B: 请求时混合基础 feeds<br>C: Hybrid | A ✅ 读请求非常快。 ❌ 用户多时写放大巨大；偏好变化要重建。<br>B ✅ 不为每个用户写 feed；偏好实时生效。 ❌ 读路径要 merge 多个 feeds，cursor 更复杂。<br>C ✅ 活跃用户预计算，普通用户 request-time mixing。 ❌ 系统复杂。 | 基础 feeds：`feed:technology:US`、`feed:business:US`、`feed:trending:US`。<br>大多数用户 request-time mixing。<br>超活跃用户可预计算 home feed cache。 |
| Breaking News Boost | A: 全局替换用户 feed 为 trending<br>B: 临时提升 trending 权重<br>C: 按 topic/region 相关性 boost | A ✅ 最快让所有用户看到。 ❌ 破坏个性化体验，可能不相关。<br>B ✅ 保留用户偏好，同时提高重大新闻曝光。 ❌ ranking 规则更复杂，需要 decay。<br>C ✅ 更精准。 ❌ 需要事件分类和地域识别。 | Trending feed 有时间衰减 score。<br>Breaking news 临时提高 trending 权重，例如 10% -> 25%，但不完全覆盖个人偏好。<br>严重公共安全事件可走 emergency override。 |
| Dedup 和 Canonicalization | A: 按 URL 完全匹配<br>B: URL canonicalization + hash<br>C: 内容相似度 dedup | A ✅ 实现简单。 ❌ tracking params、http/https、移动端 URL 会导致重复。<br>B ✅ 去掉 utm、排序 query params、normalize host/path 后更准。 ❌ 不同文章可能 URL 相似，仍需 source_guid/title 辅助。<br>C ✅ 能合并不同 URL 的同源新闻。 ❌ 计算成本高，误合并风险。 | 先用 `source_guid` 和 canonical_url_hash。<br>对高价值/热门新闻再做 title/content similarity 合并。<br>Dedup id 写唯一约束，确保 webhook+polling 重复不会双入库。 |
| Feed Index 和 Cache Consistency | A: 写文章时同步更新所有 feed index/cache<br>B: CDC 异步更新 feed index<br>C: Lazy update | A ✅ 文章立即可见。 ❌ 写放大大，文章入库延迟高。<br>B ✅ Article DB 写路径轻；Feed Builder 可水平扩展。 ❌ Feed cache 最终一致，短时间看不到新文章。<br>C ✅ 不为没人看的 feed 维护缓存。 ❌ 首次请求慢。 | 热门 feed：CDC 增量维护 Redis/Feed Index。<br>冷门 feed：lazy build + TTL。<br>Feed API 需要能从 durable Feed Index 回源，cache 不是 source of truth。 |
