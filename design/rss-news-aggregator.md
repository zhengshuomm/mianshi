...
| --- | --- | --- | --- | --- |
| Pagination Consistency：Offset vs Cursor vs Snapshot Cursor | Offset pagination<br>✅ 实现简单<br>❌ 新闻 feed 持续插入新文章，翻页会重复或漏文章 | 基于 article_id / rank_time 的 cursor<br>✅ 比 offset 稳定<br>❌ 个性化混排多个 feed 时，一个 cursor 不够表达每个 feed 的进度 | Snapshot cursor<br>✅ 固定 `snapshot_watermark_article_id`，翻页期间新文章...<br>❌ cursor 更大，服务端/客户端要处理版本和过期 | Topic feed 用 monotonic article_id cursor。<br>Personalized feed 用 snapshot cursor + per feed offsets。 |
| Low Latency Feed：Redis TTL Cache vs Real-time Cached Feeds with CDC | Redis cache with TTL<br>✅ 实现简单，读取低延迟<br>❌ TTL 内 stale | Real-time cached feeds with CDC<br>✅ Article DB 变更后主动更新 feed cache，freshness 好<br>❌ CDC/consumer 失败会导致 cache drift，需要 rebuild | Hybrid<br>✅ CDC 增量更新，TTL/rebuild 兜底<br>❌ 多一层复杂度 | 热门 feed 用 Redis ZSET + CDC 实时更新。<br>Redis key 设置 TTL 或定期 rebuild 防漂移。 |
| 30 分钟 Freshness：Webhook + Polling | 只靠 polling<br>✅ 系统可控，不依赖 publisher 主动通知<br>❌ 延迟取决于轮询间隔 | 只靠 webhook<br>✅ 延迟低，抓取少<br>❌ webhook 可能丢、延迟、签名错误，不能保证覆盖所有 source | Webhook + polling<br>✅ webhook 加速，polling 兜底，满足 30 分钟 SLA<br>❌ 需要去重，避免 webhook 和 polling 重复入库 | Webhook 进入高优先级 pipeline。<br>Polling 以 source priority 调整频率，确保大部分 source 30 分钟内发现新文章。 |
| Media Content：Inline Proxy vs S3 + CDN | 直接引用 publisher image URL<br>✅ 不用存储媒体<br>❌ 外链失效、慢、被防盗链，无法统一尺寸 | 后端 proxy 图片<br>✅ 控制访问<br>❌ 后端带宽压力大 | S3 + CDN<br>✅ 低延迟、高可用、可生成 thumbnails、多尺寸<br>❌ 多了媒体处理 pipeline 和版权/过期策略 | Media Processor 异步下载和生成 thumbnail。<br>S3 存 object，CDN 分发。 |
| Personalization：预生成用户 feed vs Request-time Mixing | 为每个用户预生成完整 feed<br>✅ 读请求非常快<br>❌ 用户多时写放大巨大 | 请求时混合基础 feeds<br>✅ 不为每个用户写 feed<br>❌ 读路径要 merge 多个 feeds，cursor 更复杂 | Hybrid<br>✅ 活跃用户预计算，普通用户 request-time mixing<br>❌ 系统复杂 | 基础 feeds：`feed:technology:US`、`feed:business:US`、`feed:trending:US`。<br>大多数用户 request-time mixing。 |
| Breaking News Boost | 全局替换用户 feed 为 trending<br>✅ 最快让所有用户看到<br>❌ 破坏个性化体验，可能不相关 | 临时提升 trending 权重<br>✅ 保留用户偏好，同时提高重大新闻曝光<br>❌ ranking 规则更复杂，需要 decay | 按 topic/region 相关性 boost<br>✅ 更精准<br>❌ 需要事件分类和地域识别 | Trending feed 有时间衰减 score。<br>Breaking news 临时提高 trending 权重，例如 10% -> 25%，但不完全覆盖个人偏好。 |
| Dedup 和 Canonicalization | 按 URL 完全匹配<br>✅ 实现简单<br>❌ tracking params、http/https、移动端 URL 会导致重复 | URL canonicalization + hash<br>✅ 去掉 utm、排序 query params、normalize host/path...<br>❌ 不同文章可能 URL 相似，仍需 source_guid/title 辅助 | 内容相似度 dedup<br>✅ 能合并不同 URL 的同源新闻<br>❌ 计算成本高，误合并风险 | 先用 `source_guid` 和 canonical_url_hash。<br>对高价值/热门新闻再做 title/content similarity 合并。 |
| Feed Index 和 Cache Consistency | 写文章时同步更新所有 feed index/cache<br>✅ 文章立即可见<br>❌ 写放大大，文章入库延迟高 | CDC 异步更新 feed index<br>✅ Article DB 写路径轻<br>❌ Feed cache 最终一致，短时间看不到新文章 | Lazy update<br>✅ 不为没人看的 feed 维护缓存<br>❌ 首次请求慢 | 热门 feed：CDC 增量维护 Redis/Feed Index。<br>冷门 feed：lazy build + TTL。 |
