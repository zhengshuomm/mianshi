...
| --- | --- | --- | --- | --- |
| 实时协议：Polling vs SSE vs WebSocket | Polling<br>✅ 实现简单，纯 HTTP，负载均衡和重试都容易<br>❌ 延迟取决于轮询间隔 | SSE<br>✅ 基于 HTTP，浏览器原生支持自动重连<br>❌ 单向通道 | WebSocket<br>✅ 双向、低延迟、协议灵活<br>❌ 连接管理复杂 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| Pub/Sub 分发：pull-based partition vs push-based routing | partitioned pub/sub pull<br>✅ 天然有 backpressure<br>❌ 订阅关系变化频繁时，很多 worker 可能拉到没有本地观众的消息 | 直接 push 到 SSE / WebSocket server<br>✅ 只发给有观众的节点，延迟低，内网无效流量少<br>❌ 路由表更新频繁 | Hybrid<br>✅ 热门视频走 push routing，冷门视频走共享消费池，成本和延迟平衡<br>❌ 系统路径更多，调试和容量规划更复杂 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| 让看相同视频的人到哪里：单节点 vs server group vs 完全随机 | 同一视频 sticky 到单个 server<br>✅ 本地 broadcast 最简单<br>❌ 热门视频会把单节点打爆 | 同一视频 sticky 到 server group<br>✅ fanout 只打到这个 video 的 server group<br>❌ 需要管理 group membership、连接迁移、负载不均 | 完全随机分配连接<br>✅ LB 简单，连接分布均匀<br>❌ 一个热门视频的评论可能要 fanout 到几乎所有 stream servers | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| Routing metadata 如何维护 | 只维护 `user_id -> node_id`<br>✅ 简单，按用户推送直接<br>❌ live comment 是按 video fanout，仅有 user routi... | 维护 `video_id -> list(node_id)`<br>✅ fanout 范围明确，不需要遍历所有用户<br>❌ 用户进出视频频繁，列表更新和过期清理压力大 | 三张 routing 表组合<br>✅ 支持不同查询方向<br>❌ 元数据一致性复杂，需要 TTL、heartbeat 和幂等更新 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| 历史评论分页：DB cursor vs Elasticsearch search_after vs cache | DB cursor<br>✅ 稳定、正确、容易和 source of truth 对齐<br>❌ 复杂搜索、过滤、审核条件不灵活 | Elasticsearch `search_after`<br>✅ 复杂查询能力强，深分页比 offset 更稳定<br>❌ 索引最终一致，不适合作为评论正确性的唯一来源 | recent comments cache<br>✅ 延迟低，能明显减轻 DB 压力<br>❌ cache 可能过期或缺失，需要 DB fallback | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| 多 DC 同步：同步写 vs CDC 异步 vs video home region | 同步跨 DC 写<br>✅ 一致性强，读任何 DC 都更接近最新<br>❌ 写延迟高 | CDC / Kafka 异步复制<br>✅ 本地写低延迟<br>❌ 会有延迟、重复、乱序，需要幂等和排序策略 | video home region<br>✅ per-video ordering 更清晰<br>❌ 远端用户发评论可能跨区写，延迟上升 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| 热点与不均衡：固定容量 vs 动态扩容 vs 降级 | 固定 server group<br>✅ 实现简单，容量规划直观<br>❌ 热点突发时容易过载 | 动态扩容 server group<br>✅ 可以把资源集中给热门视频<br>❌ 扩容时需要更新 routing、迁移新连接、处理旧连接 | 服务降级<br>✅ 保护核心写入和观看体验<br>❌ 用户看到的评论可能不是全量实时评论 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
| 推送可靠性：实时推送 vs cursor 补齐 vs per-user inbox | 只靠实时推送<br>✅ 实现简单，延迟最低<br>❌ 连接断开或 server failure 会丢事件 | 实时推送 + cursor 补齐<br>✅ 在线低延迟，断线后可恢复<br>❌ 客户端需要 merge、dedup 和维护 cursor | per-user inbox<br>✅ delivery tracking 精确<br>❌ 公开 live comment 会产生巨大写放大，不适合大规模观看场景 | 先给简单可运营方案，再说明规模/一致性要求变化时如何演进。 |
