...
| --- | --- | --- | --- | --- |
| 多阶段推荐：召回 vs 粗排 vs 精排 | 全量视频直接精排<br>✅ 理论上排序最准确<br>❌ 视频量巨大，不可行 | 两阶段：召回 + 精排<br>✅ 架构简单，适合中等规模<br>❌ 召回候选太多时精排压力高 | 多阶段：召回 + 粗排 + 精排 + 重排<br>✅ 可扩展，延迟可控<br>❌ 系统复杂，调参和 debug 难 | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| 召回策略 | Trending recall<br>✅ 冷启动好，稳定<br>❌ 不够个性化 | Embedding ANN recall<br>✅ 个性化强，能发现长尾兴趣<br>❌ embedding 更新和 ANN index freshness 有成本 | Social/follow recall<br>✅ 用户关注关系强相关<br>❌ 只靠关注会变窄 | I would frame this as both an ML quality problem and a serving-systems problem. The Staff-level points are feature freshness, training-serving consistency, bias and guardrail metrics, plus a safe fallback when the model, feature store, or ranking service is unhealthy. |
| Ranking Objective | 优化 CTR<br>✅ 简单，反馈多<br>❌ 标题党和低质量短点击可能上升 | 优化 watch time<br>✅ 更贴近短视频消费<br>❌ 可能推荐低价值但上瘾内容 | 多目标优化<br>✅ 平衡留存、互动、质量、安全<br>❌ 目标权重难调 | I would separate the write path for scores from the read model for ranks. Redis sorted sets are excellent for hot Top-K serving, but durable score history, anti-cheat review, and shard-level aggregation should remain recoverable from the source of truth. |
| 实时兴趣建模 | 只用长期画像<br>✅ 稳定<br>❌ 无法捕捉用户当下意图 | 只用 session 行为<br>✅ 响应快<br>❌ 容易被短期噪声带偏 | 长期 + 短期融合<br>✅ 既稳定又灵活<br>❌ 特征和模型更复杂 | I would treat realtime delivery as a visibility optimization, not as the source of truth. Online users get low-latency push, but reconnects must use a cursor or current-state read to recover missed events; that keeps the system correct when gateways, pub/sub, or clients fail. |
| 探索 vs 利用 | 完全按模型最高分<br>✅ 短期指标高<br>❌ 新视频和新兴趣无法获得曝光 | 固定比例随机探索<br>✅ 简单<br>❌ 体验波动大 | Bandit / uncertainty-based exploration<br>✅ 更智能地探索高潜力内容<br>❌ 实现复杂 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 多样性和去重 | 只按 score 排<br>✅ 简单<br>❌ feed 可能被同 topic/creator 洗屏 | 规则重排<br>✅ 容易控制体验<br>❌ 规则多了会压低模型效果 | Diversity-aware reranking<br>✅ 在 score 和多样性之间平衡<br>❌ 需要 topic/creator embedding 和调参 | I would avoid promising end-to-end exactly-once. The stronger Staff-level framing is at-least-once delivery with deterministic idempotency keys, idempotent sinks, and replayable raw events; that gives us failure recovery without pretending the broker solves external side effects. |
| 内容安全和质量 | 推荐后再过滤<br>✅ 实现简单<br>❌ 浪费 ranking 资源，且可能漏 | 入库时审核 + 推荐时过滤<br>✅ 双重保护<br>❌ 审核延迟影响新视频分发 | 质量分参与排序<br>✅ 低质量内容自然降权<br>❌ 质量模型误判会影响创作者 | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
| 评价指标 | - | - | - | I would start by naming the invariant this deep dive protects, then choose the simplest option that preserves that invariant under retries, partial failure, and hotspot traffic. Everything outside that boundary should be asynchronous, observable, and rebuildable. |
