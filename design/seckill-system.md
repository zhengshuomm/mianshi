...
| --- | --- | --- | --- | --- |
| 库存扣减：DB 直接扣减 vs Redis 预扣 vs 库存令牌 | DB 直接扣减<br>✅ source of truth 简单<br>❌ 秒杀峰值会打爆热点库存行，write contention 严重 | Redis 原子预扣<br>✅ 低延迟，高吞吐<br>❌ Redis 和 DB 存在一致性问题，需要异步落库和 reconciliation | 库存令牌预生成<br>✅ 只有拿到 token 的请求进入订单链路，天然削峰<br>❌ token 发出但订单创建失败时，需要回收或补偿 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| 防超卖：Pessimistic Lock vs Optimistic Lock vs Atomic Operation | Pessimistic lock<br>✅ 正确性直观<br>❌ 秒杀热点行锁等待严重，吞吐差 | Optimistic lock<br>✅ 无锁等待，平时性能好<br>❌ 秒杀场景冲突极高，大量 CAS 失败重试会放大压力 | Atomic conditional update / Lua<br>✅ 单次原子判断 `stock > 0` 并扣减，延迟低<br>❌ 单 key 仍可能成为 Redis 热点 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| Push 请求还是排队：直接打下单 API vs Virtual Queue | 所有用户直接请求下单<br>✅ 架构简单，用户无等待<br>❌ 活动开始瞬间后端会被洪峰打穿 | Virtual queue<br>✅ 保护下单核心链路<br>❌ 实现排队公平性、过期、重复入队、断线恢复更复杂 | Token gate<br>✅ 更强削峰，流量可控<br>❌ 用户体验更硬，可能被认为不公平 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| 同一用户重复请求：前端禁用 vs Idempotency vs Unique Constraint | 前端禁用按钮<br>✅ 简单，减少误触<br>❌ 无法防刷新、重试、脚本请求、网络超时 | Idempotency key<br>✅ 客户端重试返回同一 `request_id`，不会重复入队<br>❌ 需要保存 key 和请求 hash，处理过期策略 | DB unique constraint<br>✅ 最终兜底，防止一个用户创建多单<br>❌ 如果只靠 DB，重复请求已经进入下游，浪费资源 | 我不会轻易承诺端到端 exactly-once。更稳的 Staff 表达是 at-least-once delivery + deterministic idempotency key + idempotent sink + raw event replay，这样失败恢复可控，也不会把外部副作用交给 broker 幻想解决。 |
| 同步创建订单 vs 异步创建订单 | 同步创建订单<br>✅ 用户立刻拿到订单结果<br>❌ 订单 DB、支付、库存都暴露在秒杀峰值下 | 异步创建订单<br>✅ 下单 API 快速返回<br>❌ 用户需要查询 `request_id` | 同步预占 + 异步订单<br>✅ Redis 预扣成功后用户基本抢到资格<br>❌ 订单落库失败要补偿，系统复杂 | 我会先说清这个 deep dive 保护哪个 invariant，再选择能在 retry、partial failure、hotspot traffic 下守住该 invariant 的最简单方案。边界之外的派生链路要异步、可观测、可重建。 |
| 支付超时：库存一直占用 vs Expiration 状态机 | 抢到后永久占库存直到用户支付<br>✅ 实现简单<br>❌ 恶意用户可以占库存不付款，库存利用率低 | 10 分钟支付 hold<br>✅ 用户有支付时间<br>❌ 支付 callback 和过期取消有竞态 | 支付成功后再确认库存<br>✅ 不占库存<br>❌ 支付后没货体验差，退款复杂 | 我会先定义业务 invariant，比如不超卖、唯一 winner、一次成功扣款。关键状态转移用 conditional write 或状态机保护；所有外部副作用都要幂等，并通过异步 reconciliation 修正异常。 |
| Redis 和 DB 一致性：强同步 vs 异步落库 vs Reconciliation | Redis 扣减后同步写 DB<br>✅ 结果更快落到 source of truth<br>❌ DB 仍在高峰路径上，容易拖慢请求 | Redis 扣减后写 MQ，异步落库<br>✅ 入口低延迟<br>❌ MQ/worker 失败时需要补偿和重放 | DB 预生成库存流水，Redis 只是缓存<br>✅ 审计更清楚<br>❌ 预生成和回收 token 成本更高 | 我会让 source-of-truth write 成为 correctness boundary，然后通过 outbox 或 CDC 发布变更。cache、search index、realtime push、read model 都是 derived state，可以最终一致，但事件必须 versioned、idempotent、replayable。 |
| 结果通知：Polling vs SSE | Client polling<br>✅ 实现简单，兼容性好，无长连接管理<br>❌ QPS 增加，结果有几秒延迟 | SSE<br>✅ 用户体验好，适合 server-to-client 状态更新<br>❌ 长连接占资源，断线重连和浏览器连接数限制要处理 | WebSocket<br>✅ 低延迟双向通信<br>❌ 秒杀结果通知一般不需要这么重 | 我会把实时推送当成 visibility optimization，而不是 source of truth。在线用户低延迟 push；断线重连必须用 cursor 或 current-state read 补齐，这样 gateway、pub/sub、client 任意失败都不影响最终正确性。 |
| 防刷和安全：限流 vs 验证码 vs 风控 | IP/user/device 限流<br>✅ 简单有效，能挡明显洪峰<br>❌ 代理 IP、设备伪造会绕过 | 验证码 / 签名 token<br>✅ 提高脚本成本<br>❌ 影响用户体验 | 风控评分<br>✅ 可以结合账号历史、设备、行为、IP 信誉<br>❌ 模型和规则复杂，可能误杀 | 我会把 admission control 放在昂贵或稀缺资源之前，而不是系统过载之后。bounded queue、timeout、priority、load shedding 能让尾延迟和失败模式可预测。 |
