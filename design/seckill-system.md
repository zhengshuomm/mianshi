...
| --- | --- | --- | --- | --- |
| 库存扣减：DB 直接扣减 vs Redis 预扣 vs 库存令牌 | DB 直接扣减<br>✅ source of truth 简单<br>❌ 秒杀峰值会打爆热点库存行，write contention 严重 | Redis 原子预扣<br>✅ 低延迟，高吞吐<br>❌ Redis 和 DB 存在一致性问题，需要异步落库和 reconciliation | 库存令牌预生成<br>✅ 只有拿到 token 的请求进入订单链路，天然削峰<br>❌ token 发出但订单创建失败时，需要回收或补偿 | 秒杀主路径用 Redis token / 原子预扣。<br>DB 是最终库存账本。 |
| 防超卖：Pessimistic Lock vs Optimistic Lock vs Atomic Operation | Pessimistic lock<br>✅ 正确性直观<br>❌ 秒杀热点行锁等待严重，吞吐差 | Optimistic lock<br>✅ 无锁等待，平时性能好<br>❌ 秒杀场景冲突极高，大量 CAS 失败重试会放大压力 | Atomic conditional update / Lua<br>✅ 单次原子判断 `stock > 0` 并扣减，延迟低<br>❌ 单 key 仍可能成为 Redis 热点 | Redis Lua 做入口原子扣减。<br>DB 落库用唯一约束和库存流水保证最终账本。 |
| Push 请求还是排队：直接打下单 API vs Virtual Queue | 所有用户直接请求下单<br>✅ 架构简单，用户无等待<br>❌ 活动开始瞬间后端会被洪峰打穿 | Virtual queue<br>✅ 保护下单核心链路<br>❌ 实现排队公平性、过期、重复入队、断线恢复更复杂 | Token gate<br>✅ 更强削峰，流量可控<br>❌ 用户体验更硬，可能被认为不公平 | 普通活动直接限流 + Redis 预扣。<br>超热点活动使用 Redis virtual queue。 |
| 同一用户重复请求：前端禁用 vs Idempotency vs Unique Constraint | 前端禁用按钮<br>✅ 简单，减少误触<br>❌ 无法防刷新、重试、脚本请求、网络超时 | Idempotency key<br>✅ 客户端重试返回同一 `request_id`，不会重复入队<br>❌ 需要保存 key 和请求 hash，处理过期策略 | DB unique constraint<br>✅ 最终兜底，防止一个用户创建多单<br>❌ 如果只靠 DB，重复请求已经进入下游，浪费资源 | 前端禁用只是体验。<br>Seckill Service 用 idempotency key 和 Redis `user_claim` 快速去重。 |
| 同步创建订单 vs 异步创建订单 | 同步创建订单<br>✅ 用户立刻拿到订单结果<br>❌ 订单 DB、支付、库存都暴露在秒杀峰值下 | 异步创建订单<br>✅ 下单 API 快速返回<br>❌ 用户需要查询 `request_id` | 同步预占 + 异步订单<br>✅ Redis 预扣成功后用户基本抢到资格<br>❌ 订单落库失败要补偿，系统复杂 | 秒杀用同步 Redis 预占 + 异步订单创建。<br>明确返回 `accepted/queued`，不要承诺订单已创建。 |
| 支付超时：库存一直占用 vs Expiration 状态机 | 抢到后永久占库存直到用户支付<br>✅ 实现简单<br>❌ 恶意用户可以占库存不付款，库存利用率低 | 10 分钟支付 hold<br>✅ 用户有支付时间<br>❌ 支付 callback 和过期取消有竞态 | 支付成功后再确认库存<br>✅ 不占库存<br>❌ 支付后没货体验差，退款复杂 | 订单创建后 `pending_payment`，带 `expires_at`。<br>支付成功只能 `pending_payment -> paid`。 |
| Redis 和 DB 一致性：强同步 vs 异步落库 vs Reconciliation | Redis 扣减后同步写 DB<br>✅ 结果更快落到 source of truth<br>❌ DB 仍在高峰路径上，容易拖慢请求 | Redis 扣减后写 MQ，异步落库<br>✅ 入口低延迟<br>❌ MQ/worker 失败时需要补偿和重放 | DB 预生成库存流水，Redis 只是缓存<br>✅ 审计更清楚<br>❌ 预生成和回收 token 成本更高 | Redis 预扣 + MQ 异步落库。<br>OrderDB/InventoryDB 是最终事实。 |
| 结果通知：Polling vs SSE | Client polling<br>✅ 实现简单，兼容性好，无长连接管理<br>❌ QPS 增加，结果有几秒延迟 | SSE<br>✅ 用户体验好，适合 server-to-client 状态更新<br>❌ 长连接占资源，断线重连和浏览器连接数限制要处理 | WebSocket<br>✅ 低延迟双向通信<br>❌ 秒杀结果通知一般不需要这么重 | 下单结果默认 polling。<br>Virtual queue 位置可以用 SSE。 |
| 防刷和安全：限流 vs 验证码 vs 风控 | IP/user/device 限流<br>✅ 简单有效，能挡明显洪峰<br>❌ 代理 IP、设备伪造会绕过 | 验证码 / 签名 token<br>✅ 提高脚本成本<br>❌ 影响用户体验 | 风控评分<br>✅ 可以结合账号历史、设备、行为、IP 信誉<br>❌ 模型和规则复杂，可能误杀 | Gateway 限流 + WAF + captcha/token + risk score。<br>下单 token 短期有效，并绑定 user/device/activity。 |
